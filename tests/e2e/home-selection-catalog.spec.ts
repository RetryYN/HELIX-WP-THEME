...
for (const [device, width] of [['pc', 1440], ['sp', 375]] as const) {
  test(`finished HOME selection, comparison facts and saved reason / ${device}`, async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath(`home-catalog-${device}.png`), fullPage:true });
