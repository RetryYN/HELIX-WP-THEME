...
for (const [device, width] of [['pc', 1440], ['sp', 375]] as const) {
  test(`finished EVENT selection, comparison facts and saved reason / ${device}`, async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath(`event-catalog-${device}.png`), fullPage:true });
