...
for (const [device, width] of [['pc', 1440], ['sp', 375]] as const) {
  test(`finished EVENT selection, comparison facts and saved reason / ${device}`, async ({ page }) => {
    await page.setViewportSize({ width, height: 950 });
    await page.goto(base + route);
    await page.locator('[data-face="event"]').click();
    await expect(page.locator('#event-start')).toBeVisible();
    await expect(page.locator('.tile-open').first()).toHaveAttribute('data-entry-id', 'event-finished:seminar');
    await page.locator('#event-finished-only').click();
    ...
    await expect(page.locator('#comparison-facts')).toContainText('補助情報');
    await expect(page.locator('#comparison-facts')).toContainText('共通部品の所属');
    for (const label of ['対象と判断', '申込方式', '会場案内', '補助情報', '受付不能時', '区間の順序']) await expect(page.locator('#comparison-facts th[scope="row"]', { hasText: label })).toHaveCount(1);
    const first = page.locator('#compare .compare-grid>section').first();
    await expect(first.locator('img')).toHaveAttribute('src', new RegExp(`after-seminar-${device}\\.jpg$`));
    await first.getByRole('button', { name:'保留', exact:true }).click();
    await first.locator('textarea').fill('入口の案内と本文の情報量を比較してから選ぶ');
    await page.keyboard.press('Escape');
    await page.reload();
    await expect(page.locator('#search')).toHaveValue('完成EVENT');
    await expect(page.locator('.tile-open')).toHaveCount(5);
    ...
    await page.screenshot({ path:`docs/research/2026-09-15-event-completion/catalog-${device}.png`, fullPage:true });
    await page.locator('#event-with-parts').click();
