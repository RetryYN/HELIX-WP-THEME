...
test('AC-B tampered rollback snapshot is rejected', () => { const state = structuredClone(fixture); const receipt = dryRun(state, patches.value); const applied = apply(state, patches.value, receipt); const tamperedPoint = structuredClone(applied.rollbackPoint); tamperedPoint.before.revision = 999; expect(() => rollback(applied.state, tamperedPoint)).toThrow('Rollback snapshot digest mismatch'); });
...
test('AC-VALUE-03A token projection preserves parent six-level scales', () => { const projected = projectTokenLayer(tokenProjectionFixture.parent, tokenProjectionFixture.bridge); expect(projected.typography.slugs).toHaveLength(6); expect(projected.spacing.slugs).toHaveLength(6); expect(projected.widths.slugs).toEqual(['content', 'wide']); expect(projected.typography.slugs).toEqual(tokenProjectionFixture.parent.typography.slugs); expect(projected.spacing.slugs).toEqual(tokenProjectionFixture.parent.spacing.slugs); expect(projected.spacing.values[3]).toBe('2.25rem'); });
test('AC-VALUE-03B token projection rejects scale or settings changes', () => { const removed = structuredClone(tokenProjectionFixture.bridge); removed.spacing.slugs.pop(); expect(() => projectTokenLayer(tokenProjectionFixture.parent, removed)).toThrow('spacing projection changes slugs'); const renamed = structuredClone(tokenProjectionFixture.bridge); renamed.typography.slugs[0] = 'tiny'; expect(() => projectTokenLayer(tokenProjectionFixture.parent, renamed)).toThrow('typography projection changes slugs'); const override = structuredClone(tokenProjectionFixture.bridge); override.settings = { typography: { fontSizes: [] } }; expect(() => projectTokenLayer(tokenProjectionFixture.parent, override)).toThrow('settings override is forbidden'); });
