...
| PM-05 | WT-UI-05 | 主たる確認面（既定 SP）、共通 + device 別差分、SP / PC 両幅の語彙・PR・JSON-LD・A/B variant・SNS share・CV・バナー・タグ / 同意・監査バッジの受け皿有無を WT-OT-02 / 08 / 14 / 15 / 16 / 17 / 18 / 20 / 21 へ接続する |
| PM-06 | WT-UI-06 | SP / PC 両幅の積層規約違反、CollectionPage 欠落、画像最適化、feed・資料 DL・バナー・同意状態の公開面確認を WT-OT-02 / 09 / 14 / 15 / 16 / 18 / 21 へ接続する |
| PM-07 | WT-UI-07 | 静的 FAIL=0、SP / PC 操作境界、タグ同意前非発火、Consent Mode v2 写像、性能予算、Lighthouse / Core Web Vitals の blocking gate、実機 invalid=0 の同一 HEAD 束縛率を WT-OT-05 / 10 / 18 / 19 / 21 の前提として記録する |
| PM-08 | WT-UI-08 | manifest 列挙率、SP / PC 両幅プレビュー、共通 + device 別差分、データ層 / Consent Mode v2 同意契約、第三者プラグイン領域別検出、差分 API、MCP / REST / WP-CLI の能力集合一致、監査 export、manifest 外指定の拒否率を WT-OT-01 / 03 / 11 / 12 / 16 / 17 / 20 / 22 / 23 へ接続する |
