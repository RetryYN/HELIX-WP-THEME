...
| WT-OT-02 | WT-BR-02 | 12 種別の必須パーツ充足率、未整備 16 項目と 2026-09-03 採用 13 件の受け皿有無 |
...
| WT-OT-17 | WT-BR-02 | SP 既定の面・語彙・パーツ・SP プレビューの要求カバレッジと、PC 派生 / theme.json 尺度の宣言一致率 |
| WT-OT-18 | WT-BR-03 | SP ヘッダー・ドロワー・下部固定・専用広告面の選択率、SP 語彙挙動の JSON 宣言率、未宣言面の出力件数（0 であること） |
| WT-OT-19 | WT-BR-03 | SP の 44px / 16px / 横スクロール 0・固定面の本文 / CTA 被覆 0、mobile Lighthouse・SP 主体 CWV・device type 別 A/B / CV 集計の実施率 |
| WT-OT-20 | WT-BR-01 | head・body 開始直後・body 終端のタグ slot 充足率、version 付きデータ層契約の必須 ID 充足率、UI / MCP / DB 選択の一致率 |
| WT-OT-21 | WT-BR-03 | 同意前の計測 / 広告タグ非発火率、同意なしイベント保存件数（0）、slot 外スクリプト件数（0）、タグ転送の性能予算内率 |
