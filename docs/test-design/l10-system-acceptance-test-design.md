...
| WT-AT-VOCAB-01 | WT-FR-VOCAB-01 | 14 語彙それぞれに受け皿（core + style / 新規ブロック）が対応表で 1 対 1 に決まり、実使用上位 7 種と販売系 4 種が描画され、新規ブロックは 6 つ + 空き 1 枠（上限 7）に限定される / メディア枠の 5 択を切り替えても語彙の本文構造が変わらず、画像は alt と width / height を持つ | 受け皿のない語彙、比較専用テーブルを本文中の手組み比較表と同じ語彙で扱うこと、または 8 つ目の新規ブロックがあれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
...
| WT-AT-VOCAB-03 | WT-FR-VOCAB-03 | 広告パーツ、アフィリエイトリンク、または商品リンクを含む記事にだけ、テーマの最小文字サイズ以上・AA コントラスト・記事上部の固定 1 箇所を満たす控えめな PR 表記がファーストビュー内に自動で出て、含まない記事には出ない。ボタン・バナーのすぐ近くには置かず、表示デザインと表示ページ制御が選べる / 既定文言は「本記事にはプロモーションが含まれます。」で、CTA・商品カード束の不要な PR バッジを出さない。本文表記との重複抑止と欠落防止の両立は pending_resolution 解決後に判定する | 対象記事で表記が欠落する、対象外の記事に出る、テーマの最小文字サイズまたは AA コントラストを下回る、ボタン・バナーのすぐ近くに出る、または本文編集で消せれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
...
| WT-AT-LOOK-01 | WT-FR-LOOK-01 | 見出し尺度が単調非増加（G-T3 PASS）で、style と variant が block style として列挙される / 同一列のカードの高さが本文量に関わらず一致し、見出しが既定寸法で 1 行に収まり、型数が用途別一覧の参照 ID を持つ / 型の用途説明と見出し・囲み・CTA・比較表・グラフ・ブログカード・関連一覧・画像処理・4 軸の拡張を台帳へ対応付け、PC / SP の表示不具合と無効 JS / reduced-motion を検査する | 生値や !important で実現された装飾があれば FAIL / reduced-motion 下で装飾モーションが動く、JS 無効で要素が消える、または写真・色地上の文字が 4.5:1（大文字 3:1）未満で出れば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
| WT-AT-LOOK-02 | WT-FR-LOOK-02 | 写像した variation がスラッグ集合を変えず G-T1b PASS | 段の増減や新スラッグを伴う variation があれば FAIL | G-T1b JSON |
| WT-AT-LOOK-03 | WT-FR-LOOK-03 | サイトパターンごとに調査証跡（対象数・採取項目・分布）が digest 束縛され、そこから導出した variation / block style が G-T1b / G-T3 PASS / 未観察パターンを調査対象として保持し、索引の追加が可能で、未検証を対応済みと数えない | 調査証跡のないサイトパターンの variation が要求または実装に入れば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
...
| WT-AT-LP-01 | WT-FR-LP-01 | LP が CPT として作成でき、ディレクトリに依存しない URL で公開され、種別（通常 / イベント / 比較特設）が REST と manifest から列挙できる。固定ページは階層 URL のまま / LP / BLP の目的差、イベント / 比較の管理単位を JSON の種別で識別できる | LP が固定ページ階層の配下 URL になる、または種別が JSON で列挙できなければ FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
...
| WT-AT-PAGE-01 | WT-FR-PAGE-01 | 固定ページ群のパターンと事業者情報の自動充填が設定 JSON から生成され、自前フォームなしの譲渡境界が表示される | 固定ページの情報が分散、外部送信ページが手書きだけ、自前フォームや認証情報をテーマが保持、または第三者フォームとの二重送信があれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
...
| WT-AT-PAGE-02 | WT-FR-PAGE-02 | 9 系統と元 7 分類の対応、別軸、出所・採否・証拠状態・着手順を列挙でき、追加種別を既存種別の削除なしで記録できる | 未観察を範囲外へ落とす、骨格採用を個別設計の承認と扱う、または未検証を対応済みと表示すれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
| WT-AT-HOME-01 | WT-FR-HOME-01 | HOME の hero・区間・導線を PC / SP で切り替え、サイドバー開始位置と共通部品の所属を確認できる | HOME を記事の代替表示だけで済ませる、first view をサイドバーが押し崩す、または他面の独自設定が漏れれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
| WT-AT-EVENT-01 | WT-FR-EVENT-01 | PC / SP で区間・申込・受付状態を切り替え、地図とサイドバーの所属を確認し、期限・満席・受付前の遷移は専用 fixture で別途判定する | クエリでの状態表示だけを期限による遷移成功と扱う、無選択の外部埋め込みを読み込む、または申込導線が消失すれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
| WT-AT-PAID-01 | WT-FR-PAID-01 | 代表モックで無料記事と独立した管理単位を確認し、買い切り / 定期購読と 4 表示用途、未購入 / 購入済みの遷移を照合する | 未購入者へ購入者本文が露出する、無料記事への内包だけで済ませる、または課金・購読処理をテーマへ持ち込めば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
| WT-AT-INTERVIEW-01 | WT-FR-INTERVIEW-01 | 人物・所属・発言・掲載確認を持つ独立した管理単位と詳細面を、埋め込みカードの参照先と照合する | LP のカードだけで独立管理や詳細面を実証した扱いにする、または人物・発言の対応が崩れれば FAIL | 未実行（条件定義）。試作の部分証跡は current-alignment.md 参照 |
