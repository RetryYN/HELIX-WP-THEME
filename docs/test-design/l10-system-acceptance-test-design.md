...
| WT-AT-SEO-04 | WT-FR-SEO-04 | 各 SEO 要求に Google 検索セントラルの公式ドキュメント URL と 2026-09-03 の参照日が紐づき、構造化データの必須プロパティ・代表画像の OGP / image 一致・Discover 幅 1200px 以上・検索の基本・ページエクスペリエンス・rel 属性・クローキングにならない A/B 配信の準拠項目が列挙される | 出典 URL / 参照日がない、公式仕様と要求の対応が追えない、Discover 画像要件またはクローキング回避が検査されない、または対象外になった型を要求する記述が残れば FAIL | source registry |
