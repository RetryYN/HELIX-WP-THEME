...
- 拡大するもの: 面（置き場所）、共有パーツと骨格の変種、記事内語彙、見た目の引き出し、記事単位の切替、設定→JSON 写像、エージェント制御面、値の 3 域制御、構造化データ、販売構成・SEO 準拠・クローラー計測、A/B 配信・画像最適化・性能保証・差分 API・運用管理・SNS / CV / バナー・監査、SP 既定・SP 固有面 / 語彙、計測タグ slot・データ層・同意連動、第三者プラグインの領域別既定・検出 manifest（PO 2026-09-03）。
