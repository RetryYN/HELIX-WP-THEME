...
実証できたパターンは GRAPHIX-NEO へ一方向で取り込む（統合層 `docs/plans/2026-08-28-wp-theme-and-graphix-neo-plan.md`）。

- 維持するもの: JSON 宣言による目録化可能性、自己記述（health / capability）、契約スキーマ、AI 判定ロジックの不持込（判定は HELIX 側）。
- 拡大するもの: 面（置き場所）、共有パーツと骨格の変種、記事内語彙、見た目の引き出し、記事単位の切替、設定→JSON 写像、エージェント制御面、値の 3 域制御、構造化データ。
