...
現行方針: 独立HTMLの選択カタログと専用テストはPO指示で破棄した。パーツ・完成面は検証用WordPressのPoCで表示確認する。以下の独立カタログUIに関する過去の進捗は履歴であり、現在の完了条件として扱わない。
...
| 段階 | 作業 | 状態 |
| --- | --- | --- |
| 1 | 全要求の再現台帳と、WordPress上の各PoC・検証結果との対応を維持 | 別HTMLの選択カタログはPO指示で破棄。パーツの表示確認はWordPress上で行い、要求・証拠対応データだけを保持。全ACの証拠対応は継続 |
