...
| MNF-011 | 移行元テーマ非依存 | 互換性 | SWELL/Cocoon/AFFINGER/JIN/Lightning のテーマファイルに直接依存しない。WP REST API と HTML 解析のみで変換する | P0 | REQ-NF-010 |
...
**テーマ別特殊処理の設計**: テーマ固有の変換ロジック（SWELL の `lp` CPT 処理・JIN の SEO メタ読み取り等）は `ThemeAdapter` インターフェースを実装した各 Adapter クラスに閉じ込め、テーマ固有コードがコアロジックに混入しない設計にする。各 Adapter は WP REST API と HTML 解析のみを使い、テーマ PHP コードには依存しない。
...
|---|---|---|
| SWELL | `lp` CPT 抽出、ブログパーツ変換、REST 経由 settings 読み取り | SWELL PHP クラス直接参照 |
| Cocoon | 通常投稿/ウィジェット抽出、shortcode テキスト変換 | Cocoon PHP 関数 |
| AFFINGER | 設定 Options 読み取り（WP REST 経由）、shortcode カタログマッチ | AFFINGER PHP |
| JIN / JIN:R | テーマ内 SEO meta post_meta 読み取り、classic template HTML 解析 | JIN PHP クラス |
