...
| WT-TRL1-02 | 契約正本は JSON: theme.json、config/*.json（fail-fast schema 検証）、schema/*.json、openapi.yaml。面・部品・変種の追加は JSON 宣言を伴い、共通定義は fluid、device 別差分は `@mobile` / `@tablet` の上書きとして宣言する |
...
| WT-TRL1-05 | 実機ゲートはローカル docker WP 7.1。共通定義と device 別差分を SP / PC の両幅で検査する。実運用サイトは read-only。接続情報は環境変数 |
