...
| WT-TRL1-07 | 構成はテーマ + プラグイン。プラグインはテーマを替えても残すべきデータと契約（設定 JSON・投稿メタ・section ID・ゾーン定義・再利用パーツ・REST / MCP / CLI・tracking・実証記録）を持ち、表示はテーマ、判定は HELIX 側。移行機能は別プラグインに分離し、移行プラグインは薄くてよい（PO 2026-09-02）。第三者プラグインとの出力重複は設定で制御し、全プラグイン互換は非対象 |
