...
AGENT NEOは、ThemeB/テーマA/テーマC/テーマD/テーマEと同じ「汎用WPテーマ」カテゴリで価格・デザイン・SEOだけを競わない。狙う市場カテゴリは **AI運用型WPテーマ基盤** とし、個人版はアフィリエイト収益化とAI記事投入/計測、法人版はLP改善/A-B/CTA計測/LLMO/運用品質を中核価値にする。
...
この 4 原理は ThemeB/テーマA/テーマC 等が陥っている「機能追加 → 設定肥大化 → JS/CSS 肥大化」の循環を意図的に拒否し、「機能を絞る → 高速 → CV 上昇」の逆循環を選択する設計判断。第一原理 4 は「AI 全振り」の罠を回避し、非 AI ユーザー（IT に詳しくない個人ブロガー / Automation SEO サブスクなしユーザー / 手動運用に戻したい法人）にも単独で価値を提供することで、製品の市場リーチを最大化する。
...
| REQ-F-022 | H2 単位 LLM 編集 | 各 H2 セクションを auto-section_id で addressable にし（セクション = H2 + 次 H2 までの範囲）、単一セクションに対する `POST /agent-neo/v1/posts/<id>/sections/<section_id>/edit` の実行経路を提供する。AGENT NEO 側はセクション単位の dryRun + diff preview + apply + rollback を提供し、**判断ロジック（rewrite / expand / summarize / translate / restructure の選択 + プロンプト生成）は Automation SEO 側、AGENT NEO 側は POST API ハンドラ + 結果の dryRun / apply / rollback 機構のみ提供する（REQ-NF-025 enforced_by）** | P0 | ACC-022 |
| REQ-F-023 | 要素差し替え機構（Element Swap）| 内部リンク（link_id, テキスト保持・URL 差替え）/ CTA（cta_id swap）/ バナー（banner_id swap）/ 画像（media_id swap）/ LP（blueprint_id 全体差替え）/ 法人版再利用パーツ（reusable_part_id swap）を、**安定 ID 経由で個別 swap** 可能にする実行 API を提供する。**swap 判断ロジック（どの cta_id / banner_id / blueprint_id に swap するか）は Automation SEO 側、AGENT NEO 側は安定 ID による swap 実行 API のみ提供する（REQ-NF-025 enforced_by）** | P0 | ACC-023 |
| REQ-F-024 | AI 自律 A/B テスト機構 | variant の自動配信（seo-tool-connector の variant_id 経由）/ 計測（impression / click / CV）/ 勝者反映 / loser archive / 緊急停止 CLI `wp agent-neo ab-test stop --post_id=X` を含む**実行基盤**を提供する。法人版は variant 採用に**承認 gate**（編集者/承認者ロール）、個人版は全自動。**variant 生成・統計判定・勝者選定は Automation SEO 側、AGENT NEO 側は variant 配信機構 + 計測連携のみ提供する（REQ-NF-025 enforced_by）** | P0 | ACC-024 |
...
| REQ-F-032 | AI 主導 CV 最適化 | AI suggested CTA / Personalized hero / Smart internal linking / Dynamic pricing display の**配信実行面**を提供する。**判定・計算ロジックはすべて Automation SEO 側、AGENT NEO 側は配信実行のみ提供する（REQ-NF-025 enforced_by）** | P0 | ACC-032 |
| REQ-F-033 | CV 設計監査機能 | L5 Visual Refinement 時に、cta.overload / proof.too_late / hero.vague / comparison.missing_basis / affiliate.disclosure_weak / form.too_long 等の**監査結果を受け取り表示する機能**を提供し、修正 UI とレポート出力を行う。**監査判定ロジック（cta.overload / proof.too_late / hero.vague 等の判定ルール）は Automation SEO 側、AGENT NEO 側は監査結果の表示と修正 UI のみ提供する（REQ-NF-025 enforced_by）** | P0 | ACC-033 |
| REQ-F-034 | 認知バイアスパターンライブラリ | scarcity（残数表示）/ authority（受賞・専門家推薦）/ social proof（利用者数・お客様の声）/ commitment（無料お試し→契約）/ reciprocity（無料資料 DL）を再利用可能な**静的ブロックパターン**として提供する。各パターンは a11y 配慮（適切なラベル・スクリーンリーダー対応）+ 過度な恐怖訴求は禁止。**パターン適用判断は Automation SEO 側、AGENT NEO 側はパターン定義（静的）と実体ブロックのみ提供する（REQ-NF-025 enforced_by）** | P1 | ACC-034 |
...
| REQ-F-036 | AI HTML/CSS 検証パイプライン | フリーフォームブロックの保存・適用前に必ず通過する検証層: (1) `wp_kses` 拡張 allowlist で sanitize（**ページ本体（light DOM / Shadow DOM static）コンテキストでの inline `<script>` を禁止** / `on*=` インラインイベントハンドラ禁止 / `javascript:`/`data:` URL 制限。なお sandboxed iframe 内での JS 実行はこの禁止の対象外とし ADR-026 の mode=interactive 経路で別途管理する）/ (2) CSS 自動スコープ化（全 selector に `.agent-neo-ai-block-{block_id}` プレフィックス付与）/ (3) axe-core で WCAG 2.2 AA 検証 / (4) HTML/CSS バイト数を page_type 予算にカウント / (5) 安定 anchor 属性（data-agent-section-id / cta_id / variant_id）の保護 / (6) プロンプトインジェクション検出 / (7) **JS 性能規律（コンテキスト別）**: JS は「テーマ本体を重くしない」を原則とし無条件禁止ではない。コンテキストにより以下の通り規律を分離する。**【ページ本体コンテキスト JS】**（light DOM ブロック・親側 postMessage リスナー等）: **REQ-NF-001e**（defer/async 必須・メインスレッドブロック禁止・minify + tree-shake 済み・1 ブロック ≤ 5KB 目安・超過時 L3 分割設計レビュー）を適用する。**【mode=interactive の別オリジン sandbox iframe payload JS（sandbox-origin 配信）】**（ゲーム・診断等のインタラクティブコンテンツ）: 別オリジン sandbox iframe（`<iframe src="https://<sandbox-origin>/embed/{id}" sandbox="allow-scripts">`）で配信するため、srcdoc 属性コンテキストエスケープの問題は生じない（**srcdoc 方式は廃止 / ADR-026 rev2**）。外部スクリプト向けの `defer`/`async` 属性の議論はここに適用されない。代わりに「**別オリジン iframe 隔離（ADR-026 mode=interactive）+ `loading="lazy"` + page_type 性能予算カウント + INP / Long Task 実測（親ページ性能劣化が許容閾値内）+ 実行時間制限**（CPU 負荷の高い処理は `requestIdleCallback` / Web Worker 化 / タイムアウト設定）」で非ブロッキング契約を担保する（ADR-026 §JS 許可根拠と一致）。page_type 性能予算（REQ-NF-001f）を超えた JS ロードを apply ブロックすること（**REQ-NF-001e との cross-ref 参照**） | P0 | ACC-036 |
...
| REQ-F-048 | CTA/バナー計装 + tracking export read 口 = 問い合わせ型計測公開 | コンテンツ内 `data-agent-neo-*` 計測属性の付与と、`GET /agent-neo/v1/tracking/export` の公開 read インターフェースを提供する。`section_id / cta_id / variant_id / event_type` を参照可能にし、Automation SEO が pull 型で取得する契約を前提にする | P0 | TP-ACC-001 / TP-ACC-003 / TP-ACC-004 / TP-ACC-005
| REQ-F-038 | HP/LP/固定ページ デザイン編集サンドボックス Tier 1（AGENT NEO 内蔵ライト）| **主対象: HP / LP / 固定ページ**（記事は対象外、軽量経路で別途扱う）。WP 内完結で対象ページに preview meta `_agent_neo_preview_content`、preview URL token（`/?page_id=ID&agent-neo-preview=<token>`）、agent-neo/v1 PATCH で apply（preview → production）、blueprint-level version 履歴 N 版保持。個人版・BYOK・standalone 用、Automation SEO 不要で動作。**【制約】mode=interactive（別オリジン sandbox iframe 配信）は sandbox-origin（Automation SEO 配信基盤）が必須であるため、standalone / Automation SEO 未接続環境では interactive モードは利用不可（static モードのみ提供）。** | P0 | ACC-038 |
...
| REQ-NF-001e | JS 採用時の性能担保（コンテキスト別） | 採用が認められた JS はコンテキストにより規律を分離する。**【ページ本体コンテキスト JS】**（light DOM ブロック・親側 postMessage リスナー等）: `defer` / `async` 必須、メインスレッドブロック禁止、minify + tree-shake 済み配信、外部スクリプトは `<link rel=preconnect>`、1 ブロック ≤ 5KB 目安、超過時 L3 で分割設計レビュー。**【mode=interactive の別オリジン iframe payload JS（sandbox-origin 経由で配信）】**: 別オリジン sandbox iframe（`<iframe src="https://<sandbox-origin>/embed/{id}" sandbox="allow-scripts">`）で配信し、page 本体の defer/async スクリプトとは文脈が分離される。**別オリジン iframe 隔離 + INP 実測 + page_type budget で担保**。具体的には「別オリジン iframe 隔離（ADR-026 mode=interactive）+ `loading="lazy"` + page_type 性能予算カウント（CI 計測から除外しない）+ INP / Long Task 実測（親ページ性能劣化が許容閾値内）+ 実行時間制限（`requestIdleCallback` / Web Worker 化 / タイムアウト設定）」で非ブロッキング契約を担保する。**mode=interactive の iframe payload JS は page_type 性能予算にカウントし CI 初期 JS 計測から除外しない（round16 P2 対応）**。別オリジン iframe は親と同一メインスレッドを共有し得るため「別メインスレッド保証」には依拠しない（ADR-026 §JS 許可根拠と整合 / REQ-F-036 是正後との cross-ref）。※ srcdoc 方式は廃止（ADR-026 rev2）。 | P0 |
...
| REQ-NF-028 | 計測データ取り扱い（PII非保持 / consent-gated / PULL前提）| 計測イベントは `agent_neo_consent_v2` の `analytics_storage='granted'` 時のみ収集・公開され、保存データは PII を含まない。Automation SEO の pull が既存 `edit_posts` 接続権限で `tracking/export` を取得し、テナント確定は接続側で行う | P0 | TP-ACC-002 / TP-ACC-004 / TP-ACC-005
...
| ACC-036 | REQ-F-036 | `<script>alert(1)</script><img onerror="..." src="x">` を含む HTML をフリーフォームブロック（**ページ本体コンテキスト** = light DOM / Shadow DOM static）の保存試行 | sanitize でページ本体コンテキストへの inline `<script>` 除去・`onerror` 属性除去、保存ログに違反内容記録。なお sandboxed embed（mode=interactive / ADR-026）の `<script>` はこの除去対象外（iframe sandbox 内実行が正規動作 / TC-072 参照） | XSS 防御テスト（ページ本体コンテキスト限定） |
| ACC-036e | REQ-F-036 | ゲーム・診断コンテンツを mode=interactive として sandboxed iframe 埋め込みで配信（ADR-026 / REQ-NF-001e 準拠） | (a) iframe に `sandbox="allow-scripts"` が付与され `allow-same-origin` が含まれないこと（TC-067 参照）。(b) JS バイト数が REQ-NF-001f の page_type 予算内であること（budget 超過時は apply ブロック）。(c) interactive 埋め込みを含まないページには親側 postMessage リスナーがロードされないこと。(d) mode=interactive の iframe payload JS は page_type 性能予算にカウントされ、CI 初期 JS 計測から除外されないこと（REQ-NF-001e / ADR-026 / L3-A4 準拠） | sandboxed embed 性能担保テスト（JS 許可 + 隔離確認） |
...
| AI HTML/CSS 検証パイプライン | フリーフォームブロックの保存・適用前に必ず通過する 7 層検証（sanitize / CSS scope / a11y / budget / anchor 保護 / prompt injection / JS 性能規律）。(7) は「JS 絶対禁止」ではなく「テーマ本体を重くしない性能原則（REQ-NF-001e）+ ページ本体コンテキストでの inline script 禁止」であり、sandboxed iframe 内の JS（ゲーム・診断等）は許可 |
...
| Q-012 | F-018 の「プロフィール表示」 / 「SNS フィードウィジェット」に対応する受入条件（ACC）の切り出し方針。L2 ADR-006 で「F-018 内の追加 ACC として受入条件を補完する」か「別 REQ-F として切り出す」かを決定（SNS フィードウィジェットは Phase2 送り、主要4機能は Phase1） | TL/PO | L2 ADR-006 着手前 | PM-RESOLVED（2026-06-20） |
| Q-013 | 公開指標ポリシー（L0 §6.4）の未確定項目（同意取得要否 / 集計閾値 / 保存期間 / 公開遅延 / 表示責任者）の最終値確定。G2 通過前に PO/法務確認のうえ凍結する（選択肢B：同意バナーあり前提、閾値・保存期間・責任者は L4 で Automation SEO retention と整合） | PO/法務 | G2 通過前 | PM-RESOLVED（2026-06-20） |
