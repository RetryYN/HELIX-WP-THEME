...
| 旧 L0-planning.md（削除済み） | 4 原理・ページ型別予算は WT-NFRL1-01 等へ継承済み。価格・単体販売・成功指標は失効（ADR-024） |
| 旧 L1-requirements.md・nsrm-*（削除済み） | REQ-F-045 / 046 / 025、ADR-024 の継承と REQ-F-016 / 037 の衝突記録だけを本正本へ写した。旧 gate / freeze 表示は拘束ではない |
