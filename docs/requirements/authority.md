...
- authority owner: PO
- updated: 2026-09-08
- 最新整理: [`current-alignment.md`](current-alignment.md)（WT-EVT-0307、132候補 / 289 AC / 132 test ID。旧123件から6件改定・9件追加。実装完了件数ではない）
...
| L2 | `docs/requirements/discovery/events.jsonl` と `candidate-projection.json`、最新差分 `current-alignment.md` | フロント先行の画面プロト往復（WT-EVT-0239）。最新整理 WT-EVT-0307、過去の合意 WT-AGREE-01 は head 0231 のみ | 正式判断2件（G3 / 開発スタイル）は保留。後続のPO反応、改定範囲、優先度の合意を揃える |
