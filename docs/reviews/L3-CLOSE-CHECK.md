...
| 6 | WP7.0 固有機能（Block Bindings/Interactivity/Section Styles）の採否が L3 未凍結・carry 未登録 | B | **確定** | P1 | **RESOLVED（2026-06-21 / ADR-027 D1-D3 で 3 機能とも採用を凍結。Accepted）** |
| 7 | test-plan §8 の TC-040 が誤パス（`/agent-neo/v1/posts/{id}/snapshot` → 正 `/agent-neo/v1/public/pages/{id}/snapshot`）/ TC-038（ai-crawlers/access-matrix）・TC-041（tracking/llmo-summary）が api-catalog/openapi 未定義エンドポイント参照 | C | **確定**（TC-040 パス差は api-catalog:160 で確認 / 038・041 は該当 EP を 1-165 行で未発見） | P1 | **RESOLVED（2026-06-21 / TC-038→GET /crawler-policy、TC-040→GET /public/pages/{id}/snapshot+/public/llmo/answers、TC-041→GET /tracking/llmo-summary。GET 2本を api-catalog/openapi に新設し総数 55→57）** |
| 8 | addenda A1/A2/A3 の TC 候補・新規エンドポイントが L3-test-plan / api-catalog SSOT 未登録 | C | 確定（addenda は元来 gap 列挙文書） | P1 | **A3 新規 EP 分は CARRY-TEST-ALIGN-001 に合流し、CARRY-TEST-ALIGN-001 で既に RESOLVED（2026-06-21）。残 addenda TC は既存 CARRY-A* で L4 追跡** |
...
**条件付き注記**: 本 close は「L3 設計の tentative close」。**これらの L4-entry 必達 carry は 2026-06-21 に解消済み（ADR-027 + 契約整合）**。L3 は tentative close を満たし、G3 passed を維持。

## 5. L4 entry 前 必達 carry（本チェックで新規/明確化）

| carry-id | 内容 | 由来 | 状態 |
|---|---|---|---|
| CARRY-TEST-ALIGN-001 | TC-040 パス訂正 + TC-038/041 の対応 EP を api-catalog/openapi に追加 or TC 差し替え | #7/#8 | **RESOLVED(2026-06-21)**: TC-038→GET /crawler-policy、TC-040→GET /public/pages/{id}/snapshot+/public/llmo/answers、TC-041→GET /tracking/llmo-summary。GET 2本を api-catalog/openapi に新設（総数 55→57） |
| CARRY-WP7-013 | Block Bindings API 採否を ADR-027 D1 で凍結（Accepted） | #6 | **RESOLVED(2026-06-21)**: ADR-027 D1（採用凍結） |
| CARRY-WP7-014 | Interactivity API 採否を ADR-027 D2 で凍結（Accepted） | #6 | **RESOLVED(2026-06-21)**: ADR-027 D2（採用凍結、性能予算内限定） |
| CARRY-WP7-015 | Section Styles 採否を ADR-027 D3 で凍結（Accepted） | #6 | **RESOLVED(2026-06-21)**: ADR-027 D3（採用凍結） |

---

*作成: 2026-06-21 / 上下チェック実施: Codex TL + Sonnet×2 レビュー + PM 裏取り / 修正: Edit ベース（分類器停止のため委譲不可）/ commit: Bash 復帰後*
*2026-06-21 追記: L4-entry 必達 carry 4 件を解消（ADR-027 / 契約整合）。委譲: Codex docs drafter。*
