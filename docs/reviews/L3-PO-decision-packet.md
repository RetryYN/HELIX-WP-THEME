...
| 選択肢 | 概要 | メリット | デメリット |
|---|---|---|---|
| A: 移行プラグインを wp.org 申請（lead magnet） | REQ-F-008 移行プラグインを wp.org で無料公開。GPL 公開必須 | ThemeB/テーマA ユーザーへのリーチ。AGENT NEO / Automation SEO の認知拡大 | プラグイン審査コスト・維持コスト。GPL 公開による模倣リスク（移行プラグインのコードは公開） |
...
**確定日**: 2026-06-25 / **根拠**: PO 裁定  
wp.org への申請は行わず、配布チャネルを公式サイト / Automation SEO 側に一本化することで確定。移行プラグインの wp.org 申請は不採用。配布チャネルの詳細（公式サイト / LP）は LP 制作フェーズでまとめて確定する。  
**disposition**: RESOLVED-BY-DECISION
...
**確定日**: 2026-06-25 / **根拠**: PO 裁定  
S1 価格レンジは公式サイト / 販売 LP を制作するフェーズでまとめて確定する。現フェーズ（L4）の PO 裁定対象外とし、論点を LP 制作フェーズに送る。  
**disposition**: DEFERRED-TO-LP-PHASE
...
**確定日**: 2026-06-25 / **根拠**: PO 裁定  
配布チャネルは公式サイト / Automation SEO 側からの配布に一本化することで確定。wp.org を介した配布は行わない。配布チャネルの詳細（無料 / lead magnet の具体的形式）は公式サイト / LP 制作フェーズでまとめて確定する。  
**disposition**: RESOLVED-BY-DECISION
...
Bridge Plugin（ADR-019）は既存テーマ（ThemeB/テーマA）のショートコード（`[fukidashi]`・`[jin_icon]`・`[blogcard]` 等）を AGENT NEO ブロックへ変換する役割を担う。ADR-023 の CARRY-ADR023-004（P1: blocking=true）として記録されており、変換対象ショートコードの一覧・変換方式・どのリリースバージョンで提供するかが未確定。これが確定しないと Bridge Plugin の L4 実装スプリントが着手不可。

既存 テーマA ユーザーが AGENT NEO に移行した場合、過去コンテンツ（ショートコードを含む投稿が多数ある場合がある）の互換性を保証するレベルが製品の移行容易性に直結する。「全ショートコードを変換する」と約束することは保守コミットメントになる。

**選択肢**

| 選択肢 | 変換スコープ | 提供リリース | メリット | デメリット |
|---|---|---|---|---|
| A: 主要ショートコード（fukidashi / jin_icon / blogcard）のみ最優先で提供、Phase 1 含める | 3 種のみ | Phase 1 リリース時 | AGENT NEO の主要競合（テーマA）からの移行を Phase 1 でサポート。移行訴求が強い | 3 種以外のショートコードは「非対応」と告知が必要 |
...
| C: Phase 2 送り（Bridge Plugin の核心機能は Phase 2 以降） | 0（Phase 1 では変換なし）| Phase 2 | Phase 1 の実装コスト削減。Core テーマ・API 基盤・収益化ブロックに集中 | 移行訴求が弱まる。既存 テーマA/ThemeB ユーザーへの訴求が遅れる |

**PM 推奨（参考）**

PO 確定必須。PM 視点では **選択肢 A（主要3種を Phase 1 で提供）**を推奨する。理由: (1) fukidashi / jin_icon / blogcard は テーマA / ThemeB ユーザーが最も使うショートコードで、移行の象徴的な価値がある、(2) 「3 種のみ対応」と明確に告知することでコミットメントを限定できる、(3) CARRY-ADR023-004 が P1 blocking であることから、早期の確定が L4 工程表に与える影響を最小化できる。
...
**選択肢 A（GA 環境で本格組み込み検証）**を推奨する。理由: (1) 推定 0.5〜1 人日の投資に対して「先行採用による差別化」の価値は大きい、(2) ThemeB/テーマA が Abilities API 実装 0 件である現状での先行採用は明確な訴求ポイントになる、(3) WP 7.0 GA 済みのため RC 乖離リスクはなく、コスト的リスクはほぼゼロ。最終的には L4 の工数計画との兼ね合いで PO が承認する。
...
> **2026-06-25 PO 裁定**: Q-006 / Q-004 がクローズされ、残 1 件（PO-WP7-02）のみ。

| 論点 ID | 論点名 | 猶予理由 | ADR-024 影響 | disposition |
|---|---|---|---|---|
| ~~Q-006~~ | ~~wp.org 配布 → 移行プラグイン wp.org 申請可否のみ（縮小）~~ | — | — | **PO 確定済（2026-06-25）/ RESOLVED-BY-DECISION**: wp.org 非採用・公式サイト配布一本化 |
| ~~Q-004~~ | ~~移行プラグイン 無料配布~~ | — | — | **PO 確定済（2026-06-25）/ RESOLVED-BY-DECISION**: 公式サイト配布確定・詳細は LP 制作フェーズ |
| PO-WP7-02 | 423 Locked 拒否 UX 許容可否 | L4 REST 実装スプリント着手前に確定すれば足りる | 変更なし | OPEN |

**L7 前でOK（2 件）:**

| 論点 ID | 論点名 | disposition |
|---|---|---|
| Q-003 | S1 価格レンジ（配布 LP 公開前に確定）※テーマ配布モデルと独立のサービス価格のため変更なし | **PO 確定済（2026-06-25）/ DEFERRED-TO-LP-PHASE**: 公式サイト / LP 制作フェーズで確定 |
| CARRY-ADR022-002 | Yoast Premium 非サポート告知方針（L6 受入テスト前にドキュメント整備） | OPEN |
