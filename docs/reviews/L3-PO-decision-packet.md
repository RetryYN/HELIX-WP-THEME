...
本パケットは以下の 5 グループ計 12 論点を収録する。各論点に「緊急度」を付与し、末尾に **L4 着手前に必達の裁定リスト** を一覧化する。
...
### GAP-RT-053: REQ-F-043 Open Editor Bridge Plugin の OAuth 申請フロー未定義

**論点 ID**: GAP-RT-053 / REQ-F-043 / PO-ESCALATION / security
**背景**

AGENT NEO は REQ-F-043 として「Open Editor Bridge Plugin」の有料アドオンを設計している。このアドオンは、許可リストに登録された外部AIエディタ（Claude / Codex / Cursor / Cline / Continue など + 自前OAuth申請で追加登録したツール）に対し、AGENT NEO への write 操作を許可する仕組みである。

現時点では、OAuth の発行主体（AGENT NEO 運営が認可サーバーを持つのか、外部 OAuth プロバイダに委ねるのか）、scope 制限の粒度（どの操作を許可し何を禁止するか）、外部AIエディタの審査フロー（誰がどのように「許可リスト申請」を審査するか）、および revoke 手順（発行済みトークンの失効・緊急停止フロー）が未定義である。

設計を誤ると、申請フローの不備から悪意ある第三者AIが許可リストに紛れ込み、AGENT NEO 経由で WordPress コンテンツに意図しない write 操作を行う攻撃面が生まれる。本件は **認証/認可の仕様確定を含む**ため PM 単独では仕様確定が不可能であり、PO 確認が必須。

**未確定項目**

| 項目 | 未確定内容 |
|---|---|
| OAuth 発行主体 | AGENT NEO 自身が認可サーバーを保有するか、外部 OAuth プロバイダ（例: Auth0 / Keycloak）に委ねるか |
| scope 制限粒度 | 外部AIエディタに付与する write scope の範囲（記事本文のみ / メタ情報含む / メディア含む 等） |
| 申請審査フロー | 許可リスト申請を誰がどの基準で審査し承認するか（運営手動審査 / 自動検証 / 第三者証明書等） |
| revoke 手順 | 発行済みアクセストークンの失効手順・緊急停止フロー |
| アドオン課金との紐づけ | OAuth トークン発行はアドオン購入済みユーザーのみに制限する実装設計 |

**選択肢**

| 選択肢 | 概要 | メリット | デメリット |
|---|---|---|---|
| A: 自社審査のみ（クローズドβ的運用） | 申請フォームを運営が受け付け、手動審査で許可リスト追加。OAuth は自社シンプル実装（クライアントID/Secret 発行のみ） | 初期実装コストが低い。審査品質を運営が完全コントロール | 審査スケールしない。運営負荷が高い。OAuth の安全性が実装品質に依存 |
| B: OAuth 動的登録 + scope 制限 + revoke 必須（標準準拠） | RFC 7591 OAuth 動的クライアント登録 + scope 審査フロー + RFC 7009 revoke endpoint を実装。外部AIエディタはクライアント登録 → 審査 → 発行のフロー | 標準準拠で外部AIベンダーとの統合が容易。scope 制限が明確 | 実装工数が大きい（L4 で単独 Sprint 相当）。OAuth サーバー保守コスト |
| C: 当面アドオン凍結（REQ-F-043 を Phase 2 送り） | Open Editor Bridge Plugin の実装を Phase 2 以降に延期。Phase 1 では外部AIエディタへの write 許可機能を提供しない | Phase 1 の攻撃面を最小化。本体テーマ・SEO 基盤の安全な提供に集中できる | REQ-F-043 の訴求が遅れる。外部AIエディタ統合を期待するユーザーへの訴求が Phase 1 では不可 |

**PM 推奨（参考）**

**認証/認可の仕様確定は人間(PO)確認必須**。PM は技術的推奨を提示するのみ。

PM 視点では、Phase 1 における本体テーマの安全性確保を最優先とするため、**選択肢 C（当面アドオン凍結）または選択肢 A（自社審査クローズドβ）** のいずれかを推奨する。理由: (1) OAuth フローの設計不備は write 受け口の意図せぬ公開という重大な攻撃面になる、(2) Phase 1 の L4 工数はコアテーマ実装に集中すべき、(3) 選択肢 B の標準準拠実装は Phase 2 以降で行う方が設計の熟成度が高まる。ただし **OAuth の発行主体・scope・revoke の仕様はセキュリティ要件のため、最終判断は必ず PO が行う**こと。

**この決定が unblock する GAP・ADR・carry**

- GAP-RT-053（REQ-F-043 OAuth フロー未定義 / PO-ESCALATION）
- REQ-F-043 の L4 実装スプリントのスコープ確定（選択肢 C なら L4 skip 確定 / 選択肢 A または B なら L4 carry として追加）
- `docs/security/threat-model.md` §5.1 TB-19（外部AI write 攻撃面）への対策方針反映

**緊急度**: REQ-F-043 L4 実装着手前（**本体テーマ L4 はブロックしない**。Open Editor Bridge Plugin Sprint の着手前に裁定すれば足りる。ただし選択肢 A/B を選ぶ場合は OAuth 設計が先行タスクになるため、本体テーマ L4 の後半 Sprint 着手前に確定が望ましい）

---

## L4 着手前に必達の裁定 — 優先リスト

以下 9 件は L4 実装開始前に PO 裁定が必要な論点。残り 3 件は L4 途中または L7 前での裁定でよい。
...
| 9 | GAP-RT-053 | REQ-F-043 Open Editor Bridge Plugin OAuth 申請フロー（認証/認可のため PO 確定必須） | REQ-F-043 実装スプリント着手前（本体テーマ L4 はブロックしない） |
