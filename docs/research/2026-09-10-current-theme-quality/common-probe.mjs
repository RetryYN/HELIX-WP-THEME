...
for(const name of ['rules','mincho']){
 ...
 variants[name]=execFileSync('docker',['exec',contentLab.wpContainer,'php','-r',php],{encoding:'utf8',maxBuffer:10e6});
}
try{
 for(const [face,axes]of cases)for(const width of [390,1440])for(const [variation,css]of Object.entries(variants)){
  ...
  const response=await page.goto(contentLab.baseUrl+'/quality-audit-'+face+'/?wt='+axes,{waitUntil:'networkidle'});
 ...
 for(const [face,axes]of cases){const page=await browser.newPage({viewport:{width:390,height:900},javaScriptEnabled:false});const response=await page.goto(contentLab.baseUrl+'/quality-audit-'+face+'/?wt='+axes,{waitUntil:'load'});rows.push({face,noJs:true,http:response.status(),...await page.evaluate(()=>({overflow:document.documentElement.scrollWidth>innerWidth,text:document.querySelector('main')?.textContent.trim().length??0}))});await page.close();}
