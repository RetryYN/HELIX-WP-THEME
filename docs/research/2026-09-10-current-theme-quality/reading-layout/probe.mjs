...
for(const [face,type,template,content] of [['article','post','','<!-- wp:pattern {"slug":"helix-wt/article-kit"} /-->'],['lp','page','page-lp',''],['event','page','page-event','']]){
 ...
 const response=await page.goto(`${contentLab.baseUrl}/${slug}/?wt=toc:box,event_info:inline-text`,{waitUntil:'networkidle'});
 ...
 if(mode==='after'&&face==='article'){await page.goto(`${contentLab.baseUrl}/${slug}/?wt=toc:none`,{waitUntil:'networkidle'});const count=await page.locator('main .wt-toc').count();tocNone.push({width,noJs,count});assert.equal(count,0);}
 await page.close();
 }
}
}finally{
 ...
 if(mode==='after'){
  ...
  sourceDigests['scripts/lib/content-lab-env.mjs']=createHash('sha256').update(readFileSync(fileURLToPath(new URL('../../../../scripts/lib/content-lab-env.mjs',import.meta.url)))).digest('hex');
  sourceDigests['docs/research/2026-09-10-current-theme-quality/reading-layout/probe.mjs']=createHash('sha256').update(readFileSync(fileURLToPath(import.meta.url))).digest('hex');
