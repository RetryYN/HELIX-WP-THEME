...
- 見つかった問題と事実:
  - WP 7.1 では `core/group`（flex/grid）の `blockGap` は inline `gap:` として serialize **されない**。`gap:` を書いた側が invalid になる（レビューの前提が逆だった）。main の `themes/agent-neo-theme/parts/header.html` にも同型あり → THEME-CAT-06（#40）。
  ...
  - `themes/agent-neo-theme/parts/sidebar.html`（PR #39 の枝のみ・未 merge の PoC） はどのテンプレートからも参照されない（テンプレ変種方針 #26 待ち）。
  - パターン file list は transient に載る（`wp_theme_files_patterns-*` を削除して再読込）。

## 3. 一貫性ゲートと Figma 取り込み（#36 / THEME-GATE-01 の材料）
- G-T1 / T1b / T2 / T3 / S1 / S2 を `bin/check-design-consistency.sh`（PR #36 の枝のみ・未 merge の PoC） として実装（CI 配線は PoC の範囲）。
...
- 結論: 経路は通るが、無料枠と手作業の多さから当面は不採用（`docs/design/figma-intake.md`（PR #36 の枝のみ・未 merge の PoC））。
