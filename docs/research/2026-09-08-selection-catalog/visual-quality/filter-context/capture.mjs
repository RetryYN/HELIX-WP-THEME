...
const staging = await mkdtemp(join(dirname(output), '.filter-context-capture-'));
let browser;
try {
  ...
  await writeFile(join(staging, 'observations.json'), JSON.stringify({ baseline: baselineCommit, observations }, null, 2) + '\n');
