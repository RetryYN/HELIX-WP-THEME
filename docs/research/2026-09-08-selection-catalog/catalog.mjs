...
const faceLabels = { utility: '対話型ツール', search: 'サイト内検索', inheritance: '共通設定の継承', site: '会社・規約', zone: 'バナー・配置', article: '記事', learning: '学習・ヘルプ', paid: '有料記事', interview: 'インタビュー', blp: 'BLP', category: 'カテゴリ', footer: 'フッター', lp: 'LP', home: 'ホーム', event: 'イベント', page: '固定ページ', form: 'フォーム', '404': '404' };
...
async function start() {
  ...
  const collections = [['all', 'すべて'], ['common', '共通設定・部品'], ...['inheritance', 'home', 'article', 'paid', 'interview', 'blp', 'learning', 'site', 'search', 'zone', 'category', 'lp', 'event', 'page', 'form', 'utility', 'footer', '404'].map(k => [k, faceLabels[k]])];
  ...
  function render() {
    ...
    for (const entry of entries.slice(0, limit)) {
      ...
      if (entry.finished) tile.append(el('p', entry.selectionFacts['入口の導線'] || entry.selectionFacts['対象と判断'] || entry.purpose, 'finished-route'));
  ...
  function refreshComparisonFacts() {
    ...
    const body = el('tbody');
    const factOrder = ['対象と判断', '入口の導線', '情報量', '区間の順序', '申込方式', '会場案内', '補助情報', 'サイドバー開始', '共通部品の所属', '受付不能時'];
    const availableFacts = new Set(entries.flatMap(e => Object.keys(e.selectionFacts || {})));
    const factKeys = [...factOrder.filter(key => availableFacts.has(key)), ...[...availableFacts].filter(key => !factOrder.includes(key))];
    for (const [label, value] of [
      ['目的', e => e.purpose],
      ...(factKeys.map(key => [key, e => e.selectionFacts?.[key] ?? (e.finished ? 'この構成では該当なし' : 'この候補は部品の比較')])),
