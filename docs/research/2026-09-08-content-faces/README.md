...
試作テーマに有料記事・インタビュー・BLP・獲得LP・学習を追加。コンテンツの保存と管理は別の試作プラグインの5CPTに置く。画面の確認は本READMEの手順で起動する検証用WordPressを使う。
...
content-labを使う検証器は、`content-lab-env.mjs`または`content_lab_env.py`から接続先・状態領域を取得する。これによりWP-CLI、PHP、ブラウザーの各経路が同じnetwork・container・portを使う。検証器とlauncherの双方で明示ポートとDocker名を同じ規則で検査する。
