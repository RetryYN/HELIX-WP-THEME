...
共通パーツの404検査と36画像をこの設定で更新する。ナビ検証はPC/SPとJS有無、9ヘッダー型への保存内容の反映、contentSize / wideSizeの維持、空・未設定・非公開・不正参照を対象とする。ネイティブ表示の固定ナビ、独立CTA、フッター・サイドバーのリンク、参照先を選択する管理UIは残るため、WT-AC-PARTS-02A/Bは部分検証として扱う。
