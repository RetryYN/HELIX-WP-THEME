...
静的検査は `bin/check-design-consistency.sh` をそのまま実行し、出力に6ゲートの見出しと、行全体が `FAIL=0 WARN=1` の集計結果があることを確認する。契約値と各出力 marker の欠落・改変を個別の負例で拒否する。G-E1 は `docs/research/2026-08-29-ge1-local/editor-validate-2026-08-29.json` の71行を再集計し、invalidブロックが0であることを確認する。認証情報、接続先、外部送信は証跡へ書かない。
