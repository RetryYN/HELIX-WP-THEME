# リバースエンジニアリング B — SWELL

調査日 2026-08-26 / XServer SSH 読み取り専用 / 対象 `themes/swell`（PHP 45,622 行）
...
- オートローダは `SWELL_Theme` / `SWELL_` / `LOOS_` を名前で判定して `classes/` へ解決。
  `LOOS_` が残っているのは旧ブランド（`loos/` ブロック名前空間と同じ由来）。
...
```php
const DB_NAMES = [
  'customize' => 'loos_customizer',   // カスタマイザ由来
  'options'   => 'swell_options',     // 設定画面由来（既定 540 キー）
  'editors'   => 'swell_editors',     // エディタ設定
  'others'    => 'swell_others',
];
const DB_TABLES = [ 'balloon' => 'swell_balloon' ];   // ← 独自テーブル
```

- 設定は**単一配列 4 本**に集約。既定値は `classes/Data/Default_Settings.php`（667 行）が持ち、
  実値とマージして `SWELL_Theme::$setting` / `$customize` / `$options` / `$editors` に載る。
  → **設定の総体が 1 ファイルで列挙できる**（JIN:R には無い性質）。
...
- post meta / term meta は `lib/post_meta/`（5 ファイル）・`lib/term_meta.php` に分離。
  タームメタ `swell_term_meta_display_parts` はタームへ「表示するブログパーツ」を紐づける。
...
- **メディアクエリがバケットとして一級**。呼び出し側は `'sp'` を渡すだけで、
  最終出力時にまとめて `@media` に包まれる。JIN:R がブレークポイントを 1 関数内の
  ローカル変数に埋めているのと対照的。
...
`SWELL_Theme::is_separate_css()` が真のとき、**「使われているブロックだけ CSS を読む」**ために
...
└ Pre_Parse_Blocks::init()
   ...
   │   └ loos/blog-parts / core/block は参照先 post を取得して**中身も再帰**
   ├ ターム: term_meta swell_term_meta_display_parts の参照先を走査
...
- 結果は静的プロパティ `SWELL_Theme::$used_blocks` に溜まり、以後の CSS 出力判断に使われる。
...
### 5.1 登録
```php
register_swell_blocks()  // init
├ register_normal_blocks()   … 22 種を SWELL_Theme::register_block() で
└ register_dynamic_blocks()  … 10 種は lib/gutenberg/block/{name}.php を require
```

`SWELL_Theme::register_block()`（`Utility/Others.php`）:
- `index.asset.php`（wp-scripts 生成）から依存とバージョンを読み、`swell/{name}` ハンドルで JS 登録。
  依存に `swell_blocks`（共通バンドル）を必ず足す。
...
```php
\SWELL_Theme::register_block('post-link', ['render_callback' => __NAMESPACE__ . '\cb']);
function cb( $attrs ) { … }
```
...
| URL の自動ブログカード化 | the_content | `apply_filters('swell_remove_url_to_card', …)` で無効化可 |

**目次の実装**が示す設計:
- ショートコード `[swell_toc]` は本文に `<div class="swell-toc-placeholder"></div>` を置くだけ。
...
**REST 経路の除外が明示**されている: `SWELL::is_rest()` / `is_iframe()` を見て、
...
`/swell-block-settings` `/swell-term-list` `/swell-balloon`(+copy/recover/sort)
`/swell-ct-ad-data` `/swell-ct-btn-data` `/swell-ct-pv` `/swell-reset-ad-data`
`/swell-reset-cache` `/swell-reset-settings` `/swell-do-update-action` `/swell-lazyload-contents`

- 用途は**管理系が中心**（設定取得・キャッシュ削除・ふきだし CRUD・広告クリック計測データ）。
- `swell-lazyload-contents` はコンテンツ遅延読み込み用で、`is_rest('lazyload')` の分岐と対になっている。
- 自前名前空間（`swell/v1`）を切らずコアの `wp/v2` に足しているため、**コアや他プラグインとの
  ルート衝突リスクを構造的に抱える**。
...
- `lib/pluggable.php` / `lib/pluggable_parts.php` — **子テーマから関数を差し替える前提**の
  `function_exists()` ガード付き定義群（JIN:R にはこのガードが無い）。
