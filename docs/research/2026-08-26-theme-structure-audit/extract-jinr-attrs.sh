#!/usr/bin/env bash
# THEME-INV-14: 公開記事から jinr-blocks/* の属性を帰納するための抽出。
#
...
SSH_HOST="xs887843@xs887843.xsrv.jp"
SSH_PORT=10022
SSH_KEY="$HOME/.ssh/id_ed25519_xserver"
WP_PATH='$HOME/it-shukatu-college.com/public_html'
...
$PHP --path=$WP_PATH db query \"\$Q\" --skip-column-names 2>/dev/null \
  | grep -oE '<!-- wp:jinr-blocks/[a-z-]+ \{[^}]*\}' | sort | uniq -c | sort -rn
...
for b in simplebox button blogcard fukidashi comparechild compare postlist designtitle background \\
  ...
  n=\$($PHP --path=$WP_PATH db query \"\$Q\" --skip-column-names 2>/dev/null \
       | grep -oE \"<!-- wp:jinr-blocks/\$b \" | wc -l)
  [ \"\$n\" = '0' ] && continue
  echo \"--- jinr-blocks/\$b (\$n) ---\"
  $PHP --path=$WP_PATH db query \"\$Q\" --skip-column-names 2>/dev/null \
    | grep -oE \"<!-- wp:jinr-blocks/\$b \{[^}]*\}\" \
    | grep -oE '\"[a-zA-Z0-9_]+\":' | sort | uniq -c | sort -rn
done

echo
echo '===== 3. 共通属性の実使用 ====='
for a in jinrBlocksCSSAttribute topMarginPcAttribute bottomMarginPcAttribute \\
...
echo
echo '===== 5. [jinr_fukidashi] の位置関係（INV-10 の仮説検証） ====='
$PHP --path=$WP_PATH db query \"\$Q\" --skip-column-names 2>/dev/null \
  | grep -oE '<!-- wp:jinr-blocks/fukidashi.{0,300}' | head -3

echo
echo '===== 6. 未登録ブロック jinr-blocks/profile の実体 ====='
$PHP --path=$WP_PATH db query \"\$Q\" --skip-column-names 2>/dev/null \
  | grep -oE '<!-- wp:jinr-blocks/profile.{0,300}' | head -3

echo
echo '===== 7. コアブロックのスタイル（register_block_style 由来） ====='
for s in is-style-jinr-checkmark is-style-jinr-checkmark-square; do
