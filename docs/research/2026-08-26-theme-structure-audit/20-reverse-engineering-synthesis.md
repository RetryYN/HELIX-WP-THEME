# リバースエンジニアリング統合レポート — JIN:R / SWELL の機構と Graphix NEO への含意

- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用（サーバーへの書き込みなし）
- 対象: `themes/jinr`（PHP 33,931 行）/ `themes/swell`（PHP 45,622 行）/
...
| テーマ | 性格 |
|---|---|
| **JIN:R** | **設定駆動の閉じたテーマ。** グローバル関数の直列実行で、契約もスキーマも拡張点も持たない。設定 1,225 キーと手書きアクセサ 707 個が実質の正本 |
| **SWELL** | **契約を持つ開いたテーマ。** クラス + trait + オートローダ。block.json・filter 79 本・pluggable 関数群・REST 14 本を備え、外部からの介入を前提に設計されている |
...
**JIN:R の要点**: `get_template_part()` を「ファイルを読み込む」目的に流用している。
`load-customizer-value.php` は `customizer/ui/*.php` が定義するアクセサに依存するが、
それを保証しているのは **require の行順だけ**。静的解析で依存グラフを引けない。

**SWELL の要点**: **設定の確定が最初**（`self::data_init()`）。
以降のすべてのモジュールが確定済み設定を前提にできる。
JIN:R が「描画中にアクセサを呼び、必要なら DB へ書く」のと対照的。

### 1.3 設定 — 目録を作れるか

| | JIN:R | SWELL | AGENT NEO |
|---|---|---|---|
| 格納 | 個別 option **1,225** + theme_mod | 配列 **4 グループ** + 独自テーブル `swell_balloon` | `theme.json` + `config/*.json` 7 本 |
...
| URL 自動カード化 | 持たない | 内蔵（`swell_remove_url_to_card` で無効化可） |
| REST 経路 | 区別しない | **`is_rest()` で意図的に除外** |

SWELL の `content_filter.php` は冒頭コメントで設計意図を明示している:
...
### 1.6 使用要素の把握 — SWELL の 2 パス・ドライラン
...
2. 本文: parse_blocks( do_shortcode( $content ) ) を再帰走査
       └ loos/blog-parts(attrs.partsID) / core/block(attrs.ref) は参照先 post を取得して再帰
3. ターム: term_meta 'swell_term_meta_display_parts' の参照先を走査
...
中間 JSON パイプラインと同型。`reports/INV-15-swell-pipeline-transfer.md` で
**転用可（中核）**と判定した。
...
| 環境値 | `wp_localize_script` の `JINR_VAR` に一括注入 | ブロック属性 + 設定 API |
...
`jinr/post_by_url` と `jinr/external_url`。**両方 `permission_callback => '__return_true'`**。
...
**サーバ層で `/wp-json/jinr/external_url` を遮断すれば内部呼び出しは通る**。

it-shukatu にはセキュリティ系プラグインが入っていない（solobiz には `cloudsecure-wp-security`）。
→ `reports/INV-13-jinr-rest-endpoints.md`
...
→ `reports/INV-16-jinr-render-side-effects.md` / `reports/INV-09-settings-authority.md` §4.1

### R-03 正規化リダイレクトの全面停止

```php
add_filter('redirect_canonical', 'jinr_disable_redirect_canonical');
function jinr_disable_redirect_canonical($redirect_url) { $redirect_url = false; return $redirect_url; }
```

コメントの意図は「記事内ページネーションの URL 形式」だが、**引数を見ず常に `false`**。
末尾スラッシュ・`?p=ID`・ページ送り・大文字小文字の正規化がすべて停止する。
**it-shukatu は SEO 施策サイト**であり、重複 URL とクロールバジェットに直結する。
→ `reports/INV-17-jinr-global-side-effects.md`
...
**ページキャッシュ・CDN と衝突**する。
そして有料記事（`jinr-blocks/paidpost`）の**公開記事での実使用は 0** なので、
**全ページで純粋なオーバーヘッド**になっている。
→ `reports/INV-17` / `reports/INV-11-scope-boundary.md`

### R-05 決定論との衝突

JIN:R の render_callback は**未指定属性をカスタマイザ値へフォールバック**する。
...
`ad-zone.schema.json` は「JIN:R の 4 ゾーン（h2 前挿入 / 記事終 / 関連上 / カテゴリ別上書き）」と
書くが、実装では:
...
JIN:R の「詳細設定」（余白・表示デバイス）は、**数値やトークンではなくクラス名の文字列**。
...
加えて `jinrBlocksCSSAttribute` があると `<style jsx="true">` を**ブロック単位でインライン出力**する。
→ `reports/INV-14-jinr-attribute-induction.md`

### R-08 SWELL には転用できる前例がある
...
→ `reports/INV-15-swell-pipeline-transfer.md`
...
| 項目 | 初回 | 訂正後 | 原因 |
|---|---|---|---|
| JIN:R の独自 REST | 0 本 | **2 本** | 複数行 `register_rest_route(` を単一行前提の grep が拾えなかった |
| JIN:R の `register_block_style` | 0 | **2 件**（`core/list`） | 同上 |
| JIN:R の動的ブロック | 9 種以上 | **7 種** | 登録コード全文を数え直し |
| `jinr-blocks/paidpost` の実使用 | 本文中 16 回 | **公開記事で 0 回** | 16 は**テーマソース内の文字列出現数**で、本文の使用回数ではなかった |

**加えて未解決の不整合が 1 件**: `evidence/usage-raw.txt` に `jinr-blocks/profile` が 1 回出現するが、
...
加えて**参照解決に深さ上限と訪問済み集合**を入れる（SWELL の実装には無く、循環参照で無限ループ）。
...
| `extract-jinr-attrs.sh` | 読み取り専用の属性抽出。**1 回で INV-14 / 01（属性層）/ 02 / 10 / 11 の未了 5 件が閉じる** |
...
| 本番 DB の SELECT（`theme_mods_jinr` / `sidebars_widgets` / option 一覧） | INV-16 / 03 / 09 |
| `ad-zone.schema.json` の改訂 | INV-03 |
| 資産再利用可否台帳の GRAPHIX-NEO への反映（cross-repo） | INV-12 |
| ベンダー（CROOVER inc.）への報告要否 | INV-13 |
...
| イシュー草案 | `issues/` | 17 本 |
| 機構解析 | `10-reverse-jinr.md` / `11-reverse-swell.md` / `12-mechanism-comparison.md` | 3 本 |
| 構造調査 | `01`〜`04` | 4 本 |
| 消化状況 | `PROGRESS.md` | — |
