# 構造調査 B — SWELL（solobiz-lab.com）

- 調査日: 2026-08-26 / 手段: XServer SSH 読み取り専用 + WP-CLI 読み取りクエリ
- 対象: `~/solobiz-lab.com/public_html/wp-content/themes/swell`
- テーマ: SWELL / 子テーマ **無し**（親を直接運用）
- 書き込み: なし
...
### 2.1 Gutenberg ブロック（`loos/` 名前空間・block.json 実装 50 種）
...
`pcbr` `spbr` `post_link` `post_list` `pr_notation` `review_stars` `speech_balloon` `swell_toc`
...
| 項目 | 実測 |
|---|---|
| オプション | **単一配列 `swell_options` 方式**（`classes/Data/Default_Settings.php` に既定 540 キー）+ `swell_block_settings` `swell_version` 等の少数 |
| カスタマイザ | `lib/customizer/`(34) + `classes/Customizer/`(10・独自 Control 7 種) |
| 管理画面 | `classes/SWELL_THEME/Menu/` にタブ 8 枚（Colors / Btn / Balloon / Border / Iconbox / Marker / Custom / Others）= **テーマ独自の設定 UI** |
...
| REST | `wp/v2` 名前空間に **14 ルートを間借り**（`/swell-block-settings` `/swell-term-list` `/swell-balloon`(+copy/recover/sort) `/swell-ct-ad-data` `/swell-ct-btn-data` `/swell-ct-pv` `/swell-reset-*` `/swell-do-update-action` `/swell-lazyload-contents`） |
| ブロックパターン | `lib/gutenberg/block_pattern/` に `swell-pattern/*` を登録（カテゴリ登録あり） |
| 目次 | **テーマ内蔵**（`swell_toc` ショートコード + `lib/content_filter.php` で自動挿入） |
| その他 | `Pre_Parse_Blocks.php`（ブロック事前解析）・`lib/post_meta/`(5)・`lib/update/`(69) |

## 4. 実使用（solobiz-lab.com 公開記事 7 本の実測）
...
2. **再利用パーツと広告タグを CPT で正本化**（`blog_parts` / `ad_tag`、いずれも `show_in_rest`）→ **REST から機械操作できる**。JIN:R の番号スロット shortcode と対照的。
3. **設定は単一配列 + 動的 CSS 生成**。トークンは PHP のスタイル生成器が持ち、静的ファイルではない。
4. **拡張点（filter 79）と REST 14 本**を持ち、外部からの介入余地が JIN:R より桁違いに大きい。
5. ただし **REST を `wp/v2` 名前空間に相乗り**させており、名前空間設計としては借用（自前 `swell/v1` を切っていない）。
