# 差分レジスタ — JIN:R × SWELL × HELIX-WP-THEME(agent-neo)

判定値: `対応済` / `部分` / `欠落` / `思想差`（＝埋めるのでなく設計判断が要る差）

## 0. 一枚表

| 軸 | JIN:R | SWELL | agent-neo | 差分判定 |
|---|---|---|---|---|
| テーマ形式 | クラシック | クラシック+ブロック | **FSE / theme.json v3** | 思想差 |
| カスタムブロック | 25（`jinr-blocks/`） | 50（`loos/`） | **1**（`agent-neo/embed`） | 欠落 |
| ブロック登録方式 | functions.php 一括・単一バンドル | block.json + 自前ビルド | — | 思想差 |
| パターン | 0 | `swell-pattern/*` 少数 | **24 + section-registry** | 優位 |
| ショートコード | 6（実使用 186） | 20（実使用 0） | 0 | 部分 |
| CPT | 0 | **3**（lp / blog_parts / ad_tag・全て show_in_rest） | 0 | 欠落 |
| ウィジェットエリア | 11 | **24** | **0** | 欠落 |
| メニュー | 3 | 6 | **0** | 欠落 |
| 設定の持ち方 | `jinr_*` 個別 **1,225 キー** | 単一配列（既定 540 キー） | theme.json + config/*.json | 思想差 |
...
| # | 欠落 | 実測根拠 | 影響 |
|---|---|---|---|
| D-01 | 記事内広告 / CV ゾーン | JIN:R 11 ウィジェットエリア（post-top/start/end/bottom・relatedpost-bottom・sidebar-tracking）/ SWELL 24（single_cta・before/after_related ほか） | 収益導線を置く場所が無い |
| D-02 | 再利用パーツ機構 | SWELL `blog_parts` CPT（REST 可）/ JIN:R 番号スロット shortcode | 共通パーツの一元管理が不可 |
| D-03 | LP 機構 | SWELL CPT `lp` + `single-lp.php` / JIN:R `template-full-width.php` | agent-neo は `page-lp-sample.html` + lp-* パターン 12 で代替 — CPT でない差 |
| D-04 | 装飾ブロック語彙 | 実使用 simplebox 697・button 339・blogcard 330・fukidashi 186 / loos/dt-dd 各 106・faq-item 56・step-item 28 | 既存記事をそのまま描画できない |
| D-05 | 追尾サイドバー / 固定ボトム | JIN:R `sidebar-tracking` / SWELL `fix_sidebar` `fix_bottom_menu` | 主要 CV 面が欠落 |
| D-06 | 目次 | JIN:R=RTOC プラグイン / SWELL=内蔵 | 記事構造の必須要素 |
...
| # | 優位 | 内容 |
|---|---|---|
| A-01 | エージェント接点 | REST 34 コントローラ + MCP + CLI + 中間 JSON（JIN:R は REST 0、SWELL は wp/v2 相乗り 14） |
...
| # | 論点 | JIN:R | SWELL | agent-neo | 判断すべきこと |
|---|---|---|---|---|---|
...
`jinr-blocks/simplebox`(697) / `jinr-blocks/button`(339) / `jinr-blocks/blogcard`(330) /
`jinr-blocks/fukidashi`(186)+`[jinr_fukidashi]`(186) / `jinr-blocks/comparechild`(177) /
`loos/dt`(106) / `loos/dd`(106)

**第 2 群（30〜100）**
`loos/list`(67) / `jinr-blocks/compare`(59) / `loos/faq-item`(56) / `flexible-table-block/table`(46) /
`jinr-blocks/postlist`(38) / `jinr-blocks/designtitle`(37) / `jinr-blocks/background`(33) /
`loos/dl`(30) / `loos/cap-block`(29)

**第 3 群（30 未満・後回し可）** 残り 30 種弱
**未使用（実データ 0）** SWELL の `lp` / `blog_parts` / `ad_tag` CPT・ショートコード 20 種全部・JIN:R の 13 ブロック

> 注: 「未使用」は今の 2 サイトでの話。SWELL 側は公開記事 7 本（AI 産の対照群）しか無いため、
> 母数が小さい。判断材料としては it-shukatu（59 本・人間執筆）の分布を主に見るべき。
