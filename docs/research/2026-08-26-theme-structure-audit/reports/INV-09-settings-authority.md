...
| 軸 | 件数 | 比率 | 扱い |
|---|---:|---:|---|
| A サイト固有の意味 | 50 | 21% | 移管必須（表示文言・URL・SNS 45〔うち値が空 6〕/ 記事・分類の選択 3 / WP コア 2） |
| B 見た目の選択 | 178 | 76% | 任意移管（色・フォント・レイアウト 169 / 表示 ON-OFF 7 / 機能 ON-OFF 2） |
...
option（サイト固有 84%）と theme_mods（見た目 76%）の性質の違いは §2026-08-27 の見立てどおり。
移管必須の確定値は option 152 + theme_mods 50 = **202 キー**、保留 6 はアクセサ読解で確定させる（Sol レビュー 2026-08-31 で文言 3 件を A へ、意匠 9 件と ON/OFF 3 件を B へ訂正）。
