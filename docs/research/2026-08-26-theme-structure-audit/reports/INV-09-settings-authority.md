...
| 軸 | 件数 | 比率 | 扱い |
|---|---:|---:|---|
| A サイト固有の意味 | 56 | 24% | 移管必須（表示文言・URL・SNS 48 / 記事・分類の選択 6 / WP コア 2） |
| B 見た目の選択 | 163 | 69% | 任意移管（色・フォント・レイアウト 156 / 表示 ON-OFF 7） |
| C テーマ内部状態 | 0 | 0% | なし |
| X credential | 0 | 0% | なし（credential は option 側の 2 キーのみ） |
| D 保留 | 16 | 7% | ④ 副作用既定値候補 5（INV-16 の 5 キー。値は既定値と相違＝人が設定）+ キー名から判別できない 11（アクセサ要確認） |

option（サイト固有 84%）と theme_mods（見た目 69%）の性質の違いは §2026-08-27 の見立てどおり。
移管必須の確定値は option 152 + theme_mods 56 = **208 キー**、保留 16 はアクセサ読解で確定させる。
