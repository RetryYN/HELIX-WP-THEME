...
- 一次証跡: `evidence/probe2-raw.txt`（JIN:R 名前空間・SWELL `loos/*` 全 50 名）・
  `evidence/probe3-raw.txt`（JIN:R 登録全 25 種）・`evidence/theme-features-raw.txt`（block.json 名・
  ショートコード）・`evidence/usage-raw.txt`（実使用）・`evidence/re-swell-blocks.txt`（normal/dynamic 分類）

## 1. 母集団

| 系統 | 数 | 内訳 |
|---|---|---|
| JIN:R `jinr-blocks/*` | **25** | 動的 7 / 静的 18。加えて `core/list` のブロックスタイル 2 種 |
| JIN:R ショートコード | 6 | `jinr_button` `jinr_fukidashi` `jinr_heading_iconbox` `jinr_profile` `jinr_simple_iconbox` `message` |
| SWELL `loos/*` | **50** | 通常 22（`register_block`）/ 動的 10（個別ファイル）/ インライン書式ほか 18 |
| SWELL ショートコード | 20 | 日本語別名 4 を含む |
...
| # | 意味 | JIN:R | SWELL | AGENT NEO | 実使用（JIN:R / SWELL） |
|---|---|---|---|---|---|
...
| 4 | 吹き出し | ● `fukidashi` + SC `[jinr_fukidashi]` | ● `balloon`（SSR） + SC `[speech_balloon]` `[ふきだし]` + **独自テーブル `swell_balloon`** | — | **186** / 0 |
...
| 19 | カテゴリ表示 | ● `category`（SSR） | ○ `swell-term-list` REST | — | 0 / — |
...
| 28 | プロフィール | ○ SC `[jinr_profile]` ＋**未登録ブロック `profile`** | ○ ウィジェット `SWELL_Prof_Widget` | ○ `author-profile` パターン | 1 / — |
| 29 | 目次 | ○ 外部プラグイン RTOC | ● SC `[swell_toc]` + `content_filter` | ○ レンダラ生成 | — |
...
**注目**: JIN:R の比較表（177 + 59）と SWELL の定義リスト（106 + 106）・FAQ（56）は
**それぞれのサイトで多用されているが、相手側に対応物が無い**。
Graphix NEO はこの両方を持つ必要がある（どちらのサイトも移管対象になりうるため）。

**SWELL に FAQ ブロックがあり JIN:R に無い**点は `reports/INV-06-structured-data-gap.md` と接続する。
...
| 手順 | `timeline` / `timelinechild` | `step` / `step-item` | JIN:R は「時系列」、SWELL は「手順」。**意味が微妙に違う** |
| リッチメニュー | `richmenu` / `richmenuchild` | `box-menu` / `box-menu-item` | 命名のみ |
| 定義リスト | — | `dl` / `dt` / `dd` + `dl-dt` / `dl-dd` | SWELL 側に**重複した 2 系統**がある（旧実装の残存と推定） |
...
| #5 比較表 | JIN:R 専用ブロック vs プラグイン（`flexible-table-block` 46 回） | 表全般の扱いとして別途 |
| `jinr-blocks/profile` | **登録一覧に無いブロックが本文に 1 回出現** | INV-14（廃止ブロックの残存と推定） |

## 4. ④ インライン書式の扱い — 判定

SWELL は**インライン書式を block.json で 10 種近く定義**している:
`marker` `font-size` `text-color` `bg-color` `nowrap` `inline-icon` `inline-tools`
`text-centered` `clear` `cell-bg`。JIN:R には対応物が無い（CSS 変数 `--marker1/2-color` は持つ）。
...
- [ ] **75 ブロックの属性採取**
      - SWELL: `find themes/swell -name block.json` の全 JSON を読む（機械的に取れる）
      - JIN:R: **block.json が無い**ため、実記事の `<!-- wp:jinr-blocks/… {…} -->` から帰納（INV-14）
- [ ] **save 出力マークアップの採取** — 実記事の HTML から各ブロックの出力形を確定
- [ ] `tabchild` と `tab-body` の意味差の確認
- [ ] SWELL の `dl` / `dt` / `dd` と `dl-dt` / `dl-dd` の 2 系統の関係（旧実装か）
- [ ] `inline-tools` / `cell-bg` の用途確認
- [ ] `jinr-blocks/profile`（未登録ブロック）の実体確認
- [ ] 意図語彙の命名確定（③）— 上記が揃ってから
...
| 内容 | 場所 |
|---|---|
| JIN:R 名前空間の全出現・SWELL `loos/*` 全 50 名 | `evidence/probe2-raw.txt` |
| JIN:R `register_block_type` 全 25 種 + `register_block_style` 2 件 | `evidence/probe3-raw.txt` |
...
| SWELL の CPT 定義（`blog_parts` / `ad_tag` / `lp`） | `evidence/probe4-raw.txt` |
