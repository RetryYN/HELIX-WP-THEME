...
- 一次証跡: `evidence/probe3-raw.txt`（`paidpost` 登録・セッション開始）・
  ...
  `evidence/usage-raw.txt`（実使用回数）・`evidence/probe4-raw.txt`（SWELL CPT 定義）

## 1. 課金・会員機能の実装状況

### 1.1 JIN:R — 決済がテーマに内蔵されている

| 要素 | 実測 | 証跡 |
|---|---|---|
| Stripe SDK | `vendor/stripe/` **286 ファイル**（テーマ全 679 ファイルの 42%） | `evidence/theme-structure-raw.txt` |
| ブロック | `jinr-blocks/paidpost`（**動的**・`jinr_paidpost_dynamic_render_callback`） | `evidence/probe3-raw.txt` |
| 本文フィルタ | `jinr_paid_content_display_switch`（優先度 **9**） | `evidence/re-jinr-render.txt` |
| セッション | `template_redirect` で `session_start()` + `session_regenerate_id()` | `evidence/probe3-raw.txt` L287-294 |
| post meta | `_jinr_paidpost` / `_jinr_paidpost_product_id` | `evidence/re-jinr-accessors.txt` 系 |
| option | `jinr_paidpost_secret_key`（5 参照）/ `jinr_paidpost_subscription_check`（4 参照） | `evidence/theme-features-raw.txt` |
...
`post_content` 集計）に `jinr-blocks/paidpost` は**現れない = 実使用 0**。

**この訂正はスコープ判断を大きく動かす。** 有料記事ブロックは
it-shukatu の公開記事で 1 度も使われていない。

> 補足: `evidence/usage-raw.txt` に `jinr-blocks/profile` が 1 回現れるが、
> このブロック名は `create_block_jinr_blocks_block_init()` の登録一覧（25 種）に**存在しない**。
> 旧バージョンで廃止されたブロックが本文に残っている可能性がある（INV-14 で確認）。

### 1.2 SWELL — 会員限定表示のみ・決済なし

| 要素 | 実測 | 実使用 |
|---|---|---|
| `loos/restricted-area`（動的ブロック） | 登録あり | **0** |
| `[only_login]` / `[only_logout]`（ショートコード） | 登録あり | **0** |
| 決済 | **なし** | — |

証跡: `evidence/re-swell-blocks.txt`（dynamic blocks 一覧）・`evidence/usage-raw.txt`（実使用 0）。
...
| サイト | 失われる機能 | 影響 |
|---|---|---|
| solobiz-lab.com（SWELL） | 会員限定表示 | **実データ 0 のため実質ゼロ** |
| it-shukatu-college.com（JIN:R） | 有料記事（Stripe 決済 + 購読チェック） | **公開記事での実使用 0**（`evidence/usage-raw.txt`）。下書き・非公開記事や option 設定の有無は未確認だが、**公開面では使われていない** |
...
| C. 完全に切る | 有料記事を廃止する | it-shukatu が実運用していれば収益を失う |
...
- [ ] **① it-shukatu の有料記事の実運用確認**
      - 公開記事の `post_content` に `<!-- wp:jinr-blocks/paidpost` が実在するか
      - `_jinr_paidpost` post meta が入っている記事の件数
      - `jinr_paidpost_secret_key` option に値が入っているか（**credential のため値は出力せず有無だけ判定**）
- [ ] **② SWELL の会員限定表示の実使用確認** — 実使用 0 は確認済み。追加確認は不要
- [ ] 既存購読者の有無（Stripe 側のデータ。PO しか確認できない）

> ①の判定は読み取りのみで可能だが、`jinr_paidpost_secret_key` は **credential にあたる**。
...
| SWELL の動的ブロック一覧（`restricted-area`） | `evidence/re-swell-blocks.txt` |
