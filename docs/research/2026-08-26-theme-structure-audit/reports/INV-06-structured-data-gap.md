...
- 一次証跡: `evidence/probe6-raw.txt`（JIN:R / SWELL の JSON-LD `@type` 抽出結果）・
...
| テーマ | 実装 | 行数 |
|---|---|---|
| JIN:R | `include/json-ld.php` | 344 |
| SWELL | `classes/Json_Ld.php` | 472 |
| AGENT NEO | `inc/seo/class-structured-data.php` | — |

JIN:R の `json-ld.php` は `functions.php` から**条件付きで** `get_template_part` される
（`evidence/re-jinr-boot.txt` の require 一覧 L217 に条件分岐あり）。常時出力ではない。

## 2. 出力される `@type` の比較

証跡（`evidence/probe6-raw.txt` の抽出結果をそのまま整理）:

| `@type` | JIN:R | SWELL | AGENT NEO |
|---|---|---|---|
| `Article` | — | **○** | — |
| `BlogPosting` | — | — | **○** |
| `WebPage` | — | ○ | ○ |
...
1. **JIN:R は記事本体の型（`Article` / `BlogPosting`）を出していない。**
...
| 型 | 用途 | 優先度 | 根拠 |
|---|---|---|---|
| `CollectionPage` | カテゴリ・タグ・アーカイブページ | **高** | 記事一覧が主要な流入面。SWELL は実装済み。テンプレートは `archive.html` が既にある |
| `SearchAction`（`WebSite` の `potentialAction`） | サイト内検索（サイトリンク検索ボックス） | 中 | `search.html` テンプレートが既にある。実装コストが小さい |
| `FAQPage` | FAQ ブロックを含む記事 | 中 | 実使用: `loos/faq` 6 / `loos/faq-item` 56。JIN:R 側にも FAQ 相当あり。**中間 JSON の faq ノードから機械的に導出できる** |
| `HowTo` | 手順ブロックを含む記事 | 低 | 実使用: `loos/step` 7 / `loos/step-item` 28。Google のリッチリザルト対象は縮小傾向のため優先度低 |
| `ItemList` | 比較・ランキング記事 | 低〜中 | JIN:R の `compare` 59 / `comparechild` 177。収益記事の中心なので効果は要検討 |

**設計上の含意**: `FAQPage` / `HowTo` / `ItemList` は**中間 JSON の意図ノードから自動生成できる**。
テーマが本文を後から解析する必要がない（SWELL は本文から拾う必要がある）。
これは中間 JSON 方式の明確な優位点で、INV-01（意図語彙）の設計に反映すべき。

## 4. プラグインとの二重出力リスク

両サイトの active プラグイン（`~/dev/poc-wp/evidence/audit-it-shukatu.json` ほか）:

| プラグイン | it-shukatu (JIN:R) | solobiz (SWELL) | 構造化データへの関与 |
|---|---|---|---|
...
**未確定**: JIN:R サイトで記事型の JSON-LD を誰が出しているか。
...
- [ ] **JIN:R の記事型の出所特定** — `automation-seo` プラグインか、出していないか
- [ ] `seo-simple-pack`（solobiz のみ）が JSON-LD を出しているかの確認
...
| 内容 | 場所 |
|---|---|
| JIN:R / SWELL の JSON-LD `@type` 抽出 + 行数 | `evidence/probe6-raw.txt` |
| JIN:R の `json-ld.php` 読み込み条件 | `evidence/re-jinr-boot.txt`（require 一覧 L217） |
| 両サイトの active プラグイン一覧 | `~/dev/poc-wp/evidence/audit-it-shukatu.json` |
