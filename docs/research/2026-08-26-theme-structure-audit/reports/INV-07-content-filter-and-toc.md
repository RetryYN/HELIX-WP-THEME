...
- 一次証跡: `evidence/re-swell-pipeline.txt`（`content_filter.php` 全文）・
  `evidence/re-jinr-render.txt`（`single.php` 全文 + `the_content` フィルタ一覧 + 広告挿入 grep）・
  `evidence/re-jinr-ads.txt`（`jinr_h2_ads_concert` 本体）・`evidence/re-swell-boot.txt`

## 1. 本文変換の全一覧

### 1.1 JIN:R — `the_content` に刺さるのは 3 本だけ

証跡（`evidence/re-jinr-render.txt`、grep 結果そのまま）:

```
include/custom-functions.php:1250  add_filter('the_content', 'wrap_iframe_in_div');
include/custom-functions.php:3433  add_filter('the_content', 'jinr_paid_content_display_switch', 9);
include/custom-functions.php:4523  add_filter('the_content', 'jinr_h2_ads_concert');
```

| # | 変換 | 優先度 | 内容 | 依存 |
|---|---|---|---|---|
| J-1 | `wrap_iframe_in_div` | 既定(10) | `<iframe>` を div でラップ（レスポンシブ対応） | 本文のみ |
| J-2 | `jinr_paid_content_display_switch` | **9** | 有料記事の本文差し替え | ログイン状態 / 購入状態 / セッション |
| J-3 | `jinr_h2_ads_concert` | 既定(10) | 最初の h2 の前に広告を注入 | カテゴリ / options / post meta |

**持っていないもの**: 目次・遅延読み込み・URL の自動カード化・空 p タグ除去。
これらは外部プラグインへ委譲している（it-shukatu の active 構成: `rich-table-of-content`（目次）、
...
つまり JIN:R は「**本文はほぼ素通し、周辺はテンプレートで組む**」方式。

### 1.2 SWELL — 優先度 12 に揃えたパイプライン

証跡（`evidence/re-swell-pipeline.txt`）。登録は `wp_loaded`(20)、本文フィルタは全て優先度 12。
...
| S-7 | URL の自動ブログカード化 `url_to_blog_card` | the_content | `swell_remove_url_to_card` が偽 | 投稿 DB / 外部 OGP |
...
SWELL がこの分離を実装で持っている点（`is_rest()` による経路分離）は
`reports/INV-15-swell-pipeline-transfer.md` §3 に記載。
...
| | JIN:R | SWELL | AGENT NEO（現方針） |
|---|---|---|---|
| 実装 | **持たない**（RTOC プラグイン） | テーマ内蔵 | レンダラ生成 |
| 本文への痕跡 | なし（プラグインが挿入） | プレースホルダ div または自動挿入 | `toc:false` フラグ |
| 配置指定 | プラグイン設定 | ショートコード `[swell_toc]` or 自動（最初の h2 の前） | フラグのみ |
| 広告の同梱 | — | あり（`toc_ad_position` で前後指定） | — |

### 3.2 SWELL の方式が示す解（証跡: `evidence/re-swell-pipeline.txt`）
...
if ( false !== strpos( $content, 'class="swell-toc-placeholder"' ) ) {
	...
	$content = str_replace( '<div class="swell-toc-placeholder"></div>', $toc_content, $content );
	SWELL::$added_toc = true;
} elseif ( $is_content_hook ) {
	if ( SWELL::$added_toc ) return $content;      // 二重生成防止
	…
	$tag = '/^<h2.*?>/im';
...
2. **配置は意図ノードで持つ。** `{ "type": "toc" }` を本文の任意位置に置ける。
   ...
   → SWELL のプレースホルダ方式と同じ考え方だが、HTML 文字列ではなくツリーのノードとして持つ。
...
5. **冪等性を担保する。** レンダラは同じ入力に対し目次を 1 つだけ生成する
   （SWELL の `$added_toc` に相当する保証をレンダラ内部で持つ）。
6. **ページ送りによる配置変更は表示時**（`get_query_var('page')` 相当）。JSON にもレンダラにも入れない。

### 3.4 JIN:R サイト移管時の扱い

JIN:R は目次を持たず RTOC プラグインが挿入している。本文には痕跡が無いため、
...
- [ ] JIN:R の `wrap_iframe_in_div` / `jinr_paid_content_display_switch` の本体読み出し
      （行番号は特定済み: custom-functions.php 1250 / 3433。中身は未読）
- [ ] SWELL `url_to_blog_card` の正規表現全文と、カード化の判定条件の詳細
      （`evidence/re-swell-pipeline.txt` に途中まで採取済み）
...
| 内容 | 場所 |
|---|---|
| SWELL `content_filter.php` 全文（目次・lazysizes・カード化） | `evidence/re-swell-pipeline.txt` |
| JIN:R `the_content` フィルタ 3 本の登録行 | `evidence/re-jinr-render.txt` |
| JIN:R `single.php` 全文（ウィジェットによる周辺挿入） | 同上 |
| JIN:R `jinr_h2_ads_concert` 本体 | `evidence/re-jinr-ads.txt` |
| SWELL `separate.php` / `gutenberg.php` | `evidence/re-swell-boot.txt` |
