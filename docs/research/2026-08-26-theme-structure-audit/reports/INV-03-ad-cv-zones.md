...
- 一次証跡: `evidence/theme-features-raw.txt`（両テーマの `register_sidebar` / sidebar id）・
  `evidence/re-jinr-render.txt`（`single.php` のウィジェット呼び出し位置）・
  `evidence/re-jinr-ads.txt`（`jinr_h2_ads_concert` 本体）・
  `evidence/re-swell-blocks.txt`（`Pre_Parse_Blocks` のウィジェットエリア総当たり）・
  `evidence/probe5-raw.txt`（SWELL `parts/` 構成）
...
| 正規化 ID | 意味 | JIN:R | SWELL | AGENT NEO |
|---|---|---|---|---|
| `article_before` | 記事の前（ヘッダより上） | `post-top-widget` | `single_top` | — |
| `article_head` | タイトル直下・本文の直前 | `post-start-widget` | — | `post-header` パート |
| `content_before_h2` | **本文中・最初の h2 の直前** | `jinr_h2_ads_concert`（フィルタ） | `toc_ad`（目次と連動） | **`ad-zone.schema.json` の `before_h2`** |
...
```json
"description": "広告ゾーン定義スキーマ。CARRY-A2-001: JIN:R の4ゾーン
...
**同列の 4 ゾーン**として例示している。しかし実装（`evidence/re-jinr-ads.txt`）を読むと:

- `category_override` は**ゾーンではなく、ゾーンに対する「上書き規則」**。
  `jinr_h2_ads_concert()` の中で、`before_h2` ゾーンの広告コードを
  カテゴリ 4 スロット（`jinr_choise_category_1`〜`_4`）で差し替える仕組み。
...
| 条件 | JIN:R の実装 | SWELL の実装 |
|---|---|---|
| 記事単位のオプトアウト | post meta `_jinr_ads_display == '1'` で中断 | （未確認） |
| 投稿タイプ限定 | `is_single() && post_type === 'post'` | — |
| カテゴリ別の差し替え | 4 スロット（親子カテゴリ解決つき） | — |
| デバイス別表示 | `jinr_h2_sp_display` / `--*-sp-display` | — |
...
1. **`creative_ref` は参照にする。** JIN:R は `get_option('jinr_h2_ads_code')` の
   **生 HTML をそのまま出力**している（エスケープなし）。広告コードを設定値に直接埋めると
   スキーマ検証も差し替え管理もできない。SWELL の CPT `ad_tag` 方式（`show_in_rest`）に寄せ、
   ゾーン定義は**参照だけ**を持つ（`reports/INV-04-reusable-parts-mechanism.md` の決着と同型）。
2. **`overrides` は配列にして評価順を明示する。** JIN:R の 4 スロットは
...
`jinr_h2_ads_concert()` から確認できた条件（`evidence/re-jinr-ads.txt`）:
...
if ($jinr_h2_sp_display == '1') { … }                                 // デバイス別
```

**「複数カテゴリなら term_id が小さい方」**という決定規則は、
正規化スキーマでも明示する必要がある（暗黙にすると移管で挙動が変わる）。

**未確認**: SWELL 側の条件表示（ウィジェットの表示条件プラグインを使っている可能性）。

## 4. ④ 本番の実配置（未了）

「どのゾーンに実際に何が入っているか」は未取得。これが分かると
...
```
# ウィジェットの配置状況（読み取りのみ）
wp option get sidebars_widgets --format=json --path=~/it-shukatu-college.com/public_html
wp option get sidebars_widgets --format=json --path=~/solobiz-lab.com/public_html

# 広告コードが入っているかの有無だけ確認（値は出さない）
wp option get jinr_h2_ads_code --path=~/it-shukatu-college.com/public_html | wc -c
```

> `jinr_h2_ads_code` は広告タグ（アフィリエイト ID を含みうる）。
...
| `single.php` のウィジェット呼び出し位置 | `evidence/re-jinr-render.txt` |
| `jinr_h2_ads_concert` 本体（条件・カテゴリ解決・挿入） | `evidence/re-jinr-ads.txt` |
| SWELL のウィジェットエリア総当たり（`Pre_Parse_Blocks`） | `evidence/re-swell-blocks.txt` |
| SWELL `parts/` 54 ファイルの構成 | `evidence/probe5-raw.txt` |
