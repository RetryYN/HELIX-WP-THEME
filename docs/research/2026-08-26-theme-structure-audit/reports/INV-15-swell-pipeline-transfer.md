# THEME-INV-15 レポート — SWELL の解析・変換パイプラインの転用可否

- 対象イシュー: `issues/THEME-INV-15-swell-pipeline-transfer.md`
- 状態: **①〜④ 判定完了 / ⑤（参照展開の中間 JSON 表現）は INV-04 と合流のうえ結論**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用（ソース全文読み出し）
- 一次証跡: `evidence/re-swell-blocks.txt`（`Pre_Parse_Blocks` 全文）・
  `evidence/re-swell-pipeline.txt`（`content_filter.php` 全文 + `Style.php` + `block_assets.php`）・
  `evidence/re-swell-boot.txt`（`separate.php` / `gutenberg.php` / autoloader）
...
| 3 | `is_rest()` による保存/表示の分離 | **参考のみ（思想は採用）** | 判定条件が SWELL 固有。ただし「保存 HTML ≠ 表示 HTML」の原則は中間 JSON の前提そのもの |
...
### 1.1 実装の実際（証跡: `evidence/re-swell-blocks.txt`）
...
```php
'[ad_tag'          → loos/ad-tag
'[ふきだし' / '[speech_balloon' → loos/balloon
'cap_box'          → loos/cap-block
'[full_wide_content' → loos/full-wide
'[カスタムバナー' / '[custom_banner' → loos/banner-link
'<table'           → core/table
```
...
### 2.1 実装の実際（証跡: `evidence/re-swell-pipeline.txt`）
...
| SWELL のタイミング | 中間 JSON パイプラインの層 | 対応する処理の性格 |
|---|---|---|
...
**採用すべき原則は 3 つ**:
1. 変換の順序を**優先度という数値ではなく層として固定**する（SWELL は数値で運用しており、
   プラグインとの競合を各所のコメントで回避している。中間 JSON では層で分ける）。
2. **二重適用の防止をフラグで持つ**（SWELL は `SWELL::$added_toc`）。
   レンダラは冪等であるべきなので、同種の防止機構が要る。
3. **無効化点を `apply_filters` で明示する**（`swell_remove_url_to_card`）。
...
自前の遅延読み込みエンドポイント（`swell-lazyload-contents`）だけは変換を通す。
...
- 転用しない理由: `is_rest()` の中身は SWELL 固有の判定（自前エンドポイント名の照合を含む）で、
...
ショートコード `[swell_toc]` は本文に**空のプレースホルダ div を置くだけ**:
...
if ( false !== strpos( $content, 'class="swell-toc-placeholder"' ) ) {
	...
	if ( SWELL::is_show_toc_ad() ) { $toc_ad = \SWELL_PARTS::toc_ad(); }
...
**注意点**: SWELL の実装は正規表現による HTML 文字列操作。中間 JSON では
...
`loos/blog-parts`（attrs.partsID）と `core/block`（attrs.ref）は、
...
| 内容 | 場所 |
|---|---|
| `Pre_Parse_Blocks` 全文（約 290 行） | `evidence/re-swell-blocks.txt` |
| `content_filter.php` 全文 / `Style.php` / `block_assets.php` | `evidence/re-swell-pipeline.txt` |
| `separate.php` / `gutenberg.php` / autoloader / `load_files.php` | `evidence/re-swell-boot.txt` |
| `register_block()` 実装 / `post-link.php` / `Theme_Data` / `rewrite_html` | `evidence/re-swell-detail.txt` |
