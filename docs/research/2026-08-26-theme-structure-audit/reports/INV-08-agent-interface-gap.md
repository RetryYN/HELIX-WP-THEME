...
- 状態: **②③④ 完了 / ①（SWELL 14 ルートの入出力契約）は route 名まで。本体の精読は未了**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用
- 一次証跡: `evidence/theme-features-raw.txt`（両テーマの `register_rest_route` 抽出・filter 本数）・
  `evidence/re-swell-detail.txt`・`evidence/re-jinr-ads.txt`・`reports/INV-13-jinr-rest-endpoints.md`

## 1. 全体像

| | JIN:R | SWELL | AGENT NEO |
|---|---|---|---|
| 独自 REST | 2（`jinr/*`） | 14（**`wp/v2` に相乗り**） | 34 コントローラ（`agent-neo/v1`） |
...
| ルート | 推定用途 | 機械操作での価値 |
|---|---|---|
| `swell-block-settings` | ブロック共通設定の取得（エディタ用） | **中** — ブロックの既定値を知れる |
| `swell-term-list` | タームの一覧取得 | 低（コア REST で代替可） |
| `swell-balloon` / `-copy` / `-recover` / `-sort` | ふきだしの CRUD・並べ替え | **中** — 独自テーブル `swell_balloon` への唯一の API 経路 |
| `swell-ct-ad-data` / `-btn-data` / `-pv` | 広告・ボタンのクリック計測、PV の記録 | **高** — 収益計測データの取得口 |
| `swell-reset-ad-data` / `-cache` / `-settings` | リセット系（**破壊的**） | 触らない |
| `swell-do-update-action` | 更新時アクションの実行 | 触らない |
| `swell-lazyload-contents` | コンテンツ遅延読み込み | 低 |
...
## 3. ② JIN:R サイトを機械操作する経路

JIN:R には操作用 API が存在しない（2 本の REST はブロック描画のための読み取り専用・
詳細は `reports/INV-13-jinr-rest-endpoints.md`）。現実的な経路は次の 4 つ。

| 経路 | 可否 | 何ができるか | リスク |
|---|---|---|---|
| **A. WP コア REST（`wp/v2/posts` 等）** | ○ | 記事・固定ページの CRUD、post meta（`_jinr_*` 27 種）の読み書き | 低。Application Password で認証。PoC-1/2/4 で成立を実証済み |
| **B. WP-CLI（SSH 経由）** | ○ | option / theme_mod / ウィジェット / タームの読み書き、DB クエリ | 中。サーバー到達が前提。実行はサーバー上 |
| **C. option の直接書き換え**（`wp option update` / DB） | △ | カスタマイザ 707 項目・広告設定・`jinr_*` 1,225 キー | **高**。スキーマが無く、値の妥当性を検証する手段が無い。誤値でサイトが壊れる |
| **D. 管理画面のブラウザ操作** | ○ | 上記すべて（人間の操作と同じ） | 中。有人ブラウザ車線。低速だが安全 |

**結論**: JIN:R サイトの操作は **A（記事）+ B/D（設定）** の組み合わせになる。
C は「テーマ設定を機械が直接書く」ことになり、**スキーマ無しでの書き込みは推奨しない**。
どうしても必要なら、先に INV-09 で「どのキーが何を意味するか」の目録を作るのが前提条件。

**post meta 経由でできる記事単位の制御**（`evidence` より、`_jinr_*` 27 種）:
`_jinr_ads_display`（広告の記事単位オプトアウト）・`_jinr_thumbnail_display`・
`_jinr_relatedpost_display`・`_jinr_snsbutton_display`・`_jinr_sidebar1col_display` /
`_jinr_sidebar2col_display`・`_jinr_noindex_display`・`_jinr_canonical_display`・
`_jinr_seotitle_display`・`_jinr_description_display`・`_jinr_keyword_display`・
`_jinr_representations_display`（PR 表記）・`_jinr_profile_display`・`_jinr_title_display`・
`_jinr_hastag_display`・`_jinr_headtag_article`・`_jinr_ogp_image_url`・
`_jinr_url_youtube` ほか YouTube 系 5・`_jinr_paidpost` 系 2・`_jinr_category` 系 2。
...
3. **API バージョニングができない** — 自前名前空間なら `swell/v1` → `v2` と切れるが、
...
「実運用テーマ（JIN:R / SWELL）に対しても意味を持つか」で仕分ける。
...
| **C. 収益・広告系**（他テーマにも概念はあるが実装が違う） | `ad-zones` / `ad-tags` / `ctas` / `affiliate` | JIN:R はウィジェット + option、SWELL は CPT `ad_tag`。**per-site アダプタが要る**（INV-03 / INV-04） |
...
| JIN:R の REST 2 本の全文と内部ディスパッチ | `reports/INV-13-jinr-rest-endpoints.md` / `evidence/re-jinr-ads.txt` |
| SWELL の `pluggable` / `rest_api` 配置 | `evidence/re-probe1.txt`（`functions.php` の require 一覧） |
| `_jinr_*` post meta 27 種 | `evidence/re-jinr-accessors.txt` 系の grep 結果 |
