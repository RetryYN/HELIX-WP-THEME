...
- 状態: **仕分け方法を確定 / 採取済みの部分集合（JIN:R 50 of 151・SWELL 29 of 155）に適用。
  全量の仕分けは追加採取が要る**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用
- 一次証跡: `evidence/probe6-raw.txt`（CSS カスタムプロパティ抽出・総数と先頭 50/29 件）・
  `evidence/re-swell-pipeline.txt`（`Style` クラス）・`evidence/re-jinr-boot.txt`（CSS 生成関数）

## 1. 総数と保持方式

| テーマ | 変数の総数 | 保持方式 | 生成のタイミング |
|---|---|---|---|
| JIN:R | **151** | カスタマイザ値（`jinr_*` 個別 option 1,225 / theme_mod） | `wp_head` で単一 2,098 行関数が全生成 |
| SWELL | **155** | 単一配列 `swell_options`（既定 540 キー） | `classes/Style/`（11 生成器）が動的生成・`$modules` 分離あり |
...
（JIN:R は 50 件、SWELL は `*.php` 由来の 29 件）。全量の仕分けには再採取が要る（§5）。
...
JIN:R の実測には `--header-style-default` / `--header-style-slope` / `--header-style-triangle` /
...
命名規則からして **JIN:R の変数体系は「部品の見た目を変数化したもの」**と判断してよい。

裏付け: 動的 CSS 生成関数は、テーマカラーから
`jinr_hex_to_hsl()` で **hue +30 / +45、明度 +9 を関数内でハードコード計算**しており
（`evidence/re-jinr-boot.txt` L270-272）、派生色を**トークンとして持っていない**。
派生規則がコードに埋まっている＝トークン体系が存在しない証拠。

### 3.2 SWELL（`*.php` 由来 29 件 / 155 件中）
...
**SWELL には D（レイアウト寸法）という第 4 の類型がある。** これは A に含めてよい
...
`--swl-fb` / `--swl-fb_pc` / `--swl-fb_tab` は SWELL 固有のプレフィックス付き。
...
| B. 部品固有スタイル | `styles.blocks.<block>` でブロック単位に書ける | **条件付きで可**。ただし**部品がブロックとして存在する場合のみ**。JIN:R の `--compare-*` のように独自ブロック前提のものは、Graphix NEO に対応ブロックが無ければ写せない |
...
| 文脈依存の値（`--the-*`） | SWELL 全般 | `theme.json` の `styles.blocks` + CSS カスケードで再現。**宣言では表せない部分が残る** |

## 5. 未了項目（追加採取が要る）

- [ ] **JIN:R 151 件の全量取得**（現在 50 件）— `grep -rhoE '\-\-[a-zA-Z0-9_-]+\s*:' | sort -u` の全出力
- [ ] **SWELL 155 件の全量取得**（現在 PHP 由来 29 件）— `*.css` / `*.scss` を含めた全出力
...
1. **JIN:R のトークン体系は移植対象にならない。** 151 件はほぼ B（部品固有）と C（分岐フラグ）で、
   意味的トークンが存在しない。派生色の規則もコードに埋まっている。
   → **参照のみ**（`reports/INV-12-asset-reuse-ledger.md` #2 の判定材料）
2. **SWELL は A + D が一定量あり、`theme.json` へ写せる部分がある。**
...
3. **トークン正本は JSON 一方向投影にできる。** ただし条件が 2 つ:
   ...
   逆に混ぜると JIN:R と同じ「2,098 行の生成関数」に向かう。
...
| JIN:R の派生色ハードコード（hue +30/+45・明度 +9） | `evidence/re-jinr-boot.txt` L270-272 |
| SWELL の `Style` クラス（バケット・`$modules`・front/editor 出し分け） | `evidence/re-swell-pipeline.txt` |
