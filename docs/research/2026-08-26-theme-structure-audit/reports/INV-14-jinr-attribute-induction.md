# THEME-INV-14 レポート — JIN:R ブロック属性表の帰納

- 対象イシュー: `issues/THEME-INV-14-jinr-attribute-induction.md`
- 状態: **③（フォールバック対応）を 1 ブロック分確定 / 共通属性パターンを特定 /
  ①（全量の帰納）は抽出スクリプトを用意して未実行**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用
- 一次証跡: `evidence/re-jinr-ads.txt`（`jinr_blog_card_dynamic_render_callback` 全文）・
  `evidence/probe3-raw.txt`（登録 25 種）・`evidence/usage-raw.txt`（実使用）

## 1. なぜ帰納が要るか

JIN:R は **block.json を持たない**。属性の型・既定値はエディタの単一 minified バンドル
（`editor/build/index.js`）の中にしか無く、機械的に読める契約が存在しない。
一方、実記事の `<!-- wp:jinr-blocks/xxx {…} -->` には**実際に使われた属性が JSON で残っている**。
公開記事 59 + 固定 10 という母集団があるので、そこから帰納するのが現実的な唯一の道。

## 2. 判明している属性（`blogcard` — 全文読了）

`jinr_blog_card_dynamic_render_callback($block_attr, $content)` が参照する属性の全列挙。
...
| `blogcardDesign` | デザイン種別 | **`jinr__blogcard_design()`** ← サイト設定 |
| `blogcardTitle` | ラベル文言 | **`jinr__blogcard_title()`** ← サイト設定 |
| `blogcardLabel` | ラベル | **`jinr__blogcard_title()`** ← サイト設定 |
| `postTitleExternal` | 外部リンクのタイトル | 空文字 |
| `postUrlExternal` | 外部リンクの URL | 空文字 |
| `postImageExternal` | 外部リンクの画像 | **`jinr_noimage_url('small_size')`** |
...
$jinrBlockClassName     = $block_attr['className'] ?: '';
$jinrBlockCSSAttribute  = $block_attr['jinrBlocksCSSAttribute']
    ? '<style jsx="true">' . $block_attr['jinrBlocksCSSAttribute'] . '</style>' : '';
```
...
| `jinrBlocksCSSAttribute` | **ブロック単位のインライン CSS** | 生 CSS 文字列。`<style jsx="true">` として出力 |

**これが JIN:R の「詳細設定」の実体**。値は数値やトークンではなく
...
| 自サイト REST | `rest_do_request('/jinr/post_by_url')` の内部ディスパッチ |
| 投稿 DB | `url_to_postid()` → `get_the_post_thumbnail_url()` ほか |
| サイト設定 | `jinr_get_this_site_domain()` / `jinr__thumbnail_original_used()` |

## 3. フォールバック対応表（③・1 ブロック分確定）

| ブロック | 属性 | フォールバック先 |
|---|---|---|
| `blogcard` | `blogcardDesign` | `jinr__blogcard_design()` |
| `blogcard` | `blogcardTitle` / `blogcardLabel` | `jinr__blogcard_title()` |
| `blogcard` | `postImageExternal` | `jinr_noimage_url('small_size')` |
| （残り 6 動的ブロック） | — | **未読** |

`button`（実使用 339）は CV ボタン設定（`jinr__spcv_all_color` / `_category1〜3_color` 等）を
参照する可能性が高いが未確認。**実使用が最多クラスなので優先して読む。**

## 4. 抽出手順（①・スクリプト化済み・未実行）

`extract-jinr-attrs.sh` を同ディレクトリに用意した。行うことは 3 つ。

1. 公開記事の `post_content` から `<!-- wp:jinr-blocks/… {…} -->` を全抽出
2. ブロック名ごとに属性キーを集計（出現数・値のサンプル）
3. 未登録ブロック（`jinr-blocks/profile`）と `jinrBlocksCSSAttribute` の実使用を数える

```bash
bash docs/research/2026-08-26-theme-structure-audit/extract-jinr-attrs.sh > evidence/jinr-attrs-raw.txt
```
...
| 問い | 出典イシュー |
|---|---|
| `[jinr_fukidashi]` 186 回はブロック由来か手書きか | INV-10 §2 |
| `jinr-blocks/profile`（未登録ブロック）の実体 | INV-01 §3.4 / INV-11 補足 |
| `jinrBlocksCSSAttribute` の実使用有無 | INV-02 §7 |
| `is-style-jinr-checkmark` 系クラスの実使用 | INV-02 §2.3 |
| 25 ブロック × 属性キー × 値域 | INV-01 §5（属性層） |

## 6. 未了項目

- [ ] `extract-jinr-attrs.sh` の実行と結果の分類
...
| 内容 | 場所 |
|---|---|
| `jinr_blog_card_dynamic_render_callback` 全文 | `evidence/re-jinr-ads.txt` |
