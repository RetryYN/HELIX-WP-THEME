# THEME-INV-17 レポート — JIN:R のグローバル改変の影響

- 対象イシュー: `issues/THEME-INV-17-jinr-global-side-effects.md`
- 状態: **コード解析完了 / ①〜③（実挙動の確認）は未実施。検証手順を定義**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用
- 一次証跡: `evidence/probe3-raw.txt` L276-L302（該当コード全文）・
  `evidence/re-jinr-boot.txt`（`functions.php` のフック一覧）
...
**it-shukatu-college.com は SEO 施策サイト**（人間執筆 59 記事・繁忙期に収益が立つ実績サイト）。
...
JIN:R には `_jinr_canonical_display` という post meta があり（`evidence` の post meta 27 種）、
...
**`jinr-blocks/paidpost` の公開記事での実使用は 0**。
有料記事を使っていないなら、このセッション処理は**全ページで純粋なオーバーヘッド**になる。

## 4. 検証手順（シェル復帰後・PO 承認後）

### 4.1 正規化リダイレクトの実挙動（自サイトへの HTTP GET のみ）

```
# 末尾スラッシュ有無
curl -sI https://it-shukatu-college.com/<記事slug>  | head -1
curl -sI https://it-shukatu-college.com/<記事slug>/ | head -1
# ?p=ID 形式
curl -sI 'https://it-shukatu-college.com/?p=<記事ID>' | head -1
# ページ送りの正規化
curl -sI https://it-shukatu-college.com/page/1/ | head -1
```
...
```
curl -s https://it-shukatu-college.com/<記事slug> | grep -i 'rel="canonical"'
```
...
curl -sI https://it-shukatu-college.com/ | grep -i 'set-cookie'
...
```php
// jinr-child/functions.php（案・未適用）
remove_filter('redirect_canonical', 'jinr_disable_redirect_canonical');
remove_action('template_redirect', 'jinr_init_session_start');
```

**前提**: 記事内ページネーションが壊れないことの確認（§6）。
JIN:R の `object/nextpage.php` が `<!--nextpage-->` によるページ送りを担っており、
`redirect_canonical` を戻すと `/pages/2/` 形式が `/2/` へリダイレクトされる可能性がある。

## 6. 未了項目

- [ ] §4.1〜4.3 の実挙動確認（PO 承認後）
...
| `functions.php` のフック登録一覧 | `evidence/re-jinr-boot.txt` |
| `paidpost` の実使用 0 の確認 | `evidence/usage-raw.txt` / `reports/INV-11-scope-boundary.md` |
| post meta `_jinr_canonical_display` の存在 | `reports/INV-08-agent-interface-gap.md` §3 |
