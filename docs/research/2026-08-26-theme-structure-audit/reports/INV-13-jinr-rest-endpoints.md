# THEME-INV-13 レポート — JIN:R の未認証 REST エンドポイント

- 対象イシュー: `issues/THEME-INV-13-jinr-rest-ssrf.md`
- 状態: **コード解析パート完了 / 到達性検証は未実施（PO 承認待ち）**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用（`sed -n` によるソース読み出しのみ）
- 対象サイト: it-shukatu-college.com（JIN:R 1.4.6・WP 7.0.2）

> ## ⚠ 証拠ギャップ（2026-08-26 検証で判明・要是正）
> 本レポートの目玉である REST 2 本の登録・`__return_true`・`file_get_contents()` は、
> **本レポート本文への手写しのみで、生キャプチャファイル（`evidence/re-jinr-rest.txt`）が未生成**。
> `evidence/theme-features-raw.txt` の JIN:R `register_rest_route` 欄は単一行 grep のため**空**で、
> ここには JIN:R の REST が写っていない（SWELL 分のみ採取済み）。
> 独立裏取りがあるのは `post_by_url` の内部ディスパッチ（`evidence/re-jinr-ads.txt` の
> `new WP_REST_Request('GET','/jinr/post_by_url')` → `rest_do_request`）**のみ**。
> `external_url` の存在・両者の permission_callback・SSRF 構造は現状**未採取**。
> **是正**: Bash/SSH 復旧後に PROGRESS.md 手順 3（`re-jinr-rest.txt` の生採取。読み取り専用）を
> 最優先で実行し、本文の引用が実ソースと一致することを確定させる。到達性の HTTP 実証は別途 PO 承認。

## 1. 結論

JIN:R は独自 REST を **2 本**登録している。両方とも `permission_callback` が `__return_true`
（＝**認証・権限チェックなし**）。うち `jinr/external_url` は、クエリで受け取った URL を
検証せずに `file_get_contents()` へ渡しており、**SSRF の構造**を持つ。

| # | ルート | メソッド | permission_callback | リスク |
|---|---|---|---|---|
| 1 | `jinr/post_by_url` | GET (`WP_REST_Server::READABLE`) | `__return_true` | 情報露出（低〜中） |
| 2 | `jinr/external_url` | GET (`WP_REST_Server::READABLE`) | `__return_true` | **SSRF（高）** |

前回の構造調査で「JIN:R の独自 REST は 0 本」と報告した点の訂正。
...
### 2.1 `jinr/external_url`（3775-3800 付近）
...
### 2.2 `jinr/post_by_url`（3670-3700 付近）
...
custom-functions.php:3616  add_action('rest_api_init', 'jinr_slug_register_views');
```
...
`jinr/post_by_url` は **ブロック描画のために内部から呼ばれている**。
`jinr_blog_card_dynamic_render_callback()`（内部リンクのブログカード）が、
...
`jinr/external_url` の呼び出し元は、外部リンクのブログカード作成時に
...
| サイト | セキュリティ系プラグイン | 備考 |
|---|---|---|
| it-shukatu-college.com（JIN:R） | **なし** | `audit-it-shukatu.json` の active 11 件に該当なし |
| solobiz-lab.com（SWELL） | `cloudsecure-wp-security` | JIN:R サイトには入っていない |
...
|---|---|---|---|---|
| A. WAF / サーバ層で `/wp-json/jinr/external_url` を遮断 | HTTP 経由のみ遮断 | ○（テーマ外） | **なし**（内部ディスパッチは通る） | 小 |
...
- [ ] **到達性の確認** — 自サイトに対して `/wp-json/jinr/external_url?url=…` を実際に叩く。
...
| ブロック描画からの内部ディスパッチ | `evidence/re-jinr-ads.txt`（`jinr_blog_card_dynamic_render_callback`） |
| プラグイン構成 | `~/dev/poc-wp/evidence/audit-it-shukatu.json`（2026-08 時点の監査） |

> 注: §2 のソースは調査セッション中に SSH 経由で読み出したものを転記した。
> 生キャプチャファイルとしての再取得は未実施（`evidence/re-jinr-rest.txt` として別途採取する）。
