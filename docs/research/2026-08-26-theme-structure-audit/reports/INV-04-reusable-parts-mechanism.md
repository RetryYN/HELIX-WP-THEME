...
- 状態: **① SWELL 側完了 / JIN:R 側は所在未特定 / ③④ 判定完了**
- 調査日: 2026-08-26
- 手段: XServer SSH 読み取り専用
- 一次証跡: `evidence/probe4-raw.txt`（`lib/post_type.php` 全文）・
  `evidence/re-swell-blocks.txt`（`Pre_Parse_Blocks` の参照解決）・
  `evidence/re-swell-detail.txt`（`Theme_Data` の DB 定義）・`evidence/probe2-raw.txt`

## 1. SWELL — CPT による正本化（完全に読了）
...
| 参照元 | 属性 | 証跡 |
|---|---|---|
| `loos/blog-parts` | `attrs.partsID` | `evidence/re-swell-blocks.txt`（`check_parsed_block`） |
| `core/block`（同期パターン） | `attrs.ref` | 同上 |
| ターム単位の表示 | term_meta `swell_term_meta_display_parts` | 同上（`Pre_Parse_Blocks::init`） |
...
|---|---|---|---|---|
| SWELL | CPT `blog_parts`（`show_in_rest`） | `rest` | ○ | ○（post revisions） |
| JIN:R | 番号スロット型 theme options | `cli` or `db` | △（スキーマなしのため非推奨） | **×** |
| Graphix NEO | 未定（CPT 方式を推奨） | `rest` | ○ | ○ |

**設計上の推奨**: Graphix NEO は **SWELL 方式（`public=false` + `show_in_rest=true` の CPT）を採る**。
理由は本レポート §1.1 のとおり、「単体公開しないが機械操作できる」という要件に
WordPress の標準機構がそのまま合致するため。番号スロット方式（JIN:R）は
...
- [ ] **JIN:R の番号スロット実装の特定** — `jinr-setting.php` / `custom-functions.php` の精読。
      オプションキー名・上限・保存形式（HTML 生文字列か）を確定する
- [ ] SWELL の `loos/blog-parts` 2 箇所の参照先が実在するかの確認（壊れた参照の可能性）
- [ ] `lp` CPT の扱い（INV-03 の LP 機構と合流）
- [ ] `swell_term_meta_display_parts` によるターム単位の表示の要否判断
...
| `Pre_Parse_Blocks` の参照解決（partsID / ref / term_meta） | `evidence/re-swell-blocks.txt` |
| `Theme_Data` の DB 名定義・独自テーブル | `evidence/re-swell-detail.txt` |
| JIN:R に CPT が無いことの確認 | `evidence/theme-features-raw.txt` / `evidence/probe2-raw.txt` |
