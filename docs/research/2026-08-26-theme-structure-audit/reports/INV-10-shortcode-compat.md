...
- 状態: **②③④ 判定完了 / ①（`[jinr_fukidashi]` 186 回の由来）は仮説まで。検証手順を定義**
- 調査日: 2026-08-26
...
| ショートコード | 回数 | 備考 |
|---|---|---|
| `[jinr_fukidashi` | **186** | 同名ブロック `jinr-blocks/fukidashi` の使用回数と**完全一致** |
| `[smartslider` | 1 | プラグイン `smart-slider-3` 由来 |
| `[jinr_profile` | 1 | |
| `[jinr_heading_iconbox` | 1 | |
| `[contact` | 1 | プラグイン `contact-form-7` 由来 |

**未使用**: `jinr_button` / `jinr_simple_iconbox` / `message`

### 1.2 SWELL — 20 種登録・実使用 0
...
speech_balloon / swell_toc
+ 日本語別名: ふきだし / アイコン / カスタムバナー / ブログパーツ
```

本文中の実使用: **0**（solobiz は公開記事 7 本のみ。母数が小さい点に注意）。

**日本語別名の存在**が示す設計意図: 非技術者が本文へ直接書くことを想定している。
SWELL は同じ機能をブロック・ショートコードの両方で提供し、
`Pre_Parse_Blocks::check_content_str()` が**ショートコード記法も文字列検査で拾う**
（`[ふきだし` / `[speech_balloon` → `loos/balloon`）ことからも、
両方が実運用で使われる前提の実装になっている（`reports/INV-15-swell-pipeline-transfer.md` §1.3）。

## 2. ① `[jinr_fukidashi]` 186 回の由来 — 仮説

**観測**: `jinr-blocks/fukidashi`（ブロック）186 回 と `[jinr_fukidashi`（ショートコード）186 回が**完全一致**。

**確定している事実**:
- `jinr-blocks/fukidashi` は**静的ブロック**（`render_callback` を持たない。`evidence/probe3-raw.txt`）。
  つまり保存時に save 出力が `post_content` へ焼き込まれる。
- `jinr_fukidashi` ショートコードは `include/shortcode.php` に登録されている。

**仮説**: **ブロックの save 出力がショートコード文字列を含んでいる。**
静的ブロックのマークアップとして `[jinr_fukidashi ...]` を出力し、
...
| **未使用のショートコード** | 対応不要 | JIN:R 3 種・SWELL 20 種は実データ 0 |
...
- ただし**設計意図としては重要**: 「非技術者が扱える語彙にする」という要求は、
  ...
  `loos/cap-block` のような実装由来の名前ではなく、`balloon` / `caption_box` のように
  意味が分かる名前を採る根拠になる。
...
wp db query "SELECT post_content FROM wp_posts
  ...
  | grep -o '<!-- wp:jinr-blocks/fukidashi.\{0,400\}' | head -3
```

- 出力に `[jinr_fukidashi` がブロックコメント直後に現れれば **ブロック由来（仮説どおり）**
- 現れなければ手書き。その場合はショートコード単独の出現箇所を別途数える

あわせて `include/shortcode.php` の `jinr_fukidashi` 実装（368 行のうち該当部）を読み、
...
- [ ] `[jinr_profile` / `[jinr_heading_iconbox`（各 1 回）の実記事特定 — 少数なので個別対応でよい
- [ ] SWELL 側の母数不足の補正 — solobiz の記事が増えたら再計測
...
| SWELL の文字列検査によるショートコード補完 | `evidence/re-swell-blocks.txt` |
