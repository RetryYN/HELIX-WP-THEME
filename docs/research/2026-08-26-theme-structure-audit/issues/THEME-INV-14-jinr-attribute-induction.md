# THEME-INV-14: JIN:R ブロック属性表を保存済みコンテンツから帰納する
...
つまり SWELL のように「block.json を読めば属性表ができる」という道が無い。
一方で `<!-- wp:jinr-blocks/xxx {…} -->` のコメントには**実際に使われた属性が JSON で残っている**。
...
## 調査項目
1. 公開記事の `post_content` から `jinr-blocks/*` のブロックコメントを全抽出し、
...
3. render_callback を持つ 9 種以上について、**未指定時に参照されるカスタマイザ値**を対応付ける
   （例: `blogcardDesign` 未指定 → `jinr__blogcard_design()`）
4. 「保存属性 + サイト設定 → 実効属性」を解決する関数を定義できるか判定する
5. `jinrBlocksCSSAttribute`（ブロック単位インライン CSS）の実使用有無と扱いを決める
