...
| テーマ | 保持方式 | 規模 |
|---|---|---|
| JIN:R | `jinr_*` **個別オプション 1,225 キー** + カスタマイザ 162 ファイル | 巨大・分散 |
| SWELL | 単一配列 `swell_options`（既定 **540 キー**）+ 独自管理画面タブ 8 枚 | 集約 |
...
## 調査項目
1. `jinr_*` 1,225 キーを分類（サイト固有の意味 / 見た目 / テーマ内部状態 / 廃棄可）
2. `swell_options` 540 既定キーを同じ軸で分類
