...
- JIN:R: CPT **無し**。再利用パーツは番号スロット型 shortcode + テーマオプション（`jinr_*` 1,225 キーの一部）
  → **REST 経路が無く、オプション書き換えしか手が無い**
- agent-neo: CPT 0・該当機構なし（パターンは静的ファイル）

現状の実データは両サイトとも `blog_parts` 0 件（SWELL サイトは記事 7 本のみ）。
**機構としては存在するが運用では未使用**という状態を、移植判断でどう扱うか決める必要がある。

## 調査項目
1. `blog_parts` / `ad_tag` の保存形（post_content にブロック列）と参照形（ブロック属性の ID 参照）を採取
2. JIN:R の番号スロット（1〜10 上限）の実体（theme options のどのキーか）を特定し、上限の根拠を確認
