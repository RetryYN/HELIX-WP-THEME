# THEME-INV-17: JIN:R のグローバル改変（正規化リダイレクト無効化・全ページセッション）の影響を確定する

labels: investigation, seo, performance, priority:high
depends: なし

> **状態: 一次完了 / PO 承認待ち**（2026-08-26）／レポート: `../reports/INV-17-jinr-global-side-effects.md`
...
> **残**: 実挙動の確認（HTTP GET・**PO 承認が要る**）と `object/nextpage.php` の精読。

## 背景（コード実測 — `10-reverse-jinr.md` §9.5 / 証跡 `evidence/probe3-raw.txt` L254-L302）
...
**it-shukatu-college.com は SEO 施策サイト**であり、この挙動は成果に直結しうる。
...
これは JIN:R テーマの実装であり、修正するとテーマ更新で戻る。
対処するなら子テーマ側で `remove_filter('redirect_canonical', 'jinr_disable_redirect_canonical')` /
`remove_action('template_redirect', 'jinr_init_session_start')` を行う形になるが、
