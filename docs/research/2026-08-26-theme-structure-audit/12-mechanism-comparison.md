...
`10-reverse-jinr.md` / `11-reverse-swell.md` と、本リポの `agent-neo-theme` + `agent-neo-core` の実装から。
...
**含意**: JIN:R は「どのファイルが何に依存しているか」を機械的に知る方法が無い。
...
| | JIN:R | SWELL | agent-neo |
|---|---|---|---|
| 格納 | `jinr_*` 個別 option 1,225 + theme_mod | `swell_options` 等 **配列 4 グループ** + 独自テーブル `swell_balloon` | `theme.json` + `config/*.json` 7 本 |
...
SWELL 側は既定値ファイルとの差分を取れば機械的に抽出できるが、
**JIN:R 側は 707 のアクセサを 1 つずつ読むしか目録を作る方法が無い**。
...
SWELL は生成器が分割されているぶん**意味への再解釈がしやすい**。
JIN:R は 2,098 行の巨大関数を読み解く以外に対応表を作る道が無い。
さらに JIN:R の DB write 副作用は、**読み取り専用のつもりの解析でサイト状態が変わりうる**という
...
**含意（THEME-INV-07）**: SWELL は「保存 HTML」と「表示 HTML」を明確に分けている。
これは中間 JSON パイプラインの発想と同型で、**変換レイヤの置き場所として既に成立している前例**。
JIN:R は本文をほぼ素通しし、装飾はブロックの save 出力に固定されている。

## 5. 「使われているものを知る」機構

- **SWELL**: `Pre_Parse_Blocks` が `wp_head(0)` で
...
- SWELL のブロックは block.json という機械可読な契約を持つので、**属性表を自動生成できる**。
- JIN:R のブロックは契約が PHP と JS バンドルに散っており、**属性は保存済み HTML から
  逆算するしかない**（実記事の `<!-- wp:jinr-blocks/… {…} -->` を集めて帰納する）。
- さらに JIN:R は未指定属性がサイト設定で変わるため、**同じ保存内容 → 同じ出力を仮定できない**。
...
**含意（THEME-INV-08）**: JIN:R サイトを機械操作する現実的な経路は
「WP コア REST（投稿本体）」＋「option 直接書き換え」の 2 つだけ。
テーマ由来の安全な操作 API は存在しない。

## 8. 移植方針への含意（まとめ）

1. **JIN:R は「実装を移植する」対象ではない。** 契約が無く、環境結合が強く、拡張点も無い。
   取るべきは**出力マークアップと意味構造**であって、コードではない。
2. **SWELL は契約（block.json）と変換パイプラインの前例として読む価値が高い。**
...
4. 3 者に共通して欠けているのは**「設定 → 出力」の依存を機械的に追える形**。
   ...
   移管対象（THEME-INV-09）は「JIN:R/SWELL の設定を agent-neo 側の JSON へ写せるか」で判定する。
