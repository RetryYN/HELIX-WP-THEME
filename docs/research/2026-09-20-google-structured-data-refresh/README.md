...
| Product | 購入できない編集記事は product snippet、購入可能な商品面は merchant listing の契約を分ける。商品情報は初期 HTML に置く方が安定し、JS 後挿入だけを正本にしない。 | `WT-FR-SELL-02` の表示と JSON-LD の一元化、販売経路別の未検証境界を維持する。 |
