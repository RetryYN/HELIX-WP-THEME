...
- ◎ 14 / ○ 17 / △ 15 / × 28（2026-09-05 に PO 指摘由来の 8 行を追加: アイキャッチ・メディア枠・カード高さ・自動コントラスト・アニメーション・奥行き・空間パターン・カテゴリ ミニ HOME）。見た目の土台（トークン・variation・囲み・ボタン・手順・FAQ・pros-cons・アイコン）は揃い、**「選ぶ場所」（S5 slot 割当、S6 投稿メタ、S7 設定 JSON、S8 device 差分、S9 正本参照）が未実装**。手組み HTML で置いたメディア部品・商品系は新規ブロック 6 種への置換待ち。
