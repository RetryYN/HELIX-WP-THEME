...
- 追記 2026-09-05（2）: PO 指摘を受けて header 9 型・footer 3 型 + サイトマップ・末尾セット・リンクカード 3 型・カテゴリ ミニ HOME・404 3 変種 + CV・ゾーン Z1〜Z11・サイトパターン variation 5 本をカタログ（scratchpad、DOM 注入）で提示。反映先は #98 / #100 / #102 / #107 / #109 / #112 / #122 / #129。「h2 ×3 / h3 ×2 / ボタン ×3」は PO 指定ではなく LOOK-01 の Claude 最小案が要求文に残ったもの。PO 指示 2026-09-05「目標数ではなく用途に必要なだけ」により、固定数ではなく用途由来の一覧へ置き換える改定候補（#122、要求改定 PR で扱う）。
...
- ◎ 14 / ○ 17 / △ 15 / × 33（2026-09-05 に PO 指摘由来の 8 行を追加: アイキャッチ・メディア枠・カード高さ・自動コントラスト・アニメーション・奥行き・空間パターン・カテゴリ ミニ HOME）。見た目の土台（トークン・variation・囲み・ボタン・手順・FAQ・pros-cons・アイコン）は揃い、**「選ぶ場所」（S5 slot 割当、S6 投稿メタ、S7 設定 JSON、S8 device 差分、S9 正本参照）が未実装**。手組み HTML で置いたメディア部品・商品系は新規ブロック 6 種への置換待ち。
