...
| SP 専用広告面 | SP-02 | S5: slot 条件描画（第三者の表示制御プラグインでは不可） | × | #102 |
...
| 影 1 段・出現 1 種・hover transition | LOOK-01, NFR-A11Y-02 | S1: 影の強弱、reduced-motion で停止 | ○ 出現アニメは停止するが、ボタン hover の transition は未対応（是正 PR で対応） | – |
...
- ◎ 13 / ○ 18 / △ 15 / × 33（2026-09-05 に PO 指摘由来の 8 行を追加: アイキャッチ・メディア枠・カード高さ・自動コントラスト・アニメーション・奥行き・空間パターン・カテゴリ ミニ HOME）。見た目の土台（トークン・variation・囲み・ボタン・手順・FAQ・pros-cons・アイコン）は揃い、**「選ぶ場所」（S5 slot 割当、S6 投稿メタ、S7 設定 JSON、S8 device 差分、S9 正本参照）が未実装**。手組み HTML で置いたメディア部品・商品系は新規ブロック 6 種への置換待ち。
