...
?>
<!-- wp:html --><div class="wt-tail-icons" aria-label="共有先"><p class="wt-tail-icons__label">この記事を共有</p><?php foreach ( $wt_targets as $t ) : ?><a class="wt-sns wt-sns--<?php echo esc_attr( $t[0] ); ?>" href="<?php echo esc_url( $t[2] ); ?>" target="_blank" rel="noopener nofollow" aria-label="<?php echo esc_attr( $t[3] ); ?>" data-wt-sns="<?php echo esc_attr( $t[0] ); ?>"><?php echo wp_kses( $t[1], array( 'i' => array( 'class' => array(), 'aria-hidden' => array() ), 'span' => array( 'aria-hidden' => array() ) ) ); ?></a><?php endforeach; ?><button type="button" data-wt-share="copy" aria-label="リンクをコピー">⧉</button></div><!-- /wp:html -->
