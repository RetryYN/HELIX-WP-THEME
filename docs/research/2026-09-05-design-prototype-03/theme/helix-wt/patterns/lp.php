...
<form class="wt-lp-download wt-lp-download--form-inline wt-lp-form-inline" method="post" action="/lp/"><div class="wt-lp-download__card"><img src="<?php echo esc_url( $u ); ?>/feature-1.png" alt="資料の表紙イメージ" width="160" height="200" loading="lazy" decoding="async"><div><h3>選び方チェックリスト（PDF・12 ページ）</h3><label for="lp-dl-name">お名前</label><input id="lp-dl-name" name="name" type="text" autocomplete="name" required><label for="lp-dl-email">メールアドレス</label><input id="lp-dl-email" name="email" type="email" autocomplete="email" required><button class="wt-lp-cta-action" type="submit">ダウンロードする</button><p class="wt-lp-form__note">入力内容は PoC のため送信されません。</p></div></div></form></div>
<!-- /wp:html -->
</section>
<!-- /wp:group -->
