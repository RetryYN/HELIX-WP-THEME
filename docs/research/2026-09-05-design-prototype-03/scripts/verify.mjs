    ...
    for (const [axis, values] of Object.entries(AX)) {
      for (const v of values) {
        ...
        const r = await p.evaluate(([axis, v, values, visSrc]) => {
          ...
          return { body, shown, heroVisible, taps: taps.length, below44, forms, h1: document.querySelectorAll("h1").length, h1Visible: Array.from(document.querySelectorAll("h1")).filter(vis).length, sliderNavVisible };
        ...
        const pass = r.body && JSON.stringify(r.shown) === JSON.stringify(expectShown) && r.heroVisible === 1 && r.h1Visible === 1 && r.below44.length === 0 && r.forms.length === expectForms && r.forms.every((f) => f.submit === 0 && f.inputs >= 2 && f.method === "get" && /^#/.test(f.action) && f.labelled) && (r.sliderNavVisible === null || r.sliderNavVisible === js);
