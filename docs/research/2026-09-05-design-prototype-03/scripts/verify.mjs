...
// (r) formFace（段 11、WT-EVT-0289「フォームの項目追加とかの項目調査」/ WT-EVT-0301「進めて」）: フォーム面 /contact/（page.html + helix-wt/form）と完了 /thanks/（helix-wt/form-thanks）。11 軸の全値を PC / SP / SP JS 無効で検査: 項目の並び（種別 9 × セット 4 は functions/inc/form.php の表と同じ順）、必須表示 2 型、レイアウト 5 型（2col = PC で form が 2 列、label-left = PC で行が 2 列、placeholder-only = ラベルは読み上げ専用で placeholder あり、steps = JS ありは段 1 だけ可視・JS 無効は全部可視）、同意 3 型、送信文言 7 型、エラー 3 型（空送信 → 必須項目だけが is-error、項目下 / 上部まとめ / 両方、フォーカス先）、captcha 3 型、代替導線 5 型、完了 2 型。遷移（JS あり / なし）: 入力 → 確認（値の一致、修正するで値が戻る）→ 完了（separate = /thanks/ へ redirect し ?wt= を引き継ぐ / inline = 同 URL）、confirm=no は直接完了、inline-review は JS ありで同一ページ見直し・JS 無効は確認画面。形式検査（メール / 電話 / 郵便番号 / かな / captcha）の文言。44px、h1 1 つ、外部要求なし、POST・同一ホスト・nonce
{
  ...
  const KINDS = { contact: ["name", "name-kana", "company", "email", "tel", "subject-select", "message"], apply: ["name", "email", "tel", "subject-radio", "people-count", "message"], download: ["name", "company", "email", "tel", "postal", "address"], reservation: ["name", "name-kana", "email", "tel", "date-pref", "time-pref", "people-count", "message"], newsletter: ["email"], recruit: ["name", "name-kana", "email", "tel", "postal", "address", "subject-select", "date-pref", "message", "attachment"], quote: ["company", "department", "name", "email", "tel", "industry-select", "budget-select", "message", "attachment"], trial: ["name", "email", "tel", "company", "how-found"], diagnosis: ["company", "name", "email", "tel", "industry-select", "yesno-questions"] }; // download は同意がリンク文（privacy-link）だが、同意の出し方は form_consent 軸で決まる（既定 checkbox）
  const SETS = { minimal: ["name", "email", "message"], standard: ["name", "company", "email", "tel", "subject-select", "message"], full: ["name", "name-kana", "company", "department", "email", "email-confirm", "tel", "postal", "address", "url", "subject-select", "message", "attachment", "how-found", "newsletter-optin"] };
  const REQ = new Set(["name", "name-kana", "email", "email-confirm", "subject-select", "subject-radio", "message", "date-pref", "time-pref", "people-count", "industry-select", "yesno-questions", "consent", "captcha"]);
  const SUBMIT = { send: "送信する", confirm: "確認画面へ", apply: "申し込む", register: "登録する", download: "ダウンロード", next: "次へ進む" };
  const KIND_SUBMIT = { contact: "送信する", apply: "申し込む", download: "資料をダウンロード", reservation: "予約を申し込む", newsletter: "登録する", recruit: "エントリーする", quote: "見積りを依頼する", trial: "無料で試す", diagnosis: "診断を申し込む" };
  ...
  const READ = ([visSrc]) => { const vis = eval(visSrc); const $ = (s) => document.querySelector(s); const $$ = (s) => Array.from(document.querySelectorAll(s)); const form = $(".wt-form__form"); const rows = $$(".wt-form__row"); const marks = rows.filter((r) => r.querySelector(".wt-form__req")).map((r) => [r.getAttribute("data-wt-field"), r.querySelector(".wt-form__req").className.includes("--label") ? "label" : "asterisk"]);
    const hiddenLabel = rows.filter((r) => vis(r) && r.querySelector(".wt-form__label")).map /* 可視の行だけ（steps の非表示段は除く） */((r) => { const l = r.querySelector(".wt-form__label"); const b = l.getBoundingClientRect(); return [r.getAttribute("data-wt-field"), b.width <= 1 && b.height <= 1]; });
    ...
    return { fields: rows.map((r) => r.getAttribute("data-wt-field")), visibleFields: rows.filter(vis).map((r) => r.getAttribute("data-wt-field")), marks, hiddenLabel, placeholders: rows.filter((r) => r.querySelector("input[placeholder], textarea[placeholder]")).length, submit: form ? (form.querySelector(".wt-form__submit") || {}).textContent : null, submitVis: vis(form && form.querySelector(".wt-form__submit")), nextVis: vis($(".wt-form__next")), steps: $$(".wt-form__steps li").map((l) => l.className), formCols: form ? getComputedStyle(form).gridTemplateColumns.split(" ").filter((x) => x && x !== "none").length : 0, rowCols: rows[0] ? getComputedStyle(rows[0]).gridTemplateColumns.split(" ").filter((x) => x && x !== "none").length : 0, consentBox: !!$("#wt-f-consent"), privacyText: vis($(".wt-form__privacy")), captchaRow: vis($(".wt-form__row--captcha")), captchaSlot: vis($(".wt-form__captcha-slot")), side: $(".wt-form__side") ? { tel: !!$(".wt-form__side a[href^='tel:']"), mail: !!$(".wt-form__side a[href^='mailto:']"), chat: !!$(".wt-form__side button"), app: !!$(".wt-form__side a[href*='#line']"), vis: vis($(".wt-form__side")) } : null, method: form ? (form.getAttribute("method") || "").toLowerCase() : null, sameHost: form ? new URL(form.getAttribute("action"), location.href).host === location.host : null, nonce: !!(form && form.querySelector("input[name=wt_form_nonce]")), hp: !!(form && form.querySelector(".wt-form__hp input")), below44, h1: $$("h1").filter(vis).length, bodyAx, errRows: rows.filter((r) => r.classList.contains("is-error")).map((r) => r.getAttribute("data-wt-field")), errInline: $$(".wt-form__error").filter(vis).length, summary: vis($("#wt-form-summary")), summaryLinks: $$("#wt-form-summary a").length, focused: document.activeElement ? (document.activeElement.id || document.activeElement.tagName) : null, invalid: rows.filter((r) => r.querySelector("[aria-invalid=true]")).map((r) => r.getAttribute("data-wt-field")), step: $(".wt-form") ? $(".wt-form").getAttribute("data-wt-step") : null, jsReady: !!(form && form.getAttribute("data-wt-js")) }; };
  const rows = [];
  ...
  for (const [dev, cfg, js] of [["pc", PC, true], ["sp", SP, true], ["sp", SP, false]]) {
    ...
    for (const [axis, values] of Object.entries(AX)) for (const v of values) {
      ...
      const steps = layout === "steps"; const step1 = ["name", "name-kana", "company", "department", "email", "email-confirm", "tel", "postal", "address", "url"];
      let pass = r0.bodyAx.includes(`wt-${axis.replace(/_/g, "-")}-${v}`) && JSON.stringify(r0.fields) === JSON.stringify(ef) && r0.h1 === 1 && r0.method === "post" && r0.sameHost && r0.nonce && r0.hp && r0.below44.length === 0 && external.length === 0 && r0.jsReady === js && r0.step === "input";
      ...
      if (layout === "placeholder-only") pass = pass && r0.hiddenLabel.filter(([f]) => !["subject-radio", "yesno-questions", "date-pref", "captcha", "attachment"].includes(f)).every(([, h]) => h) && r0.placeholders >= ef.filter((f) => !["consent", "privacy-link", "captcha", "subject-select", "subject-radio", "attachment", "date-pref", "time-pref", "yesno-questions", "industry-select", "budget-select", "how-found", "newsletter-optin"].includes(f)).length; else pass = pass && r0.hiddenLabel.every(([, h]) => !h);
      if (steps) { pass = pass && r0.steps.length === 3 && (js ? (r0.visibleFields.every((f) => step1.includes(f)) && r0.nextVis && !r0.submitVis && r0.steps[0].includes("is-current")) : (r0.visibleFields.length === ef.length && r0.submitVis)); } else pass = pass && r0.visibleFields.length === ef.length && r0.submitVis && !r0.nextVis;
      // 空送信の結果: 必須項目だけ is-error（steps + JS は段 1 の必須だけ）。項目下 / まとめ / 両方。フォーカス先
      const expectErr = steps && js ? reqF.filter((f) => step1.includes(f)) : reqF;
      pass = pass && JSON.stringify(r1.errRows) === JSON.stringify(expectErr) && r1.step === "input" && r1.h1 === 1;
      if (errmode === "inline") pass = pass && r1.errInline === expectErr.length && !r1.summary && JSON.stringify(r1.invalid) === JSON.stringify(expectErr) && r1.focused === "wt-f-" + expectErr[0];
      else if (errmode === "top-summary") pass = pass && r1.errInline === 0 && r1.summary && r1.summaryLinks === expectErr.length && r1.focused === "wt-form-summary";
      else pass = pass && r1.errInline === expectErr.length && r1.summary && r1.summaryLinks === expectErr.length && r1.focused === "wt-form-summary";
    ...
    // 形式検査の文言（不正値）
    { await goto(p, FORM + "?wt=form_fields:full,form_captcha:question", js); await p.fill("#wt-f-name", "山田"); await p.fill("#wt-f-name-kana", "ヤマダ"); await p.fill("#wt-f-email", "bad"); await p.fill("#wt-f-email-confirm", "bad2@example.com"); await p.fill("#wt-f-tel", "12"); await p.fill("#wt-f-postal", "12"); await p.fill("#wt-f-url", "example.com"); await p.selectOption("#wt-f-subject-select", { index: 1 }); await p.fill("#wt-f-message", "x"); await p.fill("#wt-f-captcha", "8"); await p.check("#wt-f-consent"); await clickSubmit(p);
      ...
      const expect = [["name-kana", "ひらがなで入力してください。"], ["email", "メールアドレスの形式が正しくありません。"], ["email-confirm", "メールアドレスが一致しません。"], ["tel", "電話番号の形式が正しくありません。"], ["postal", "郵便番号は 7 桁で入力してください。"], ["url", "https:// から始まる URL を入力してください。"], ["captcha", "答えが違います。"]];
    ...
    for (const confirm of ["yes", "no", "inline-review"]) for (const thanks of ["separate", "inline"]) {
      ...
      pass = pass && done.thanks === "contact" && done.h1 === 1 && done.btn && !done.form && (thanks === "separate" ? done.url === THANKS + "?wt=" + encodeURIComponent(q).replace(/%2C/g, "%2C") && !done.inline : done.url.startsWith(FORM) && done.inline);
      rows.push({ dev, js, axis: "flow", v: q, step1, done, external: Array.from(new Set(external)), pass });
    }
    // steps の段階送り（JS あり）: 段 1 の不備は段 1 に留まる → 埋めて次へ → 段 2 → 段 3 → 確認へ
    if (js) { await goto(p, FORM + "?wt=form_layout:steps", js); await p.click(".wt-form__next"); await p.waitForTimeout(300);
      ...
      await p.fill("#wt-f-name", "山田 太郎"); await p.fill("#wt-f-name-kana", "やまだ たろう"); await p.fill("#wt-f-email", "taro@example.com"); await p.click(".wt-form__next"); await p.waitForTimeout(300);
      const s2 = await p.evaluate(() => ({ cur: document.querySelector(".wt-form__steps li.is-current").getAttribute("data-wt-step-i"), done: document.querySelectorAll(".wt-form__steps li.is-done").length, vis: Array.from(document.querySelectorAll(".wt-form__row")).filter((r) => r.offsetHeight).map((r) => r.getAttribute("data-wt-field")), prevVis: !!document.querySelector(".wt-form__prev") && !document.querySelector(".wt-form__prev").hidden, focused: document.activeElement.id }));
      await p.click(".wt-form__prev"); await p.waitForTimeout(200); const s2b = await p.evaluate(() => ({ cur: document.querySelector(".wt-form__steps li.is-current").getAttribute("data-wt-step-i"), name: document.querySelector("#wt-f-name").value })); await p.click(".wt-form__next"); await p.waitForTimeout(200);
      await p.selectOption("#wt-f-subject-select", { index: 1 }); await p.fill("#wt-f-message", "本文"); await p.click(".wt-form__next"); await p.waitForTimeout(300);
      const s3 = await p.evaluate(() => ({ cur: document.querySelector(".wt-form__steps li.is-current").getAttribute("data-wt-step-i"), submitVis: !document.querySelector(".wt-form__submit").hidden, nextVis: !document.querySelector(".wt-form__next").hidden, vis: Array.from(document.querySelectorAll(".wt-form__row")).filter((r) => r.offsetHeight).map((r) => r.getAttribute("data-wt-field")) }));
      await p.check("#wt-f-consent"); await p.click(".wt-form__submit"); await p.waitForTimeout(500); const s4 = await p.evaluate(() => (document.querySelector(".wt-form") || { getAttribute: () => null }).getAttribute("data-wt-step"));
      const pass = s1.cur === "1" && JSON.stringify(s1.err) === JSON.stringify(["name", "name-kana", "email"]) && s2.cur === "2" && s2.done === 1 && JSON.stringify(s2.vis) === JSON.stringify(["subject-select", "message"]) && s2.prevVis && s2.focused === "wt-f-subject-select" && s2b.cur === "1" && s2b.name === "山田 太郎" && s3.cur === "3" && s3.submitVis && !s3.nextVis && JSON.stringify(s3.vis) === JSON.stringify(["consent"]) && s4 === "confirm";
      rows.push({ dev, js, axis: "flow", v: "steps", s1, s2, s2b, s3, s4, pass }); }
    await ctx.close();
  }
  const axRows = Object.values(AX).reduce((n, a) => n + a.length, 0);
  out.formFace = { pageSource, rows, pass: pageSource === "wp-cli" && rows.length === 3 * (axRows + 1 + 6) + 2 && rows.every((x) => x.pass) };
}
...
const checkList = [
  ...
  ["categoryVariants", out.categoryVariants.pass], ["pageParts", out.pageParts.pass], ["sideFace", out.sideFace.pass], ["sideDefaults", out.sideDefaults.pass], ["sideOwner", out.sideOwner.pass], ["formFace", out.formFace.pass],
];
