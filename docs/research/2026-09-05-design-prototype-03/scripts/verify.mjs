...
// (m) categoryVariants（段 6、WT-EVT-0283）: カテゴリ 12 軸の全型 × PC / SP / SP JS 無効。軸 class・当該型だけ可視・型固有の実体（件数 = wp-cli の投稿数、絞り込みリンクは 200 で同じカテゴリ面に留まる、並べ替えは先頭記事が変わる、右カラムの実トラック数、一覧の実カラム数、カード要素の可視、ランキングの置き場所、CTA の到達先・非送信フォーム・LINE グリフ）・h1 1 つ・44px・到達先なしのページ内リンク 0
{
  ...
  const read = async (cfg, dev, js) => { const ctx = await browser.newContext({ ...cfg, javaScriptEnabled: js }); const p = await ctx.newPage(); const results = [];
    for (const [axis, values] of Object.entries(AX)) for (const v of values) {
      ...
      const r = await p.evaluate(([axis, v, dev, js, visSrc]) => { const vis = eval(visSrc); const $ = (s) => document.querySelector(s); const $$ = (s) => Array.from(document.querySelectorAll(s)); const visN = (s) => $$(s).filter(vis).length; const tracks = (el) => el ? getComputedStyle(el).gridTemplateColumns.split(" ").filter((x) => x && x !== "none").length : 0;
        ...
        if (axis === "cat_cta") { o.ctaShown = ["lp-banner", "newsletter", "line"].filter((x) => vis($(`.wt-cat-cta--${x}`))); const c = $(`.wt-cat-cta--${v}`); o.ctaHref = c && c.querySelector("a") ? c.querySelector("a").href : null; const form = c ? c.querySelector("form") : null; o.nlForm = form ? { method: (form.getAttribute("method") || "").toLowerCase(), action: form.getAttribute("action"), submit: form.querySelectorAll("button:not([type]), button[type=submit], input[type=submit]").length, labelled: !!form.querySelector("label[for=wt-cat-nl-email]") && !!form.querySelector("#wt-cat-nl-email"), poc: form.getAttribute("data-wt-poc-form") } : null; const g = c ? c.querySelector(".wt-i") : null; o.lineGlyph = g ? { vis: vis(g), size: Math.min(g.getBoundingClientRect().width, g.getBoundingClientRect().height), mask: (getComputedStyle(g).maskImage || getComputedStyle(g).webkitMaskImage || "none") !== "none", color: getComputedStyle(g).color, aria: c.querySelector("a") ? c.querySelector("a").getAttribute("aria-label") : null } : null; }
      ...
      if (axis === "cat_cta") { pass = pass && only(r.ctaShown, v); if (v === "newsletter") pass = pass && !!r.nlForm && r.nlForm.method === "get" && r.nlForm.action === "#cat-newsletter" && r.nlForm.submit === 0 && r.nlForm.labelled && r.nlForm.poc === "no-submit"; if (v === "line") pass = pass && !!r.lineGlyph && r.lineGlyph.vis && r.lineGlyph.size >= 16 && r.lineGlyph.mask && !/rgba\(0, 0, 0, 0\)|transparent/.test(r.lineGlyph.color) && /LINE/.test(r.lineGlyph.aria || ""); }
  ...
  for (const u of urls) { const res = await p.request.get(u, { maxRedirects: 0 }); const ok = res.status() === 200 && (!/\/category\/topic-index\//.test(u) || /wt-category/.test(await res.text())); http.links.push({ url: u.replace(BASE, ""), status: res.status(), ok }); }
  const firstTitle = async (u) => { await p.goto(u, { waitUntil: "load" }); return p.evaluate(() => { const t = document.querySelector(".wt-cat-list .wp-block-post-title"); return t ? t.textContent.trim() : null; }); };
  const t0 = await firstTitle(BASE + CATEGORY), t1 = await firstTitle(BASE + CATEGORY + "?orderby=title&order=asc"), tY = await firstTitle(BASE + CATEGORY + "?year=2026");
  http.sort = { defaultFirst: t0, titleAscFirst: t1, yearFirst: tY, pass: !!t0 && !!t1 && t0 !== t1 && !!tY };
  await ctx.close();
  const all = [...sp, ...pc, ...spNoJs];
  out.categoryVariants = { expectedCount, countSource, sp, pc, spNoJs, http, pass: all.length === 47 * 3 && all.every((x) => x.pass) && http.links.length >= 6 && http.links.every((l) => l.ok) && http.sort.pass };
}
