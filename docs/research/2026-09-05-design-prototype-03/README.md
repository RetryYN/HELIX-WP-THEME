...
7. **SNS アイコン（0262 の解釈を反映）**: footer の SNS slot の数字プレースホルダ「1 2 3」を 4 導線（X / 写真共有 / 動画 / チャット。aria-label にサービス名、href は `#` のダミー）の自作グリフに、記事末 icons-row の X / LINE も同じグリフに（はてなは文字 B!）。グリフは交差線・カメラ形・再生ボタン付き枠・吹き出しで、各社のロゴ画像・ロゴの形を模した図形は使わない（Astra 是正: 当初の写真共有グリフは角丸四角 + 円でロゴに近かったためカメラ形に変更）。サービス名と共有 URL 自体は #157 の icons-row で導入済みで本 PR で増やしていない。
8. **verify +7**: `headerVariants`（PC 5 型 + SP 5 型 × JS 有効 / 無効 = 20 判定: 軸 class・部品描画・ナビ可視・SP は全操作要素 44×44、PC はナビ文字リンク以外の操作要素 44×44・型固有部品（center のナビ下段 / two-rows のナビ帯 6 リンク + CTA + 検索 / tel の tel: リンク + CTA）・band は背景 = accent かつ文字 4.5:1・overlay は absolute / 透過 / hero 上端 < 60px / 白文字 + 影 / 上端スクリム α ≥ .82（白背景の画像でも 4.5:1）・JS 無効で同じ要素数）、`announceFullWidth`（可視・左端 ≥ 0・右端 ≤ viewport・幅 ≥ viewport − gutter×2 かつ ≤ viewport。PC は 1120 の旧上限超え）、`numboxNum`（期待番号 01-1 / 01-2 / 02-1 を DOM から組み立て、同じフォントの span で描いた幅と ::before の幅を ±1.5px で照合。counter-set / counter-increment の値も完全一致）、`graphsMore`（5 型 × PC / SP / SP JS 無効: 視覚要素が実際に可視（矩形・display・visibility・opacity）・表の値と視覚値の照合（grouped は --v / data-v / 棒幅の比、column は --v と棒高と値の順、score は data-v と塗り数、gauge は --v と表示値、radar は 5 点 × 2 系列 + 凡例 2 + 軸ラベル 5）・role=img の数を型ごとに固定・figcaption・SP の横はみ出しなし・JS 無効で同じ）、`relatedNoFixture`（6 + 1 件、既定カテゴリ名は wp-cli の `default_category` から取得（未指定時は Uncategorized で代用し source に記録）、自記事なし、画像は読込済み naturalWidth > 0）、`lineIcon`（可視 mark 2 + 追尾 1 がすべて描かれたグリフ（mask-image あり・16px 以上・色が透明でない）・文字 LINE なし、qr の PC / SP 文言切替、ボタン文言 / aria-label に LINE）、`snsIcons`（footer 4 導線 + 記事末 3 導線（x / line / hatena の順）、描かれたグリフ（はてなは文字 B!）・aria-label・44×44・footer は rel トークンに nofollow、JS 無効でも同じ）。
