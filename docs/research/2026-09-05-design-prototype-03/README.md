...
11. Astra 1 巡目（PR #166）「merge 不可」の是正: 【重大】追尾が機能しない（grid の align-items:start で aside が内容高に縮み、last-widget / toc-only の sticky が動く余地が無い）→ aside を align-self:stretch で本文の高さまで伸ばし、verify は 1500px スクロール前後の矩形で「追尾対象が上端に留まり先頭ウィジェットは画面外、aside 高 ≥ 本文高」を判定。【改善】ドロワー複製の参照（for / aria-labelledby / aria-describedby / # リンク）が元要素を指す → id の対応表で書き換え。aria-modal のフォーカス封じ込めが無い → ドロワー以外の body 直下を inert にし、閉じたら戻す（verify は Shift+Tab 後もフォーカスがドロワー内、閉じた後に開くボタンへ復帰）。verify がドロワーの中身を検査していない → 開いたパネル内で順序・フォーム・44px・参照先・# 導線を検査、メガメニューはリンクの可視数。台帳の最終値と README / 用語集 / コメントの不一致 → 同期。観察の other:toc-dropdown が未実装 → toc-dropdown ウィジェットを追加（20 種）。【軽微】台帳 §3a の「試作 03 の現状」→「段 10 着手前」と明記。
