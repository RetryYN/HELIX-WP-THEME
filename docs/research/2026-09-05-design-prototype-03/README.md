...
5. **撮影 `scripts/shots-reaction15.mjs`**: 既定が変わって写真が変わるものを同名で撮り直し（記事全長 article-full / 画面単位 article-screen-NN / 幅 width-* / HP 区間セット 5 種 / 固定ページ一覧 / HP owned セット。分割数・画面数が変わった系列は旧末尾を退避）。部品単体の要素撮影は本文列幅が同じ（`--wt-content-max`）ため撮り直さない。新規: side-from 3（below-hero / top / top + left、PC）、記事・固定ページの既定 first view 2（PC）、記事 SP の画面単位 11〜16 枚目。
