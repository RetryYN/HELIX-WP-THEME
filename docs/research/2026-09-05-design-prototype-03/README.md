...
8. `style.css` / スクリプト版 0.3.14 → 0.3.15。用語集に hero 3 型・CTA search・区間セット 2 種・double-cta・event-sections 4 種・申込 3 型・page-part 15 種・page-parts・面 page を追加。
9. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 84 / fail 0**（83 + `pageParts`、skipped なし）、総合 `pass: true`。`homeFace` 234 行、`eventFace` 126 行 + embedSet、`pageParts` 3 行 + feedSet（展開保存ページ含む、cleanupOk）。INDEX 690 → **783**（新規 96、同名置換 15 のうち分割数が減った旧チャンクと旧 event-page-full 6 枚は INDEX から除外）。初回実行で落ちた 3 件の是正: (a) 検索 CTA の内部レイアウト規則に display を含めたため軸 class なしでも表示され、hero の文字位置も下がって fullbleed / slider の SP コントラスト（α 補間）が落ちた → display は軸 class の規則だけに持たせる、(b) verify の期待順序が既定軸 none（schedule / speakers / map）で隠れる区間を含んでいた → 当該軸の値と既定値から除外して照合、(c) カウントダウンの JS 無効時の表示 `--` が WP の texturize でダッシュに変換された → 文字参照 `&#45;&#45;` で書く。撮影で沿革の年が別カラムに落ちた（grid の自動配置）→ time / div / ::before の列を明示。
