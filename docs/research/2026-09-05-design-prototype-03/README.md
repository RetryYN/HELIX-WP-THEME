...
3. 実機で見つけた実バグ: フォームの POST 処理は `template_redirect`（テンプレート解決前）で走るため、イベント面の判定（`$_wp_current_template_id` / `is_page_template()`）が false になり検証が contact の項目で走った（項目が無いのでエラーも出ず入力に戻るだけ）→ `wt_is_event_page()` に block 階層の規則（template meta が空なら page-{slug}.html）を自前で当てる判定を追加。LP のフォーム区間は段 5 の `lp_sections:extended` のときだけ表示される（block も同じ）。
4. verify: `eventFace` の event_apply に block-form（43 型 × 3。フォームは POST・送信ボタンあり・同一 URL として検査）。`formSlots` 新設 **10 行**（固定値）: LP / イベント × PC / SP / SP JS 無効（ブロックが見え、種別が文脈どおり、他の型は見えない、POST・同一ホスト・nonce、項目の並び、イベントは hero の申込導線が残る、h1 1 つ）+ 既定では DOM にフォームブロックが無い 2 行 + 入力 → 確認 → 完了で /thanks/ へ redirect し種別を引き継ぐ 2 行。
