...
4. verify: `eventFace` の event_apply に block-form（43 型 × 3。フォームは POST・送信ボタンあり・同一 URL として検査）。`formSlots` 新設 **20 行**（固定値）: LP / イベント × PC / SP / SP JS 無効（ブロックが見え、種別が文脈どおり、他の型は見えない、区間の見出し 1 つでブロックの見出しは無い、POST・同一ホスト・nonce、項目の並び、イベントは hero の申込導線が残る、h1 1 つ）+ 既定では DOM にフォームブロックが無い 2 行 + JS 無効の不正値（イベント = 参加形式なし・人数 0、LP = メール形式）がサーバ検証で当該項目のエラーになる 2 行 + 入力 → 確認（種別・項目集合の完全一致・入力値と select の選択名を確認画面で照合）→ 完了で /thanks/ へ redirect し種別を引き継ぐ 2 行 + 境界（slug が event / lp でない固定ページは面 page のまま）1 行 + wp-cli で状態を変えて必ず戻す 7 行（LP の template meta を外して slug 解決 = 既定でブロック無し / block で出る 2、event / lp に別テンプレート page-canvas を明示割当すると面 page になりブロックも出ない 2、静的フロントページにすると front-page.html が優先し面は home 3 = イベント slug 解決 / イベント明示割当 / LP 明示割当）。
...
6. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 90 / fail 0**（89 + `formSlots`、skipped なし）、総合 `pass: true`。`formSlots` **20 行 全 pass**、`eventFace` 43 型 × 3 全 pass。既存 89 検査の合否は変化なし。
