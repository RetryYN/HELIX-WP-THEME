...
3. **numbox × num の連動（0272、Claude 解釈）**: h2 が `is-style-wt-numbox` の本文で、h3 の `is-style-wt-num` を「親番号-連番」（01-1, 01-2 / 02-1）にし h2 ごとに振り直す。`.wp-block-post-content:has(> .is-style-wt-numbox)` で本文単位に切り替えるため、numbox を使わない記事の h3 は従来の単独連番。**適用範囲の限界（Astra 是正）**: `:has()` は本文単位の判定なので、numbox を 1 つでも使う本文では num の h3 がすべて「直前の numbox 番号-連番」になる。通常 h2 が混在する場合も h2 ごとに連番は振り直すが、通常 h2 は親番号を進めない（「numbox 以外の h2 の配下だけ単独連番」は CSS では判定できないため、混在は想定外とし、WT-EVT-0272 の解釈文の「numbox 以外の h2 では従来どおり」は本文単位に読み替える。events.jsonl は append-only のため 0272 は書き換えず本節で補正）。実装上の実測: 親（post-content）に `counter-reset:wt-h3` があると、h2 側の `counter-reset` は Chromium 151 で後続兄弟に伝わらず（02-3 になる）、`counter-set:wt-h3 0` なら伝わる。解釈が違えば PO 反応で修正する。
