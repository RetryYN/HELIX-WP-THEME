...
2. **観察の要点（観察事実）**: HP hero は fullbleed 34% / slider 28% / cards-carousel 11% / product-shot 7%（欠測除外 n=61）で前回の最多型 text-only と一致しないため、暫定既定値は据え置き。先頭セクションは banner-row / news / greeting。区分別上位区間: A 企業 = contact-band / news / service-cards / features / greeting、B サービス = features / contact-band / service-cards / numbers / cases / banner-row / download / logos、D 店舗・スクール = service-cards / contact-band / features / news / banner-row / courses / faq / recruit、E 学校・団体 = contact-band / events / features / news / courses / sns-feed。イベント主集計は hero date-place-block 62%、info icon-list 52%、apply（複数選択）external-form 46% / closed-notice 32% / ticket 15% / postcard 7% / receipt-upload 7% / messaging-app 5%、target-audience（A 69%）・organizer（85%）・tickets（B 75%）・prizes / entry-steps / target-products（D 62 / 62 / 50%）。
...
4. **HP**: hero +3（cards-carousel / product-shot / search-box）、hero CTA +1（search。form ではなく role=search の div で暗黙送信なし）、区間セット +2（shop-school / school-org）と既存セットの補強（corporate に greeting、service に logos）、問い合わせ帯 +1（double-cta）、区間 +8（greeting / logos / stores / events / gallery / sns / history / recruit。recruit 以外は固定ページ用パーツを `wp:pattern` で転用）。カルーセルの class は既存の関連記事 carousel（`.wt-carousel__nav` を body class で隠す規則）と衝突したため `.wt-hcar__*` に分離。
5. **イベント**: 区間セット軸 `event_sections`（seminar 既定 / conference / festival / campaign。B・C は小標本のため「選べる型」）を新設し、区間は既定で非表示・セットで表示と順序を決める（軸 none はセットより優先）。申込 +3（receipt-upload / postcard / messaging-app。台帳の other:* 3 語を語彙へ正式採用する提案）。区間 +8（対象者 / 主催 / 賞品 / 対象商品 / 応募の流れ / 審査員 / ギャラリー / カウントダウン。固定ページ用パーツの転用）。注意事項区間の見出しから「主催」を外した（主催は独立区間）。
6. **verify**: `homeFace` 28 型 × 3 + 区間セット 5 × 3 = 99 行（区間内のパーツが可視・h2 あり・可視画像に読込失敗なし）、`eventFace` 41 型 × 3 = 123 行 + 設定状態（全行で区間の集合と順序を ESETS と照合、パーツ区間の見出し・画像）、新規 `pageParts`（/parts/ を PC / SP / SP JS 無効で: 15 パーツが順に可視・h2・全幅（内側 ≤ 1120px かつ viewport − 48px 以上）・画像読込失敗なし・form なし・検索欄 1・44px・#導線の到達先可視・外部 http(s) 要求なし・カウントダウンは JS で数字 / JS 無効で "--" と開催日・SNS フィード未設定、option 設定で同一ホストの遅延 iframe → 削除して未設定へ戻る cleanupOk）。
7. **撮影 `scripts/shots-reaction13.mjs`**（reaction12 方式）: HP hero 3 + CTA search + double-cta、イベント申込 3、パーツ 15（/parts/ で単体）× SP / PC、全長分割: HP 区間セット 5 種（corporate / service は同名置換、旧チャンクは退避）、イベント区間セット 4 種（旧 event-page-full-* は退避）、パーツ一覧ページ。
...
9. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 84 / fail 0**（83 + `pageParts`、skipped なし）、総合 `pass: true`。`homeFace` 99 行、`eventFace` 123 行 + embedSet、`pageParts` 3 行 + feedSet（cleanupOk）。INDEX 690 → **778**（新規 96、同名置換 15 のうち分割数が減った旧チャンクと旧 event-page-full 6 枚は INDEX から除外）。初回実行で落ちた 3 件の是正: (a) 検索 CTA の内部レイアウト規則に display を含めたため軸 class なしでも表示され、hero の文字位置も下がって fullbleed / slider の SP コントラスト（α 補間）が落ちた → display は軸 class の規則だけに持たせる、(b) verify の期待順序が既定軸 none（schedule / speakers / map）で隠れる区間を含んでいた → 当該軸の値と既定値から除外して照合、(c) カウントダウンの JS 無効時の表示 `--` が WP の texturize でダッシュに変換された → 文字参照 `&#45;&#45;` で書く。撮影で沿革の年が別カラムに落ちた（grid の自動配置）→ time / div / ::before の列を明示。
10. 未決（PO 判断待ち）: date-place-block をイベント hero の暫定既定値へ上げるか（§5 の規則「両台帳で最多」の例外提案）、other:* 3 語の語彙への正式採用、gallery / countdown（未観察の追加提案）を残すか。
