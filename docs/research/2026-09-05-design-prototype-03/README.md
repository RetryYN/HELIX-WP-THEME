...
3. **公開・法令上の配慮**: 事例・口コミ・数値は PoC 用の架空文と明記。受賞名・認証名・媒体名・LINE の実 URL は使わない（リンクは `#line` 等のダミー）。QR は実コードではなくダミー模様の SVG（aria-label に明記）。フォームは `action="/lp/"` で送信されない旨を注記。星の色 `#d97706` と LINE 緑 `#06c755` はブランドカラーではなく機能色として採用（LINE 緑は同社ガイドラインの色に近いが、ロゴは使用していない）。
4. **verify** `lpParts`: 15 型 × SP/PC で「body に軸 class・当該型だけ表示・テキストあり・タップ要素 44px 以上・全入力に label・画像に alt と読み込み」、form は external なら form 要素なし + リンクあり／inline なら form あり、download は form-inline だけ form あり、line qr は PC で QR 可視・SP ボタン非表示、SP で逆。追尾 LINE は fixed・44px 以上・viewport 内・float-cta 非表示。JS 無効でも表示型・テキストが同じ。`lpSections` に extended の順序を追加。
5. **撮影 `scripts/shots-reaction9.mjs`**（reaction7/8 方式）: 新規 41 枚 = extended の分割（SP 3 + PC 6）+ 15 型 × 2 + line-sticky × 2。`CATALOG-INDEX.json` 463 → **504**。撮影環境の実測: extended（SP 11,576 css px / PC 8,556 css px）は要素撮影・fullPage とも DPR1 でも "Unable to capture screenshot"、PC は 1440×4000 / 2200 の viewport でも "Page crashed" となったため、SP 4000px・PC 1600px の viewport（DPR1）で上から順に分割撮影し、dev ごとにブラウザを起動し直す方式にした（1 インスタンスで SP→PC を続けると PC の goto で落ちる）。分割数は撮影前に区間群の高さから決めて予定集合の照合に使う。
6. `style.css` 0.3.10 → 0.3.11。用語集に lp-interview / lp-review / lp-rating / lp-download / lp-form / lp-line と extended・line-sticky を追加。
7. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 70 / fail 0**（69 + `lpParts`、skipped なし）、総合 `pass: true`。`lpSections` は 4 型（full / short / trust は段4 のまま、extended は 17 区間の順序を照合）。`lpVisibleAnchors` は 4 型（extended の副 CTA は `#comparison`）。
