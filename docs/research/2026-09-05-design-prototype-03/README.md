...
2. **設計**: 設定の束を 2 つに分ける。記事側の束 = 既存 `side_layout / side_sticky / side_sp / side_set / side_nav`、HP 側の束 = 新設 `home_side_layout / home_side_sticky / home_side_sp / home_side_set / home_side_nav`（HP 側の既定セットは corporate）。束の台帳 `wt_side_bundles()`（名前 → 軸接頭辞）に登録し、後続の継承セット追加はここに 1 行 + 軸 5 本を足す。PO 決定 WT-EVT-0299「継承しない = 非表示にしない。継承・独自設定・非表示を分離」により独自設定の束 `own_side_*`（既定セット minimal、Claude 暫定）を追加。面ごとに束を選ぶ軸: `cat_side`（article 既定 / home / own = 独自設定 / classic = 段 6 の独自 aside / off = 非表示）、`event_side`（off 既定 / home / article / own）、`page_side`（home 既定 / article / own / off）。HOME は HP 側の束固定で `home_side_layout:none` が OFF、記事は記事側の束固定、LP は持たない。`side_from`（hero の下から）は HP の束に付随。
...
5. **verify**: `sideOwner` 新設（カテゴリ 5 型 / イベント 4 型 / 固定ページ 4 型 / HOME OFF / 束の分離 3（HP で side_set を変えても変わらない、記事で home_side_set を変えても変わらない、own は own_side_set だけを見る）/ LP に無し × PC / SP = 36 行）。`sideFace` / `sideDefaults` は HP を `home_side_*`、固定ページを `page_side:article` で記事側の束に当てる（`sideQ`）。`categoryVariants` は段 6 の独自 aside を `cat_side:classic` で検査（初回実行で前行 load-more の残り処理が次の遷移を中断 ERR_ABORTED → 1 回再試行）。
...
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 87 / fail 0**（86 + `sideOwner`、skipped なし）、総合 `pass: true`。`sideFace` 210 行 / `sideDefaults` 26 行 / `sideOwner` 36 行。INDEX 860 → **879**（reaction16: 93 撮影、新規 19 = cat-side 5 型 × SP / PC の 10 + event-side 4 型 PC + page-side 4 型 PC + home-side-off PC、同名置換 = 段 6 の classic 写真（category-columns / sidebar / children sidebar-tree / ranking sidebar）と HP / 固定ページの束切替後の再撮影（side-from 3 / side-default-page を含む）、stale 0）。初回 verify の不一致 3 件は項目 11。
