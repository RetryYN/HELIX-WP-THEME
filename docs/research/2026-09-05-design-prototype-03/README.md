...
3. 既定: すべて現状維持（ヘッダー・フッター = 共通 site、固定 CTA = HOME・イベント・LP は own、記事・カテゴリ・固定ページは共通の `fixed:none`）。共通の `fixed` の 4 型（none / float-cta / sp-bottom-bar / float-tel）は HP の `home_fixed` と同じ（Claude 案）。
...
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 89 / fail 0**（88 + `chromeOwner`、skipped なし）、総合 `pass: true`。`chromeOwner` 148 行 全 pass。既存の検査（headerVariants / footer / homeFace / eventFace / lp / sideOwner / formFace 等）は既定が現状維持のため変化なし。
9. 制約・未決: 「増やす方向で」の対象の特定（未決 2 件か次段か全体方針か）は PO の確認待ち。ヘッダーの own の束は `header` / `sp` の 2 軸（`width` は共通）。フッター off は法定表示（copyright・法定リンク）も出ない（非表示 = 描画しない。LP の `lp_legal` は LP 面の区間なので別）。固定 CTA の generic 描画は HP と同じ 4 型で、イベントの float-apply（申込導線）は own のときだけ。lp_form / event_apply への段 11 ブロック追加は未着手（次段候補）。既定値は現状維持で、既定の変更は PO 判断。
