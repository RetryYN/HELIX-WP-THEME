...
7. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 86 / fail 0**（85 + `sideDefaults`、skipped なし）、総合 `pass: true`。`sideDefaults` 26 行、`sideFace` 210 行。INDEX 851 → **860**（撮り直し 72 = 同名置換、新規 11 = side-from 3 + 既定 first view 2 + 記事 SP の画面単位 11〜16 枚目 6。Astra 1 巡目【重大】: 旧 reaction1 の 10 画面上限で SP の末尾（本文末・サイドバー・フッター）が欠けていた → 上限を撤廃し全高から枚数を決める（SP 16 / PC 10）。分割数の変化による退避 0）。初回の実行で落ちた 3 検査は既定にサイドバーが出たことによる検査側の前提: `homeFace` が共通サイドバーの検索フォーム（GET・home_url）を HP のフォーム（非送信・`#`）として数えていた → `.wt-side` 内のフォームを除外（サイドバーの検索は sideFace / sideDefaults で検査）。`pageParts` が各パーツの幅 = viewport を期待 → 本文列（wt-side-main）の幅に（SP は 1 カラムで viewport と同じ）。`sideFace` の HP × full セットで aside が本文と同じ高さになり追尾の余地が無く、末尾ウィジェットが aside の末尾（margin-bottom 分上）に押し上げられる → その状態を正として判定に追加。あわせて SP では `.wt-side-layout` の左右余白をやめ（本文列は段 10 以前と同じく端まで）、aside 側に余白を持たせた。
