...
1. **related:slider（WT-EVT-0261、Claude 案）**: `related` 軸に `slider` を追加。1 画面 1 枚（600px 以上は 2 枚）の scroll-snap、`article.js` が前後ボタンとドット（`role=tablist`、44px、`aria-selected`）を生成。自動送りはしない。JS 無効時は横スクロールのみ（ボタン・ドットなし）。既存の carousel（複数枚・8 割送り）は変更なし。
2. **SNS シェア（WT-EVT-0262、PO 確認済み解釈「icons-row に SNS アイコン」）**: `parts/article-tail.html` の共有先 slot を `patterns/share-sns.php` に置換。X / LINE / はてなブックマーク + リンクコピー。href は PHP（`get_permalink` / `get_the_title` を rawurlencode）で生成し JS 無効でも動く。`target=_blank rel="noopener nofollow"`、44px 円形。アイコンは各社ロゴ（商標）を使わず文字グリフ（X / LINE / B!）。共有先の URL は各サービス公式の共有エンドポイント（機能上必要な第三者ドメインで、調査対象の固有名ではない）。**Facebook は未収録**: 共有 URL のパス（sharer）が公開情報検査の非公開 guard（`.public-safety.local.regex`）に含まれており、検査を通すための例外追加は「理由・owner・期限を持つ PO 判断」（`docs/governance/public-repository-safety.md`）のため、PO 判断待ちとした。判断が出れば 1 行追加で対応できる。
...
4. **比較表 rich（WT-EVT-0264 / 0265、Claude 案）**: block style `wt-compare-rich`「比較表: 画像・アイコン・購入リンク」。製品画像の行（`img.wt-tcell__img` 96px、SP 72px）、○×アイコン付きセル（`.wt-tcell__icon--ok/ng/mid`）、購入ボタンの行（`a.wt-tbtn`、44px 以上、`rel="sponsored nofollow"`）。先頭列 sticky、SP は横スクロール。catalog `#cat-table-rich`。
...
6. **PR 表記の既定文言（WT-EVT-0267、PO 決定）**: `functions.php` の自動挿入文・catalog `#cat-pr`・`article-kit.php` の文言を「本記事にはプロモーションが含まれます。」に統一（旧「本記事にはアフィリエイト広告を含みます。評価・掲載順は報酬額で決めていません。」）。他の pr-notice 4 型の文言はプレースホルダ（型の説明文）のまま。`pr:auto` の検出語には「プロモーション」+「含みます」の共起が既にあり、既定文言自体が検出対象になる（重複抑止が働く）。
7. **verify**（`--wpclidir` 指定、**pass 69 / fail 0**、skipped なし）: 追加 6 項目 `prNoticeText`（実記事 pr:on と catalog の文言一致）／`relatedSlider`（SP 1 枚・PC 2 枚、snap、ドット数 = ページ数、選択 1、44px、前後ボタン、1.5 秒待って自動送りなし、JS 無効時はドットなしで横スクロール到達可）／`shareSns`（3 サービス、permalink を含む href、_blank + noopener、44px、外部ホスト、JS 無効でも同じ）／`depthFloat`（motion:on で animation `wt-float`、hover で transform・影が変化し animation 停止、reduced-motion で両方なし。浮遊中は locator が "stable" にならないため DOM API でスクロールし `mouse.move` で hover）／`tableRich`（画像 3 が alt 付きで読み込み済み（lazy のため viewport に入れて待つ）、アイコン 6 以上、ボタン 3 が 44px 以上 + sponsored nofollow、SP は overflow-x auto で枠内）／`footerCredit`（既定 none で非表示、text で法的表記の下に表示、nofollow）。`relatedQuality` は 7 型 × 2 = 14 件に拡張（slider も同一行高・2 行 clamp・16:9 を満たす）。
