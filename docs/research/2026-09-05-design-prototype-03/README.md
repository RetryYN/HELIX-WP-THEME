...
- CSS: `data-wt-lum` ごとにスクリムの不透明度を切替（下端で dark .55 / mid .78 / light .90、未計測 = 既定 .82 の強スクリム）。**同一オリジンでない・読めない画像は属性を付けず既定（強）に倒す**。
- ゲートの検査方法: 白文字 (L=1) と合成輝度 `L × (1 − α)` の比 `(1.05) / (Lc + 0.05)` を本文 4.5:1・大文字（≥18.67px bold / ≥24px）3:1 で判定。G-E1 では `data-wt-lum-value` と CSS のスクリム値から算出（`scripts/verify.mjs` §6 と同じ式）。実測は §4 参照。
...
|---|---|---|---|---|---|---|---|---|---|
| 試作 03 記事 SP 390 | 17 | 1.6 | 22.53 | 18.52 | 17.27 | 61 | 48 | 358 | 0.28 |
| 試作 03 記事 PC 1280 | 17 | 1.7 | 28 | 24 | 20 | 61 | 48 | 680（`cont` 1232 は alignfull 外枠） | 0.42 |
...
## 4. 検証結果（`results/verify.json`、`scripts/verify.mjs`）

| 項目 | 結果 |
|---|---|
| JS 無効描画（SP、`motion:on`） | `html.wt-js` なし、目次 表示・開・リンク 12、`.wt-reveal` 10 件すべて opacity 1、商品カード・比較表・関連カード 7 件 表示、本文 3,023 字表示。お知らせ帯は表示（閉じられないだけ） |
...
| コントラスト（PC、算出） | CTA ボタン 白/#c2410c **5.18:1**、本文リンク（アクセント #1d4ed8/白）6.7:1、補助文字 mute 5.69:1、PR 表記 5.69:1、目次リンク 6.19:1、帯タイトル 6.7:1、ランクバッジ 5.18:1、アウトラインボタン 6.7:1。11 項目すべて 4.5:1 以上 |
| 自動コントラスト guard | dark 画像 L 0.026 → `dark`、白文字比 16.7:1（スクリムなし 13.8）/ mid L 0.255 → `mid`、8.65:1（なし 3.44 = 不合格）/ light L 0.882 → `light`、6.05:1（なし 1.13 = 不合格）。3 輝度とも 4.5:1 以上。記事の hero アイキャッチ（生成写真）は L 0.152 → `mid` |
| 404 | 素の URL・3 変種すべて **HTTP 404**、`noindex`、謝意・原因・検索（ボタン付き）・人気 3 件・カテゴリ・ホーム・CV slot 3 枠・検索語提案 4 件 |
| 44px 監査（SP） | 記事: 対象 52、44px 以上 39、本文中インラインリンク 11（WCAG 2.5.8 の例外）、違反 2 = core の skip-link（1×1、SR 用）と関連カードのタイトル `a`（93×22。カードは `a::after` で全面が当たり判定になるため実効は 44 以上だが、監査ツールはアンカー矩形のみを見る）。404: 対象 24、17 / 5 / 同 2。カタログ: 15、12 / 2 / skip-link のみ。カルーセル変種では title `a` が 251×44 と判定境界 |
| 見出し 1 行 | h2 18.5px、18 字、358px、1 行（19.3 字まで） |
| 目次しきい値 | h2 5 / h3 7 → 目次 5 / 7、`scroll-margin-top` 76px、SP は JS で閉 |
| 動作（別スクリプト） | お知らせ帯: 閉 → 再読込後も非表示（localStorage キー 1 件）。ヘッダー: 下スクロールで隠れ、上で再表示、背景不透明。目次: `#h-3` 到達で「3 製品の比較表」が current。カルーセル前後ボタン 2 組。比較表 SP: `data-th` 32 セル、行が block（カード）。横スクロールなし |

修正した点（初回検証で判明）: PR 表記の重複判定が `is-style-wt-product` に誤一致して未挿入だった（`class="wt-pr "` 判定へ）、次に読むカードが関連グリッドの列幅を継承していた、目次リンク・FAQ summary・フッターナビが 44px 未満だった。

## 5. 描画証跡

`results/` に 151 枚（JPEG q75、長辺 1600 以下。`CATALOG-INDEX.json` に {file, face, part, variant, dev}）。内訳: 記事全長 SP/PC 2 + 画面単位 20、ヘッダー 8（PC 4・SP 3・帯）、アイキャッチ 10、目次 9、h2 12、h3 6、囲み 16、CTA 8、比較表 2、メリデメ 2、評価バー 2、リンクカード 2、PR 2、関連 8、共有 4、4 軸 on/off 24（depth 8・density 4・detext 8・motion 4）、コントラスト guard 6、404 6。全長画像は縮小で判読しにくいため `article-screen-NN-*.jpg` を併用。
