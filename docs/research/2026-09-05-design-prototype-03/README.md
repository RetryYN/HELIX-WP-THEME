...
1. **§2.30 第 10 項の未決 1 への PO 回答**として反映。`side_layout` の既定を none → **right**（記事・固定ページ・HP。台帳の記事 right 92% とも一致）。SP の既定は据え置き below-content（1 カラムで本文の下。PO の「規定値オフ」は SP で横カラムが消えることと解釈し、hidden は既定にしない。解釈は WT-EVT-0292 claude_interpretation に明記）。left は既存の軸値のまま（PO「CV 狙うなら左」）。LP は共通サイドバーを持たない（変更なし）。カテゴリ面は独自の `cat_sidebar` のまま（未決 2 は継続）。
...
5. **撮影 `scripts/shots-reaction15.mjs`**: 既定が変わって写真が変わるものを同名で撮り直し（記事全長 article-full / 画面単位 article-screen-NN / 幅 width-* / HP 区間セット 5 種 / 固定ページ一覧 / HP owned セット。分割数・画面数が変わった系列は旧末尾を退避）。部品単体の要素撮影は本文列幅が同じ（`--wt-content-max`）ため撮り直さない。新規（PC）: side-from 3（below-hero / top / top + left）、記事・固定ページの既定 first view 2。
6. `style.css` / スクリプト版 0.3.16 → 0.3.17。
7. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 86 / fail 0**（85 + `sideDefaults`、skipped なし）、総合 `pass: true`。`sideDefaults` 26 行、`sideFace` 210 行。INDEX 851 → **854**（撮り直し 72 = 同名置換、新規 5、分割数が変わって退避 2）。初回の実行で落ちた 3 検査は既定にサイドバーが出たことによる検査側の前提: `homeFace` が共通サイドバーの検索フォーム（GET・home_url）を HP のフォーム（非送信・`#`）として数えていた → `.wt-side` 内のフォームを除外（サイドバーの検索は sideFace / sideDefaults で検査）。`pageParts` が各パーツの幅 = viewport を期待 → 本文列（wt-side-main）の幅に（SP は 1 カラムで viewport と同じ）。`sideFace` の HP × full セットで aside が本文と同じ高さになり追尾の余地が無く、末尾ウィジェットが aside の末尾（margin-bottom 分上）に押し上げられる → その状態を正として判定に追加。あわせて SP では `.wt-side-layout` の左右余白をやめ（本文列は段 10 以前と同じく端まで）、aside 側に余白を持たせた。
