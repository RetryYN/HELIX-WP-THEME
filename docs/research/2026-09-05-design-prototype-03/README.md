...
3. 再撮影 `scripts/shots-reaction5.mjs`: catalog の `cta-banner-{sp,pc}` と、同 pattern を本文に含む記事 `article-full-{sp,pc}` / `article-screen-*-{sp,pc}` の計 26 枚を同名置換（新規エントリなし。撮影前にキャプションへ PR 表記が残っていれば例外で停止し、既存 JPEG は変換成功時にだけ上書き）。`CATALOG-INDEX.json` は 443 のまま。
