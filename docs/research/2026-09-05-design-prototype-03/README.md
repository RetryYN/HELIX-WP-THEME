...
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 87 / fail 0**（86 + `sideOwner`、skipped なし）、総合 `pass: true`。`sideFace` 210 行 / `sideDefaults` 26 行 / `sideOwner` 64 行（Astra 1 巡目後。初版 36 行）。INDEX 860 → **879**（reaction16: 93 撮影、新規 19 = cat-side 5 型 × SP / PC の 10 + event-side 4 型 PC + page-side 4 型 PC + home-side-off PC、同名置換 = 段 6 の classic 写真（category-columns / sidebar / children sidebar-tree / ranking sidebar）と HP / 固定ページの束切替後の再撮影（side-from 3 / side-default-page を含む）、stale 0）。初回 verify の不一致 3 件は項目 11。
