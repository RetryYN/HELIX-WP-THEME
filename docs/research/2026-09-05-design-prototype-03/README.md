...
3. **numbox × num の連動（0272、Claude 解釈）**: h2 が `is-style-wt-numbox` の本文で、h3 の `is-style-wt-num` を「親番号-連番」（01-1, 01-2 / 02-1）にし h2 ごとに振り直す。適用範囲（Astra 是正 2〜3 巡目で確定）: 「numbox の h2 と同じ親コンテナ（post-content 直下でも Group 内でも）にある num の h3」だけを親番号付きにし（`:is(.wp-block-post-content,.wp-block-post-content *):has(> .is-style-wt-numbox) > …`）、そのコンテナの直後の兄弟から wt-h3 を 0 に戻す。したがって numbox を使わない記事・別コンテナの単独 num は従来の単独連番（01 から）、h2 番号（wt-h2num）は本文全体で続く。同じ親コンテナ内に通常 h2 が混在する場合は h2 ごとに連番を振り直すが親番号は進めない（想定外の混在）。カタログ `#cat-h2-numbox-plain` 以降に「専用 class なしの Group → post-content 直下の単独 num → 後続の別 Group → 単独 num」を置き、verify が実描画文字列で N / N-1 / N-2 / 01 / N+1 / (N+1)-1 / 01（N = それより前の numbox h2 の数 + 1。カタログでは h2 型の見本があるので 02）を照合する。直後の兄弟が num の h3 自身の場合は counter-set が counter-increment の後に適用されるため 1 に set する。
