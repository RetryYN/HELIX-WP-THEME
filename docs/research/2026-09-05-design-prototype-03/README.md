...
> **現在（2026-09-08 整理）**: 段13・PO反応26回目（WT-EVT-0303）まで。保存済み検査90 pass / 0 fail、カタログ978枚。フォームは実送信・保存なし。本文は段ごとの履歴であり、「34軸」「本PR」「未決」等を最新の総数・判断状態として読まない。最新要求・未決・着手順は [現在の足並み](../../requirements/current-alignment.md)、進行は [PLAN](PLAN.md)、種別の抜け漏れは [台帳](../../requirements/discovery/page-type-ledger.md)。次は有料記事・インタビュー・BLPの代表モック B。G3未実施。
...
## 2.25 段5 — HP 面・イベントページ（WT-EVT-0277、Claude 案）

PO 反応 17 回目「何度も言うが、HPページは？イベントとかが組めるページは？なんでいったことを無視するの？」（WT-EVT-0277、WT-PROT-REACTION-38）を受け、試作 03 に **HP 面（フロントページ）** と **イベントページ**（固定ページ template `page-event`）を追加した。要求への昇格は PO 判断（本節は試作反応の材料）。型の既定値は台帳 `../2026-09-05-parts-pattern-taxonomy/home-event-recapture/`（HP 39 件・イベント主集計 8 件、codex-astra 8 巡目「可」、§1e）の観察から選んだ Claude 案で、「一般的標準」ではなく用途限定の試作候補。

1. **HP 面 6 軸**（`?wt=home_*`、`templates/front-page.html` → pattern `helix-wt/home`、`body.wt-face-home`）: `home_hero` text-only（既定、台帳 33%）/ slider / fullbleed / split / article-grid / video、`home_hero_cta` double（既定、62%）/ single / none / tel-button、`home_sections` corporate（既定）/ service / media（用途別の区間セット。corporate = お知らせ→サービス→理由→数字→事例→会社概要→アクセス→問い合わせ、service = サービス→特長→数字→事例→料金→FAQ→CTA 帯→問い合わせ、media = 記事グリッド→カテゴリ→ランキング→バナー列→お知らせ→CTA 帯→問い合わせ）、`home_news` list-with-date（既定、46%）/ tabs / cards / none、`home_contact` tel-form（既定、36%）/ form-only / tel-only / line / none、`home_fixed` none（既定。sticky-header 55% はヘッダー既定で常時）/ float-cta / sp-bottom-bar / float-tel。slider は `assets/js/home.js`（scroll-snap。前後ボタン・ドット・矢印キーのページ送りのみで自動送りはしない。`prefers-reduced-motion` ではスクロールを瞬時移動にする。JS 無効でも横スクロールで全スライドに届く）。
2. **イベントページ 9 軸**（`?wt=event_*`、`templates/page-event.html` → pattern `helix-wt/event`、`body.wt-face-event`、固定ページ slug `event`）: `event_hero` key-visual（既定、主集計 50%）/ photo-overlay / date-place-block / text-only、`event_info` inline-text（既定、62%）/ table / icon-list / none、`event_schedule` none（既定、38%）/ table / timeline / accordion、`event_speakers` none（既定、50%）/ cards-photo / list / single-profile、`event_apply` inline-form（既定、38%）/ external-form / ticket-link / closed-notice、`event_status` open（既定、62%）/ none / few-seats / ended、`event_map` none（既定、50%）/ static-image / text-only（埋め込み地図＝外部サービスは使わない）、`event_fixed` none / sp-bottom-bar / float-apply、`event_share` none / icons / add-to-calendar（後 2 軸は観察に無い型を Claude 案として追加）。区間は 概要 / 開催情報 / スケジュール / 登壇者 / 参加区分 / 申込 / アクセス / FAQ / 協賛 / 過去の開催 / 注意事項・主催。フォームは PoC のため `method="get" action="#…"` で送信しない（`data-wt-poc-form="no-submit"`、ボタンは `type="button"`）。
3. **白文字 hero の可読性**: fullbleed / slider / photo-overlay は文字を下寄せにし、スクリムを `to top, .88 0% / .84 78% / .40 100%` にする（文字矩形の上端が hero 下端から 7 割前後に来るため、α ≥ .84 の帯を 78% まで伸ばす）。下寄せの `display:flex` は表示型の切替（`display:none`）を壊さないよう body 軸 class で限定する。verify は gradient の stop を解析し、文字矩形の上端位置の α を線形補間して白背景でも 4.5:1（α ≥ .82）を要求する（当初の「下端 α のみ」の判定は文字が上に置かれても通る穴だったため置換）。
4. **固定導線と footer**: 固定導線（float-cta / float-tel / sp-bottom-bar / float-apply）がある面では追尾の totop を出さず、footer 下端に 72px + safe-area の余白を足して法定リンクと重ならないようにする。チェックボックスは 24px にし、それを包む label（44px 行）を操作領域として監査する（checkbox / radio は label が 44×44 以上かつ自身が 24px 以上なら合格）。
5. **verify +5**（`scripts/verify.mjs` 末尾、SP / PC / SP JS 無効）: `homeFace`（各軸の全型で body 軸 class・当該型だけ可視・他型は非可視・区間セットの表示区間と順序・h1 が 1 つ可視・操作要素 44px（SP は 24px 下限も）・フォームは get + 送信ボタン無し + label 紐付け）、`homeHeroContrast`（3 の補間 α + テキスト hero の文字色 4.5:1）、`homeFixedOverlap`（固定導線が viewport 内で footer の操作要素・totop と重ならない）、`eventFace`（9 軸 × 全型、申込導線の可視、地図の `role=img`、SP 操作要素）、`eventHeroContrast`（photo-overlay の補間 α、他 3 型は文字・バッジ・メタの 4.5:1）。
6. **撮影 `scripts/shots-reaction11.mjs`**（reaction10 方式 + 全長は viewport 分割。分割数は撮影前に高さから決めて予定集合の照合に使い、区間数が期待と違えば撮影前に失敗する）: 新規 **108 枚**（HP: hero 6 型 × PC/SP、CTA 3 型、区間セット 3 種の全長分割、お知らせ 3 型、問い合わせ 4 型、固定導線 float-cta PC+SP / sp-bottom-bar / float-tel SP。イベント: hero 4 型、開催情報 3 型、スケジュール 3 型、登壇者 3 型、申込 4 型、受付状態 3 型、地図 2 型、共有 2 型、固定導線、全長分割）。INDEX 526 → **634**。
7. `style.css` 0.3.12 → 0.3.13。用語集に face `home` / `event` と 16 パーツ（home 6 + event 10）を追加。
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 82 / fail 0**（77 + 5）、総合 `pass: true`。初回実行で落ちた 5 件の是正: article-grid hero に h1 が無かった（「新着記事」を追加）、fullbleed / slider / photo-overlay の白文字が α .28〜.62 の帯に乗っていた（3）、固定導線が footer の法定リンク・totop と重なっていた（4）、申込フォームのチェックボックスが 13px だった（4）。2 回目で落ちた 2 件: 下寄せの `display:flex` が非表示型にも効いて hero が 2 つ見えた（body 軸 class で限定）、スクリム α .84 の帯（50%）より上に文字上端（67〜74%）があった（78% へ）。3 回目で落ちた 1 件: SP 幅の `.wt-event-schedule--table{display:block}`（横スクロール用）が body 軸 class で限定されておらず、timeline / accordion でも表が見えていた（`body.wt-event-schedule-table` で限定）。撮影側は `event_status` の撮影対象を旧既定の photo-overlay から現既定の key-visual に合わせた。
9. Astra 1 巡目（PR #161）の是正: `style.css` が 0 バイトになっていた（テーマヘッダー消失。main の内容を復元し Version 0.3.13）。slider の写真（`img`）がスクリム `::before` より後に描かれ、白文字が写真に直接載っていた（`isolation:isolate` + `img` z-index −2 / `::before` −1 で fullbleed と同じ重ね順にし、verify は `::before` と `img` の z-index の大小と `elementFromPoint`（文字矩形の左上）が見出し内に落ちることを合否に含める）。`wp_head` 側の `home.js` 重複 enqueue（未定義 `$defer` で PHP 警告）を削除。`wt_is_event_page()` は固定ページの template meta が空で block 階層（`page-event.html` = slug 一致）により解決された場合に false になり、`wt-face-event` が付かず totop 非表示・footer 余白の規則が効いていなかった（解決済み template id `//page-event` も判定）。float-cta / float-apply が SP で viewport 幅を超えていた（`max-width:calc(100vw − 2rem)`）。verify の強化: 操作要素は縦横とも 44px（例外は 44×44 以上の label 内にある checkbox / radio と、段落・リスト・表セル内のインライン文リンク）、フォームは期待数（home_contact tel-form / form-only と event_apply inline-form で 1、他は既定のフォーム 1）と入力 2 個以上・`type` 省略 button / `input[type=image]` も送信ボタン扱い、地図 static-image は `img[alt]` の有無を合否に含め、hero コントラストは PC / SP / SP JS 無効の 3 条件（home 15 行・event 12 行）、固定導線の重なり検査は左右の viewport 逸脱も合否に含め event の sp-bottom-bar / float-apply も対象（SP 5・PC 2）。用語集の event 既定値と割合を主集計 n=8 に揃え、slider の説明（自動送りなし、reduced-motion は瞬時移動）と台帳 README の区分 n（カテゴリ 5〜11、イベント 1〜3）を修正。
   Astra 2 巡目の是正: slider の副 CTA「サービスを見る」が暗いスクリム上で accent 色の文字のままだった（fullbleed と同じ白文字・白枠。verify は fullbleed / slider の CTA を塗りボタンは文字 / 背景 4.5:1、透明ボタンはスクリム α から求めた実効背景で 4.5:1 かつ枠線 = 文字色で判定）。media セットに問い合わせ区間が無く CTA 帯・hero の `#contact` が届かなかった（media に問い合わせ区間を加え 7 区間に。`#service` の副 CTA は media では出さない。verify は各セットで可視のページ内リンク全件の到達先が可視であることを判定し、撮影は media の分割が 1 枚増えて 108 枚 / INDEX 634）。JS 無効でも slider の前後ボタンが `display:flex` で `hidden` を上書きして見えていた（`:not([hidden])` に限定。verify は slider の nav が JS 有効で可視・無効で非可視であることを判定）。44px の例外説明を実装（label 内 checkbox / radio、インライン文リンク）に合わせた。
   Astra 3 巡目の是正: `home_contact:none` で hero / CTA 帯 / 固定導線の `#contact` が非表示区間を指していた（none のとき `#contact` 導線を出さない）。到達先の検査をセット切替時だけでなく HP / イベントの全軸・全型（PC / SP / SP JS 無効）に広げ、イベントの共有導線のダミー `#share` / `#calendar` は自身の枠に id を付けて到達先を持たせた（`data-wt-poc-dummy` で PoC のダミーであることを明示）。
10. 未決（PO 判断待ち）: HP / イベントを要求（L1）へ昇格するか、区間セットの構成、地図の扱い（静的画像 / テキストのみ。埋め込みは外部サービスのため不採用）。 → 2026-09-06 PO 反応 18 回目（WT-EVT-0279〜0283）で回答: HP / イベントは要求漏れ（PO 決定、L1 へ入れる。候補 WT-CAND-HOME / WT-CAND-EVENT）、区間セットは外部調査で裏づける（WT-DIR-RESEARCH-SECTIONS-01）、地図の外部サービス埋め込みは PO の問い（WT-Q-EVENT-MAP-01、未決）、Facebook は不要（PO 決定）、カテゴリ面を強化（§2.26）。

## 2.26 段6 — カテゴリ面の強化（WT-EVT-0283「強化してくれ」）

PO 反応 18 回目（2026-09-06）の 5 件目「強化してくれ。その間にいまのアーティファクトを確認しておく。」を受け、台帳 `docs/research/2026-09-05-parts-pattern-taxonomy/category-recapture/summary.md`（主集計 n=54、Astra レビュー済み）の観察型をカテゴリ面に入れた。型の選定と既定値は台帳の最多型を参考にした試作候補（Claude 案。grid 43% / chips 40% / numbers 43% のように過半数でない軸もある）。

1. **軸 12（既存 5 軸の拡張 + 新規 7 軸、`?wt=cat_*`。`cat_minihome` は変更なし）**: `cat_header` name-count（既定、68%）/ name-only / name-desc / hero、`cat_lead`（新）none（既定、78%）/ lead-text / editorial、`cat_children` none / chips（既定、40%）/ cards / steps / sidebar-tree（新、15%）/ image-banners（新、2%）、`cat_columns`（新）sidebar-right（既定、69%）/ 1col、`cat_sidebar`（新）standard（既定: カテゴリ 67% + 人気 57%）/ with-cta（+ 案内バナー 33%）/ full（+ 検索 19% / 月別 20% / 新着 15% / プロフィール 13% / タグ 13%）、`cat_list` grid（既定、43%）/ text-list（新、39%）/ featured-grid / grid-2（新、6%）/ thumb-list / timeline（新、2%）、`cat_card`（新）standard（既定: 画像 59% / chip 63% / 日付 76% / 抜粋 59%）/ minimal / rich（+ 著者 / NEW / タグ 各 11%）、`cat_filter`（新）none（既定、63%）/ tabs（15%）/ year（11%）/ tag（9%）/ sort（2%）、`cat_pagination` numbers（既定、43%）/ none（新、39%）/ load-more / prev-next、`cat_ranking` none（既定）/ sidebar / bottom / top（新、13%）、`cat_pickup`（新）none（既定、52%）/ top-featured（35%）/ editor-pick-box（13%）、`cat_cta`（新）none（既定、54%）/ lp-banner（22%）/ newsletter（11%）/ line（7%）。既定は各軸の最多型（過半数とは限らない）。app-download（6%）は第三者ストアのバッジ画像が要るため置かない。`cat_ranking` の既定 none は、台帳の sidebar 44% を `cat_sidebar` の人気記事が担うため（人気は PoC では閲覧数が無く日付順、見出しに明記）。`cat_minihome` は変更なし。
2. **既定の見た目が変わる**: 右 sidebar（2 カラム）・件数付き見出し・PC の抜粋表示が既定になったため、試作 02〜03 の既存カテゴリ写真 36 枚（header 3 / children 4 / list 3 / pagination 3 / ranking 3 / minihome 2 × PC / SP）を同名で撮り直した。core の constrained layout（content-size 680px）が 2 カラムを潰していたため、カテゴリ面の直下要素は main の幅（1120px）を使う。
3. **実装**: 動的ブロック 7 つ（`category-count` / `category-lead` / `category-filter` / `category-sidebar` / `category-pickup` / `category-cta` / `card-badges`）と `category-ranking` の `slot` 属性（top / bottom / aside の 3 か所に置き、選択と一致する 1 つだけ描く）。`templates/category.html` を右 sidebar 付きに組み替え、カードに著者名（core `post-author-name`）とバッジ（NEW 30 日以内 / タグ 3 つまで）を追加。年フィルタは `?year=` 付きカテゴリ一覧を canonical redirect が年アーカイブへ飛ばすため、カテゴリ面でだけ `redirect_canonical` を止める。並べ替えは同一サイト内の一覧の並べ替え（WP 公開 query var `orderby` / `order`）なので送信する get フォーム。メール登録は送信しない（`data-wt-poc-form="no-submit"`）。LINE 導線は自作グリフ + 文言（§2.24 の lineIcon と同じ規約）。ローカル検証台には PoC 用のタグ 3 件（初心者向け / 比較 / 導入事例）をカテゴリ内 17 記事へ順に付けた（wp-load 経由、リポ外操作）。
4. **撮影 `scripts/shots-reaction12.mjs`**（reaction11 方式 + 既存 stage3 と同じ選択子で旧型を同名置換）: 新規 **54 枚**（name-count、lead 2、children 2、columns 2（全長分割: SP 1 + 1、PC 2 + 2）、sidebar 3、list 3、card 2、filter 4、pagination none、ranking top、pickup 2、cta 3 × PC / SP）+ 置換 36 枚。INDEX 634 → **688**。用語集に部品 7 件（lead / columns / sidebar / card / filter / pickup / cta）と既存 5 部品の型・割合を追記。
...
## 2.28 段8 — HP / イベントのバリエーション拡張と固定ページ用パーツ（WT-EVT-0287「homeとイベントはもっとバリエーションを出して。調査不足じゃない？…固定ページ継投で使えるパーツをしっかりと作りこむこと。記事パーツからの転用でも可。リサーチはアストラにも一緒に見てもらうこと。」）

1. **外部調査の拡張（台帳 `../2026-09-05-parts-pattern-taxonomy/home-event-recapture-v2/`）**: 前回（HP 39 / イベント主集計 8）とは独立に再収集。HP 76 件（取得 71、前回と同一 URL の 9 件を `prev_dup` で除外 → **集計対象 62**）、イベント 54 件（取得 46、page_kind 除外 6 → **主集計 40**）。summary は `gen_summary.py` で JSON から全生成。Astra レビュー 3 巡（1 巡目「不可」: 終了ページの採用根拠・欠測分母・語彙の異表記・応募方法の複数選択・other:* の方針・未観察提案の分離・前回比較の検証不能 → 2 巡目「不可」: 出店者向け募集ページの混入（E11）・採用規則の統一・§6 の分母・区分別 apply の打ち切り・gallery の根拠・規則例外の明示 → 3 巡目「**可**」、改善 1・軽微 1 は反映済み）。1 巡目で「前回と重複なし」と書いたのは誤りで、対応表の URL 照合で 9 件の同一 URL が見つかり除外した。
2. **観察の要点（観察事実）**: HP hero は fullbleed 34% / slider 28% / cards-carousel 11% / product-shot 7%（欠測除外 n=61）で前回の最多型 text-only と一致しないため、暫定既定値は据え置き。先頭セクションは banner-row / news / greeting。区分別上位区間: A 企業 = contact-band / news / service-cards / features / greeting、B サービス = features / contact-band / service-cards / numbers / cases / banner-row / download / logos、D 店舗・スクール = service-cards / contact-band / features / news / banner-row / courses / faq / recruit、E 学校・団体 = contact-band / events / features / news / courses / sns-feed。イベント主集計は hero date-place-block 62%、info icon-list 55%、apply（複数選択）external-form 45% / closed-notice 32% / ticket 15% / postcard 8% / receipt-upload 8% / messaging-app 5%、target-audience（45%、A 69%）・organizer（85%）・tickets（32%、B 75%）・prizes / entry-steps / target-products（12 / 12 / 10%、D 62 / 62 / 50%）。HP の greeting は 40%（A 65%）、logos 15%、sns-feed 13%、events 11%、stores 10%。
3. **固定ページ用パーツ（`theme/helix-wt/patterns/page-*.php`、pattern category `helix-wt-page`「HELIX WT 固定ページ用パーツ」）**: greeting / logos-row / stores / event-list / gallery / sns-feed / timeline / countdown / target-audience / organizer / tickets / prizes / entry-steps / judges / target-products の 15 個。既存の features / numbers / cases / faq / steps / pricing / cta-banner / cta-box / product-bundle も同 category に再登録（「記事パーツからの転用」）。各パーツは alignfull の group + `.wt-lp-section-inner`（1120px）で、本文幅 680px の通常固定ページ（page.html）でも全幅で出る。証跡ページ `/parts/`（slug parts）は verify が wp-cli で作る / 更新する（冪等）。gallery と countdown は台帳で未観察の追加提案（Claude 案）と明記。sns-feed は地図埋め込みと同じ方針（URL は option `helix_wt_sns_feed_embed_url`、http(s) のみ、未設定なら外部へ接続しない、遅延 iframe）。
4. **HP**: hero +3（cards-carousel / product-shot / search-box）、hero CTA +1（search。form ではなく role=search の div で暗黙送信なし）、区間セット +2（shop-school / school-org。既存 3 セットの構成は据え置き）、問い合わせ帯 +1（double-cta）、区間 +8（greeting / logos / stores / events / gallery / sns / history / recruit。recruit 以外は固定ページ用パーツを `wp:pattern` で転用）。カルーセルの class は既存の関連記事 carousel（`.wt-carousel__nav` を body class で隠す規則）と衝突したため `.wt-hcar__*` に分離。
5. **イベント**: 区間セット軸 `event_sections`（seminar 既定 = 従来 11 区間の構成を据え置き / seminar-v2 = v2 の A 上位区間案（対象者・主催を加え sponsors・past を外す。PO 判断待ち）/ conference / festival / campaign。B・C は小標本のため「選べる型」）を新設し、区間は既定で非表示・セットで表示と順序を決める（軸 none はセットより優先）。申込 +3（receipt-upload / postcard / messaging-app。台帳の other:* 3 語を語彙へ正式採用する提案）。区間 +8（対象者 / 主催 / 賞品 / 対象商品 / 応募の流れ / 審査員 / ギャラリー / カウントダウン。固定ページ用パーツの転用）。注意事項区間の見出しから「主催」を外した（主催は独立区間）。
6. **verify**: `homeFace` 28 型 × 3 + 区間セット 5 × 3 + hero × セットの交差 45 × 3 = 234 行（区間内のパーツが可視・h2 あり・可視画像に読込失敗なし、hero 内の # 導線がどのセットでも可視の区間を指す）、`eventFace` 42 型 × 3 = 126 行 + 設定状態（全行で区間の集合と順序を ESETS と照合、パーツ区間の見出し・画像）、新規 `pageParts`（/parts/ を PC / SP / SP JS 無効で: 15 パーツが順に可視・h2・全幅（内側 ≤ 1120px かつ viewport − 48px 以上）・画像読込失敗なし・form なし・検索欄 1・44px・#導線の到達先可視・外部 http(s) 要求なし・カウントダウンは JS で数字 / JS 無効で "--" と開催日・SNS フィード未設定、option 設定で同一ホストの遅延 iframe → 削除して未設定へ戻る cleanupOk。さらに登録済みパターンの展開後 markup を固定ページ /parts-expanded/ に保存し（wp-cli `eval` で registry から取得、動的ブロック `helix-wt/sns-feed-embed` を含み静的な iframe を含まないことを検査）、保存後も option の設定 / 削除に追従することを確認）。
7. **撮影 `scripts/shots-reaction13.mjs`**（reaction12 方式）: HP hero 3 + CTA search + double-cta、イベント申込 3、パーツ 15（/parts/ で単体）× SP / PC、全長分割: HP 区間セット 5 種（corporate / service は同名置換、旧チャンクは退避）、イベント区間セット 5 種（旧 event-page-full-* は退避）、パーツ一覧ページ。分割系列（home-sections / event-sections / page-parts-full / event-page-full）はすべて旧チャンク除去の対象。
8. `style.css` / スクリプト版 0.3.14 → 0.3.15。用語集に hero 3 型・CTA search・区間セット 2 種・double-cta・event-sections 5 種・申込 3 型・page-part 15 種・page-parts・面 page を追加。
9. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 84 / fail 0**（83 + `pageParts`、skipped なし）、総合 `pass: true`。`homeFace` 234 行、`eventFace` 126 行 + embedSet、`pageParts` 3 行 + feedSet（展開保存ページ含む、cleanupOk）。INDEX 690 → **783**（main 比でファイル追加 99・削除 6（旧 event-page-full）、同名置換あり）。初回実行で落ちた 3 件の是正: (a) 検索 CTA の内部レイアウト規則に display を含めたため軸 class なしでも表示され、hero の文字位置も下がって fullbleed / slider の SP コントラスト（α 補間）が落ちた → display は軸 class の規則だけに持たせる、(b) verify の期待順序が既定軸 none（schedule / speakers / map）で隠れる区間を含んでいた → 当該軸の値と既定値から除外して照合、(c) カウントダウンの JS 無効時の表示 `--` が WP の texturize でダッシュに変換された → 文字参照 `&#45;&#45;` で書く。撮影で沿革の年が別カラムに落ちた（grid の自動配置）→ time / div / ::before の列を明示。
...
2. **観察の要点（サイドバー、n=47）**: 配置 right 85% / both 10% / left 2%。追尾（欠測除外 n=42）toc-only 43% / last-widget 40% / none 12% / whole 5%。ウィジェットは popular-ranking 87% / categories 79% / search 70% / new-posts 60% / cta-banner 57% / related-posts 45% / toc-sticky 40% / ad 38% / tags 32% / profile 21%、記録リストの先頭は search 68%。目次をサイドで追尾する型は 40%。ウィジェット数の中央値 6。サイドナビは mega-menu 35% / none 35% / toc-side 23%。SP の扱いは要約で判定できず。
3. **実装**: 軸 `side_layout`（none 既定 / right / left / both）、`side_sticky`（none / whole / last-widget 既定 / toc-only）、`side_sp`（below-content 既定 / drawer / hidden）、`side_set`（media 既定 / blog / owned / corporate / minimal / full）、`side_nav`（none 既定 / mega-menu / fixed-left-nav / fixed-right-icons / drawer-pc / toc-side）。動的ブロック `helix-wt/sidebar`（ウィジェット 20 種 = 台帳の観察 18 種（other:toc-dropdown = 折りたたみ目次を含む）+ 未観察 2 種（calendar / tel-box、Claude 案）を `wt_side_widget()` で描画。目次ウィジェットは本文と同じ id 付与 helper `wt_toc_assign_ids` / `wt_toc_items` を使う）と `helix-wt/side-nav`。single / page / front-page の 3 テンプレートを `.wt-side-layout`（grid）で包み、`side.js`（ドロワー: 右サイドバーの複製、id 接頭辞、フォーカス移動、Escape / 背景クリックで閉じる。メガメニュー: ナビに trigger を挿入、aria-expanded、Escape / 外側クリック）。JS 無効: ドロワーは本文下に表示、メガメニューは出ない。検索ウィジェットだけ実フォーム（GET、同一ホスト）、ニュースレターは form を使わない。広告枠はダミー。
...
11. Astra 1 巡目（PR #166）「merge 不可」の是正: 【重大】追尾が機能しない（grid の align-items:start で aside が内容高に縮み、last-widget / toc-only の sticky が動く余地が無い）→ aside を align-self:stretch で本文の高さまで伸ばし、verify は追尾対象の本来の位置 + 400px（最低 1500px）までスクロールした前後の矩形で「追尾対象が上端（header 高 + 16px ± 2）に留まり先頭ウィジェットは画面外、aside 高 ≥ 本文高 × 0.95」を判定。【改善】ドロワー複製の参照（for / aria-labelledby / aria-describedby / # リンク）が元要素を指す → id の対応表で書き換え。aria-modal のフォーカス封じ込めが無い → ドロワー以外の body 直下を inert にし、閉じたら戻す（verify は Shift+Tab 後もフォーカスがドロワー内、閉じた後に開くボタンへ復帰）。verify がドロワーの中身を検査していない → 開いたパネル内で順序・フォーム・44px・参照先・# 導線を検査、メガメニューはリンクの可視数。台帳の最終値と README / 用語集 / コメントの不一致 → 同期。観察の other:toc-dropdown が未実装 → toc-dropdown ウィジェットを追加（20 種）。【軽微】台帳 §3a の「試作 03 の現状」→「段 10 着手前」と明記。Astra 2 巡目: ドロワー内の目次リンクを押しても閉じない → # リンクの対象がドロワー外なら閉じてから移動（inert 解除）。ドロワー検証が media セットだけ → SP に drawer × full セットの行（newsletter / toc-dropdown を含む複製の参照先を実測、記事では目次リンクのクリック → 閉じる・inert 解除・hash・見出しが viewport 内）を 3 面分追加（210 行）。Astra 3 巡目: 参照の検査を属性別・ウィジェット別に（search の label for、newsletter の label for と aria-describedby、各ウィジェットの aria-labelledby がパネル内に実在）。
...
## 2.31 段10b — サイドバーの既定を right へ、HP は hero の下から（PO 反応 22 回目 WT-EVT-0292「サイドバーは普通置く。スマホは規定値オフになるが、ＰＣでないほうが珍しい。そしてＣＶ狙うなら左に置くって選択肢もある。サイドバーにフロートバナーなどを置けるから置かないほうが機会損失になりやすい。ＨＰなどの固定ページ側もファーストｖｉｅｗ以下にサイドバーを設けることも多いはず。サイドバーを置かないケースはもはやＬＰぐらいしかなくないか？」）

1. **§2.30 第 10 項の未決 1 への PO 回答**として反映。`side_layout` の既定を none → **right**（記事・固定ページ・HP。台帳の記事 right 92% とも一致）。SP の扱いは PO 反応 24 回目 WT-EVT-0294「選択型で」により軸 `side_sp`（below-content / drawer / hidden）として選択可能に保つ。SP の既定値は PO 未指定のため below-content（1 カラムで本文の下）は **Claude 案の暫定値**（Astra 1 巡目【重大】の指摘: 「規定値オフ」を本文下表示と読み替えて PO 決定にしない）。left は既存の軸値のまま（PO「CV 狙うなら左」）。LP は共通サイドバーを持たない（変更なし）。カテゴリ面は独自の `cat_sidebar` のまま（未決 2 は継続）。
2. **HP の開始位置 `side_from`**（新軸、HP のみ）: `below-hero`（既定。hero = first view を端まで全幅で見せ、その下から本文列 + サイドバー）/ `top`（hero を本文列に入れ、サイドバーは hero の横から本文末尾まで貫く）。記事は見出し部（パンくず・題・アイキャッチ）の下、固定ページは題の下から始まる既存構造で固定。
3. **実装**: `patterns/home.php` を `home-hero.php`（hero 9 型 + article-grid を包む group `.wt-home-hero-slot`、anchor `home-hero`）と `home-sections.php`（区間・固定導線）に分割し、`home.php` はその合成に。`templates/front-page.html` は `.wt-side-layout` の直下に hero → `.wt-side-main`（sections）→ sidebar を置く。CSS は body.wt-side-from-* × 配置 3 型で grid の行 / 列を明示（below-hero: hero `grid-column:1/-1` + 負の margin で端まで、top: hero を本文列、aside を `grid-row:1/span 2`）。`functions.php` の既定値と用語集（side-layout の既定文、side-from）を更新。
4. **verify `sideDefaults`**（新設）: `?wt` なしで記事 / 固定ページ / HP に body.wt-side-layout-right と右 aside（PC は 2 トラック、SP は 1 トラックで本文の下）、LP とカテゴリ面には `.wt-side-layout` も aside も無いこと、HP の side_from 2 型 × 配置 4 型 × PC / SP で hero と aside の矩形関係（below-hero: hero の left = 0 / right = viewport、aside と本文は hero の下。top: hero の左右 = 本文列、aside の上端 = hero の上端 ± 20px（aside の内側余白 1rem）、aside の下端 ≥ 本文の下端、左右の実配置）。26 行。
5. **撮影 `scripts/shots-reaction15.mjs`**: 既定が変わって写真が変わるものを同名で撮り直し（記事全長 article-full / 画面単位 article-screen-NN / 幅 width-* / HP 区間セット 5 種 / 固定ページ一覧 / HP owned セット。分割数・画面数が変わった系列は旧末尾を退避）。部品単体の要素撮影は本文列幅が同じ（`--wt-content-max`）ため撮り直さない。新規: side-from 3（below-hero / top / top + left、PC）、記事・固定ページの既定 first view 2（PC）、記事 SP の画面単位 13〜16 枚目（11・12 枚目は既存の同名置換）。
6. `style.css` / スクリプト版 0.3.16 → 0.3.17。
7. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 86 / fail 0**（85 + `sideDefaults`、skipped なし）、総合 `pass: true`。`sideDefaults` 26 行、`sideFace` 210 行。INDEX 851 → **860**（撮り直し 74 = 同名置換（記事 SP の画面単位 11・12 枚目は 851 に既存のため撮り直しに含む）、新規 9 = side-from 3 + 既定 first view 2 + 記事 SP の画面単位 13〜16 枚目 4。Astra 1 巡目【重大】: 旧 reaction1 の 10 画面上限で SP の末尾（本文末・サイドバー・フッター）が欠けていた → 上限を撤廃し全高から枚数を決める（SP 16 / PC 10）。分割数の変化による退避 0）。初回の実行で落ちた 3 検査は既定にサイドバーが出たことによる検査側の前提: `homeFace` が共通サイドバーの検索フォーム（GET・home_url）を HP のフォーム（非送信・`#`）として数えていた → `.wt-side` 内のフォームを除外（サイドバーの検索は sideFace / sideDefaults で検査）。`pageParts` が各パーツの幅 = viewport を期待 → 本文列（wt-side-main）の幅に（SP は 1 カラムで viewport と同じ）。`sideFace` の HP × full セットで aside が本文と同じ高さになり追尾の余地が無く、末尾ウィジェットが aside の末尾（margin-bottom 分上）に押し上げられる → その状態を正として判定に追加。あわせて SP では `.wt-side-layout` の左右余白をやめ（本文列は段 10 以前と同じく端まで）、aside 側に余白を持たせた。
8. 用語集とカタログの是正（PO 反応 23 回目 WT-EVT-0293「ヘッダーはそもそも共通では？」「記事を先頭に置く考え方をしているから共通であるべきものがかなり記事で表示されている」）: parts.header の label「記事ヘッダー」→「ヘッダー（全面共通）」。カタログ（統合層 scratch の build）は「共通（全面）」を先頭の面にし、ヘッダー / フッター / 幅 / 密度・奥行き・動き・文字の軸 / コントラスト保護 / サイドバー・サイドナビを撮影面に関わらずそこへ集約（型名に撮影面を添える。LP は専用 header / footer を持つので LP の項に残す）。並びは 共通 → HP → 記事 → カテゴリ → 固定ページ → イベント → LP → 404。INDEX の face（撮影面）は変えない。カタログ側は PC / SP 切替で片方の端末しか無い型（SP だけの固定導線・SP の分割写真）が空カードに見えた指摘に対し、切替時はカードごと隠すよう build を変更。
9. Astra 1 巡目（PR #167、head d3c3ba0）「merge 不可」の是正: 【重大】SP 既定の解釈（→ 第 1 項、WT-EVT-0294）。【重大】記事 SP の画面単位撮影が 10 枚上限で末尾を欠く（→ 第 7 項）。【改善】追尾検査の末尾固定（pinnedAtEnd）は aside が縮んだ退行も通しうる → aside の内容高 ≥ 本文高、かつ対象の下に 400px の余地が無いときだけ認める。
10. Astra 2 巡目（head 85cf74d）「merge 不可」の是正: 【改善】撮影の内訳（撮り直し 72 / 新規 11 → 74 / 9。SP 11・12 枚目は既存）。【改善】WT-EVT-0295 の PO 原文に第三者サービス名 → 伏せ字〔 〕にし source_ref に明記（完全版 WT-EVT-0296 も同様）。
11. Astra 3 巡目（head 91d54f2）「merge 可」・軽微 2 件の是正: 第 5 項の「11〜16 枚目」→「13〜16 枚目」、第 9 / 10 項の順序。


## 2.32 段10c — サイドバーの所属を面ごとに（PO 階層整理 WT-EVT-0296、指示 WT-EVT-0297「合いませんではなく、合わせろ。」）

1. **入力**: PO の Web サイト階層整理（後続で PO が共有した 9 系統の分類提案と PO 決定 WT-EVT-0298 / 0299 も参照: 分類の骨格は採用、段階化を理由に要求範囲を縮めない、段 10c は現在の対象で閉じてよいが「継承しない = 非表示」にはしない）（①HOME ②カテゴリ ③イベント ④記事 ⑤有料記事 ⑥インタビュー ⑦LP / BLP、WT-EVT-0296。原文中の第三者サービス名は伏せ字）。サイドバーの所属は「HOME = 共通になる場合とならない場合があるから ON/OFF 含めた個別設定」「カテゴリ = 記事側を継承するケースが多い」「イベント = LP に近い場合は不要 / 主 CV 重視は HP 側を継承 / 回遊導線型は記事側を継承」「LP / BLP = HP・記事の設定を引き継がない」。Claude の「今の試作と合わない」に対し PO「合わせろ」（WT-EVT-0297）。
2. **設計**: 設定の束を 2 つに分ける。記事側の束 = 既存 `side_layout / side_sticky / side_sp / side_set / side_nav`、HP 側の束 = 新設 `home_side_layout / home_side_sticky / home_side_sp / home_side_set / home_side_nav`（HP 側の既定セットは corporate）。束の台帳 `wt_side_bundles()`（名前 → 軸接頭辞）に登録し、後続の継承セット追加はここに 1 行 + 軸 5 本を足す。PO 決定 WT-EVT-0299「継承しない = 非表示にしない。継承・独自設定・非表示を分離」により独自設定の束 `own_<face>_side_*`（面専用。既定セット minimal、Claude 暫定）を追加。面ごとに束を選ぶ軸: `cat_side`（article 既定 / home / own = 独自設定 / classic = 段 6 の独自 aside / off = 非表示）、`event_side`（off 既定 / home / article / own）、`page_side`（home 既定 / article / own / off）。HOME は HP 側の束固定で `home_side_layout:none` が OFF、記事は記事側の束固定、LP は持たない。`side_from`（hero の下から）は HP の束に付随。
3. **既定の根拠**: カテゴリ = 記事側継承、イベント = OFF は PO 原文由来。HOME の ON、固定ページ = HP 側、HP 側セット corporate は原文に無い **Claude 案の暫定値**。
4. **実装**: `functions.php` に `wt_side_face()`（lp / home / event / category / page / article）、`wt_side_bundle()`（home / article / null）、`wt_side_eff($k)`。body class の `wt-side-*` は束の実効値（CSS / JS は段 10 のまま）。生の軸値が実効値と違うときは `wt-raw-side-*` に残す。面 class `wt-face-*` と `wt-side-bundle-*` を追加。カテゴリ template に共通サイドバー / サイドナビのブロックを置き（primary に `wt-side-main`）、CSS で cat_side=article は `.wt-cat-layout` の 2 列目に共通サイドバー、classic は段 6 の aside だけ、off は 1 カラム。イベント template は固定ページと同じ包み（面全体を本文列に入れる。hero を全幅に残す side_from はイベントでは使わない = 制約）。side.js をカテゴリ面でも読み込む。
5. **verify**: `sideOwner` 新設。束の台帳を `<meta name="wt-side-bundles">` から読み、面 × 束の行を生成する（カテゴリ = 束 3 + classic / off、イベント = 束 3 + off、固定ページ = 束 3 + off）+ 非表示 × サイドナビ / ドロワー 4 + own の面別分離 3（自面の own_<face>_side_set だけを見る）+ HOME OFF 2（配置のみ OFF、HP 側のサイドナビは生きる）+ 束の分離 2（HP で side_set を変えても変わらない、記事で home_side_set / own_page_side_set を変えても変わらない）+ 継承した束の SP ドロワー 2（カテゴリ / イベント。PC は aside、SP JS ありはボタンだけ、JS 無効は本文の下）+ LP に無し 1 = 27 ケース × PC / SP + カテゴリ / イベントの SP JS 無効 16 = 70 行（Astra 1〜2 巡目後。初版は 36 行、項目 12〜13）。`sideFace` / `sideDefaults` は HP を `home_side_*`、固定ページを `page_side:article` で記事側の束に当てる（`sideQ`）。`categoryVariants` は段 6 の独自 aside を `cat_side:classic` で検査（初回実行で前行 load-more の残り処理が次の遷移を中断 ERR_ABORTED → 1 回再試行）。
6. **撮影 `scripts/shots-reaction16.mjs`**: 段 6 の独自 aside を写す写真（category-columns 全長 / category-sidebar 3 型 / children sidebar-tree / ranking sidebar）を `cat_side:classic` 明示で同名置換。HP・固定ページは HP 側の束（corporate）に変わるため home-sections 5 種 / page-parts-full / side-home-owned（`home_side_set:owned`）/ side-page-corporate（`page_side:home,home_side_set:corporate`）/ side-from 3 / side-default-page を撮り直し。新規: cat-side 5（PC / SP）、event-side 4、page-side 4、HOME OFF（PC）。
7. `style.css` / スクリプト版 0.3.17 → 0.3.18。用語集 side-owner、category-sidebar に cat_side の注記。
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 87 / fail 0**（86 + `sideOwner`、skipped なし）、総合 `pass: true`。`sideFace` 210 行 / `sideDefaults` 26 行 / `sideOwner` 70 行（Astra 1〜2 巡目後。初版 36 行）。INDEX 860 → **879**（reaction16: 93 撮影、新規 19 = cat-side 5 型 × SP / PC の 10 + event-side 4 型 PC + page-side 4 型 PC + home-side-off PC、同名置換 = 段 6 の classic 写真（category-columns / sidebar / children sidebar-tree / ranking sidebar）と HP / 固定ページの束切替後の再撮影（side-from 3 / side-default-page を含む）、stale 0）。初回 verify の不一致 3 件は項目 11。
13. Astra 2 巡目（head 60cdcad、重大 0・改善 1・軽微 2、merge 可）の是正: (改善) 継承した束に `side_sp:drawer` を当てた SP JS 無効の行が無かった → カテゴリ / イベントに article+drawer を追加（PC = aside、SP JS あり = ボタンだけ、SP JS 無効 = 本文の下に aside。64 → 70 行）。(軽微) reaction16 冒頭と CAT_COLUMNS のコメントが `cat_side:own` のままだった（classic へ）、CSS コメントの `own_side_*`（面別接頭辞へ）。
12. Astra 1 巡目（head a7f8c12、重大 2・改善 2・軽微 1、merge 不可）の是正: (重大 1) 束なし（off / classic / LP）で `layout` だけ none にしていたため `side_nav:drawer-pc` / `fixed-right-icons` / `side_sp:drawer` を当てると記事側のセットがドロワーやサイドナビから出た → `wt_side_eff` は束なしのとき nav / sticky / sp も切る（非表示 = 配置・ナビ・ドロワーすべて）。(重大 2) `own_side_*` が全面共通で「第 3 の共通設定」になっていた → 面ごとの軸 `own_category_side_* / own_event_side_* / own_page_side_*`（台帳の own は接頭辞 `own_{face}_side_` で面名に置換）。(改善 1) 所属軸の許可値が固定で台帳に束を足しても選べず、CSS も束名を列挙していた → 許可値は `array_keys(wt_side_bundles())` から導出、CSS は `:not(classic):not(off)`。台帳を `<meta name="wt-side-bundles">` で出し verify が読む。(改善 2) `sideOwner` を台帳駆動に書き換え（束 × 面の行を台帳から生成 = 束を足せば行が増える）、非表示 × サイドナビ / ドロワー（4 行）、面をまたぐ own の分離（3 行）、HOME OFF でも HP 側のサイドナビは生きる（1 行）、カテゴリ / イベントの SP JS 無効（14 行）を追加。(軽微) verify / functions.php の旧コメント。
10. 前提（PO 指示 WT-EVT-0300、2026-09-07）: 「WP-THEME では選択肢と対応可能範囲を最大化し、GRAPHIX-NEO では目的に合わせて選択・構成・制約する」。9 系統の分類は対応範囲を閉じる製品分類ではなく検証の抜け漏れを探す索引、5 継承セットも固定上限ではない。本段の束の台帳（`wt_side_bundles()`）と面ごとの継承 / 独自設定 / 非表示の分離はこの前提に沿う設計で、未検証の面（有料記事・インタビュー・BLP・学習支援・会員手続き）は「未観察 = 調査対象」として次段の種別台帳へ持ち越し、対応済みとは扱わない。
...
3. 実装: `theme/helix-wt/inc/form.php`（functions.php から require）。動的ブロック `helix-wt/form`（固定ページ /contact/ に alignfull の group で置く）と `helix-wt/form-thanks`（/thanks/）。軸 11 本: `form_kind`（9 種別）/ `form_fields`（by-kind / minimal / standard / full）/ `form_required`（asterisk / label）/ `form_layout`（1col / 2col / label-left / placeholder-only / steps）/ `form_confirm`（yes / no / inline-review）/ `form_consent`（checkbox / link-only / in-submit）/ `form_submit`（auto + 8 文言）/ `form_error`（inline / top-summary / both）/ `form_captcha`（none / question / external-slot）/ `form_side`（tel / none / email / chat / messaging-app）/ `form_thanks`（separate / inline）。既定は各項目の観察最多型（項目下エラーは観察 n=3 の少数派だが Claude 暫定で既定。top-summary 2 件は型として持つ。`form_submit:auto` の「入力内容を確認する」も台帳の最多文言ではない Claude 暫定）。項目は基本 47 種 + 同意 2 種 + captcha の 50 種を定義し、台帳 §2 の観察トークン 47 種（other:* を含む）を全部含む（対応表 = `inc/form.php` の `obs`。`form_fields:full` で基本 47 + captcha + 同意 = 49 行）。
...
6. verify `formFace` 新設: /contact/ と /thanks/ を wp-cli で作る（冪等）。11 軸の全値（既定は contact 行で兼ねる）× PC / SP / SP JS 無効 = 35 × 3 行（項目の並び、必須印、レイアウトの列数・ラベルの可視性・placeholder、同意、送信文言、captcha、代替導線、添付が name を持たず multipart でない、電話が発信リンクでない、空送信のエラー = 必須項目だけ + aria-invalid・出し方・aria-describedby の参照先とまとめのリンク先が実在・フォーカス先、44px、h1、POST・同一ホスト・nonce・honeypot、外部要求なし）+ 形式検査 3 行 + nonce 改ざん / honeypot 入力 6 行 + 遷移 confirm 3 × thanks 2 × JS あり / なし = 18 行 + steps の段階送り（contact 3 段 / newsletter 2 段 / reservation 3 段）6 行 + 網羅性 1 行（台帳 observations.json の本体観察 fields トークンを verify 側の対応表で引き、`form_fields:full` に全部あること）+ 異常 POST（単値の欄を配列で送る / yes-no を別の質問キーで送る / select の選択肢外 / yes-no に余分な質問キーを足す）12 行 + top-summary × diagnosis / reservation（ラジオ・希望日・yes-no のまとめリンク先）6 行 + メールの規則（JS / サーバで同じ許可・拒否 6 例）3 行 = 160 行（件数は固定値で照合）。
...
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 88 / fail 0**（87 + `formFace`、skipped なし）、総合 `pass: true`。`formFace` 160 行（Astra 1〜3 巡目後。初版 122 行 = 軸 33 × 3 + 形式検査 3 + 遷移 18 + steps 2）全 pass。INDEX 879 → **931**（reaction17: 52 枚すべて新規、stale 0）。
...
11. Astra 1 巡目（head 7ef4a54、重大 2・改善 5・軽微 2、merge 不可）の是正: (重大 1) 添付が name 付き + multipart で送られていた → name を外し multipart にしない（ブラウザが送らない。確認画面も hidden を作らない）。(重大 2) 台帳 §2 の観察トークン（役職・性別・生年月日・学年・学校名・学部・会社かな・注文番号 …）が項目に無く、§5 の文言「送信」「確認する」も無かった → 項目 50 種（基本 47）+ 対応表 `obs`、文言 2 種追加、verify に台帳から独立した網羅性検査。(改善 1) JS とサーバの検証差 → 第 1 希望のみ必須で統一、JS のメール正規表現を `is_email` 相当に、配列 / 単値の形が違う異常 POST はエラーへ、select / radio は選択肢外を拒否。(改善 2) top-summary で `aria-describedby` が無い要素を指していた・まとめのリンク先がラジオ / 希望日 / yes-no で実在しなかった・yes-no の必須が最初の質問だけ → 参照先はまとめ（`#wt-form-summary`）、リンク先は実在する入力 id（`-1` / `-0` / `-0-y`）、必須は各質問。(改善 3) steps × 種別で段が固定 3 個だった → 存在する段だけ出し `data-wt-step-i` で対応、段ごとに項目をまとめる（種別の並びで段 1 の項目が後ろにあっても fieldset が分かれない）。(改善 4) verify の網羅性: 上記の再現ケース（steps × newsletter / reservation、top-summary の属性・リンク先、nonce 改ざん・honeypot、添付の構造）を追加し、件数は固定値。(改善 5) 電話の代替導線が発信リンクだった → 表示モック（span）。(軽微) 完了 inline は未観察の Claude 案と明記、`form_submit:auto` の文言は Claude 暫定と明記、用語集の第三者製品名を「メッセージアプリ」へ。
12. Astra 2 巡目（head eeade46、改善 3・軽微 1、merge 不可）の是正: (改善 1) JS のメール正規表現が `is_email()` と食い違っていた（`o'hara@` を拒否、`a@-example.com` を許可）→ `is_email` と同じ手順（6 文字以上、@ 1 つ、ローカル部の許可文字、ドメインは `..` なし・ラベル 2 つ以上・ハイフン始まり / 終わりなし）を JS に実装。(改善 2) yes-no の検証が非空要素の件数だけで、別の質問キー（3,4,5）でも通った → 質問キー 0..n-1 それぞれに「はい / いいえ」を要求、複数選択（checks）も選択肢外を拒否。(改善 3) 異常 POST の再現ケース（配列型 / 質問キー / 選択肢外、`form.submit()` で JS 検証を通さず送る）と、ラジオ・希望日・yes-no を持つ種別（diagnosis / reservation）の top-summary リンク先検査を追加（139 → 154 行）。(軽微) 観察トークンは 47 種、定義は基本 47 + 同意 2 + captcha = 50、full の行数 49 に数値を統一。
13. Astra 3 巡目（head 29a43c6、改善 2・軽微 1、merge 不可）の是正: (改善 1) JS の `isEmail()` がドメインの先頭 / 末尾の `.` を除去してから判定していた（`a@.example.com` / `a@example.com.` を許可、`is_email()` は拒否）→ 除去せず拒否。verify に「メールの規則」行（`o'hara@example.com` 許可、`a@-example.com` / `a@.example.com` / `a@example.com.` / `a@example..com` / `a@example` 拒否、JS あり = JS 検証・JS 無効 = サーバ検証で同じ結果）を追加。(改善 2) yes-no は必須キー 0..n-1 だけを見ていたため余分なキー（`[3]=不正な値`）が通り確認画面の hidden にも引き継がれた → キー集合が質問集合と一致しなければ拒否、verify に異常 POST「question-extra」を追加。(軽微) PR 本文の概要の 139 行を実結果に合わせた。verify `formFace` は 160 行（固定値）。
...
3. 撮影 `scripts/shots-reaction6.mjs`（reaction5 と同じ一時 dir → 予定集合との完全一致 → 退避 → 配置 → 失敗時復元 → INDEX を一時ファイル経由で置換）: 同名置換 26 枚（`cta-product-{sp,pc}`、`article-full-{sp,pc}`、`article-screen-*`。商品カード束は記事本文にもあるため）+ 新規 4 枚（`box-qa-modal-{sp,pc}` 閉状態の要素撮影、`box-qa-modal-open-{sp,pc}` ボタン押下後の viewport 撮影。dialog は top layer なので要素撮影では背景の暗転が入らない）。撮影前に catalog / 記事の両面で商品カードの存在と PR バッジ 0 を検査。`CATALOG-INDEX.json` 443 → **447**。用語集に `qa-modal` / `qa-modal open` を追加。
   - 撮影環境の実測: SP（DPR2）の全長撮影で Chromium が `Unable to capture screenshot` を返した（記事 10758 css px → 21516 device px、LP 8788 css px も失敗、カテゴリ 2003 css px は成功。main のテーマでも同じ）。`scale: "css"` は一度成功した後に同じ条件で失敗し不安定だった。全長画像は JPEG 化で長辺 1600 に縮むため、SP 全長だけ `deviceScaleFactor: 1` の別 context（幅 390・mobile エミュレーションは同じ）で撮るようにした。縮小後の情報量は同等だが、`article-full-sp.jpg` のバイト列は変わる。
...
1. **データグラフ 4 型（WT-EVT-0257、疑問形の示唆 → Claude 案として実装）**: 台帳（parts-pattern-taxonomy）にグラフの観察はなく（参照サイトの棒グラフ「アイコン」1 件のみ）、要求にも無い。比較媒体の本文で数値を見せる用途として Claude 案 4 型を置いた: `graph-bar`（横棒比較。ラベル・棒・数値の 3 列）、`graph-stack`（100% 積み上げの割合バー + 凡例）、`graph-donut`（conic-gradient のドーナツ + 中央主値 + 凡例）、`graph-line`（inline SVG の折れ線、目盛り・月ラベル）。すべて JS なし（CSS / SVG）。値の持ち方は型ごとに異なる: bar / stack は各行・区分の `--v`（%）、donut は `--a` / `--b`（累積 %）、line は SVG の座標（`points` / `circle`）。図の種類は `data-wt-graph`。系列色は accent / ok / cta の 3 色。各 figure に figcaption と同じ値の表（`table.wt-graph__data`、`screen-reader-text`）を併置し、ドーナツ・割合バー・折れ線は `role="img"` + `aria-label` に値を書く。パターン `helix-wt/graphs`（bar）・`graph-stack`・`graph-donut`・`graph-line`、catalog `#cat-graph-{bar,stack,donut,line}`。
   - `verify.mjs` `graphs`: 4 型が各 1 つ、figcaption 非空、**視覚化の値と表の値の一致**（bar / stack: 各 `--v` = 表の各行、stack は合計 100。donut: `--a`, `--b`−`--a`, 100−`--b` = 表の 3 行。line: 折れ線の点数 = 点の数 = 表の行数）、stack / donut / line の `aria-label` に表の全ラベルと全値が含まれること、系列色と面（base）/ トラック（surface）の非テキストコントラスト 3:1 以上（実測 accent 6.70 / 6.19、ok 5.02 / 4.63、cta 5.18 / 4.78）、JS 無効時に同じ内容（ラベル・値・本文テキスト）が可視で描かれること、SP 幅 390 以内、PC でも可視。Astra レビュー 1 巡目の是正: 旧判定は `--v` を持つ要素だけを集めて `every()` していたため値が全て無くても合格し、JS 無効時の可視性も見ていなかった。
   - donut の凡例は PC で右、SP で下（`@media` で縦積み）。
   - Astra レビュー 2 巡目の是正: line は目盛り（`.wt-graph__ticks`）から y 座標を値に戻して表と突合（誤差 0.05）し、点（circle）が折れ線の頂点と一致すること／可視性の祖先走査を `html` まで（`body` 含む）に拡張し、コンテナだけでなく棒の塗り・区分・ドーナツ・折れ線・点そのものの可視と大きさを要求／aria-label は「説明: ラベル 値、ラベル 値、…」の形式として区分ごとに表の行（順序どおり）の「ラベル 値」へ完全一致することを要求（3 巡目: 数値列だけの照合・ラベル位置非依存の除去をやめ、符号・接尾辞・入れ替えを拒否）。
...
3. **metrics の SP 配置（WT-EVT-0259）**: `is-style-wt-detext-metrics` は SP で 1 列に落としていたため 3 指標が縦に並び間延びしていた。Claude 案として SP でも 3 列のまま並べ、数字を `clamp(1.5rem,7.5vw,2rem)` に縮め、指標間に縦の区切り線を入れた（`@media (max-width:599px)` の上書き）。`verify.mjs` `metricsSp`: JS 有効・無効の両方で 3 指標が可視・top が同一・右端が枠内・数字のはみ出し 0・テキスト一致（Astra 是正: JS 無効 context と可視性を追加）。再撮影 `reaction6-detext-metrics-{sp,pc}`（同名置換）。
...
5. 撮影 `scripts/shots-reaction7.mjs`（reaction6 と同じ方式。Astra 是正: INDEX の書き出しを画像配置の直後・退避削除の前に移し、INDEX 更新の失敗でも画像を退避先から戻す）: 同名置換 2 + 新規 8、`CATALOG-INDEX.json` 447 → **455**。実機結果（`results/verify.json`、`--wpclidir` 指定）: `summary` **pass 63 / fail 0**（61 + `graphs` + `metricsSp`、skipped なし）、総合 `pass: true`。`style.css` 0.3.8 → 0.3.9。
...
## 2.22 段5 — PO 反応 16 回目（WT-EVT-0261〜0269）の反映

PO 発言（原文）: 「relatedにスライダーを追加。」「shareにSNS系のシェアを追加。」「icons-rowSNSアイコンつけるってことでOK？」「axis-depthこれはフロート演出できるか？」「テーブルは画像挿入やアイコン挿入ができるようにできる？」「そして購入リンクとか入れられるように。」「フッターに宣伝用のテーマ名を入れない？（笑）」「文章は本記事にはプロモーションが含まれます。でいいよ。」（PR 表記の画像を指して）。同じ回に LP 拡充・LP デザイン再収集の指示（WT-EVT-0268）と、HP 未定義・イベントページ・要求漏れの問い（WT-EVT-0269）があり、これらは本 PR では記録のみ（§8）。

1. **related:slider（WT-EVT-0261、Claude 案）**: `related` 軸に `slider` を追加。1 画面 1 枚（600px 以上は 2 枚）の scroll-snap、`article.js` が前後ボタンとドット（44px、現在ページは `aria-current`。左右キー / Home / End で隣のドットへフォーカスを移しつつ送る。ARIA の tab 役割は使わない）を生成。画面幅が変わるとページ数を再計算してドットを作り直す。自動送りはしない。（Astra 是正: tab 役割だけ付けてキーボード操作が無かった点、リサイズでドット数が古いまま残る点）JS 無効時は横スクロールのみ（ボタン・ドットなし）。既存の carousel（複数枚・8 割送り）は変更なし。
2. **SNS シェア（WT-EVT-0262。PO の確認質問「icons-rowSNSアイコンつけるってことでOK？」に対する Claude 解釈「icons-row に SNS アイコン」で実装、PO 回答待ち）**: `parts/article-tail.html` の共有先 slot を `patterns/share-sns.php` に置換。X / LINE / はてなブックマーク + リンクコピー。href は PHP（`get_permalink` / `get_the_title` を rawurlencode）で生成し JS 無効でも動く。`target=_blank rel="noopener nofollow"`、44px 円形。アイコンは各社ロゴ（商標）を使わず文字グリフ（X / LINE / B!）。共有先の URL は各サービス公式の共有エンドポイント（機能上必要な第三者ドメインで、調査対象の固有名ではない）。**Facebook は未収録**: 共有 URL のパス（sharer）が公開情報検査の非公開 guard（`.public-safety.local.regex`）に含まれており、検査を通すための例外追加は「理由・owner・期限を持つ PO 判断」（`docs/governance/public-repository-safety.md`）のため、PO 判断待ちとした。判断が出れば 1 行追加で対応できる。
3. **depth:float（WT-EVT-0263、疑問形 → Claude 案）**: `depth` 軸に `float` を追加。depth-2 の影に加え、hover で `translateY(-8px) scale(1.01)` と大きな影、`motion:on` では商品カードに 4 秒周期の浮遊 `@keyframes wt-float`（hover 中は停止）。reduced-motion では animation / hover transform とも無効。reveal（`html.wt-js body.wt-motion-on .wt-reveal.is-in{transform:none}`）に負けないよう hover 規則の特異性を上げた（実機で hover の transform が none になる不具合を verify で検出して是正）。
4. **比較表 rich（WT-EVT-0264 / 0265、Claude 案）**: block style `wt-compare-rich`「比較表: 画像・アイコン・購入リンク」。製品画像の行（`img.wt-tcell__img` 96px、SP 72px）、○×アイコン付きセル（`.wt-tcell__icon--ok/ng/mid`）、購入ボタンの行（`a.wt-tbtn`、44px 以上、`rel="sponsored nofollow"`）。先頭列 sticky（先頭セルは `functions.php` で `th[scope=row]` に変換されるため `th:first-child` も対象。Astra 是正）、SP は横スクロール。catalog `#cat-table-rich`。
5. **フッター クレジット（WT-EVT-0266、疑問形（笑）→ Claude 案、既定 none）**: `footer_credit` 軸 `none | text`。`parts/footer.html` の法的表記の下に「Theme: HELIX WT」（`rel=nofollow`）を右寄せ小文字で 1 行。既定では非表示。
6. **PR 表記の既定文言（WT-EVT-0267、PO 決定）**: `functions.php` の自動挿入文・catalog `#cat-pr`・`article-kit.php` の文言を「本記事にはプロモーションが含まれます。」に統一（旧「本記事にはアフィリエイト広告を含みます。評価・掲載順は報酬額で決めていません。」）。他の pr-notice 4 型の文言はプレースホルダ（型の説明文）のまま。`pr:auto` の述語正規表現に「含まれ（ます / る / て）」を追加し、既定文言「本記事にはプロモーションが含まれます。」を本文に書いた場合も重複抑止が働くようにした（Astra 是正: 旧述語は「含み / 含む / 掲載 / 表記」のみで「含まれます」を検出できなかった）。
7. **verify**（`--wpclidir` 指定、**pass 69 / fail 0**、skipped なし）: 追加 6 項目 `prNoticeText`（実記事 pr:on と catalog の文言一致）／`relatedSlider`（SP 1 枚・PC 2 枚、snap、ドット数 = ページ数、current 1、44px、前後ボタンが 2 つで「次へ」を押すと実際に送られ current が 2 枚目へ移る、PC 幅→SP 幅でドットが枚数どおりに作り直される、1.5 秒待って自動送りなし、JS 無効時はドットなしで横スクロール到達可）／`shareSns`（3 サービス、permalink を含む href、_blank + noopener、44px、外部ホスト、JS 無効でも同じ）／`depthFloat`（motion:on で animation `wt-float`、hover で transform が CSS の期待値 `matrix(1.01, 0, 0, 1.01, 0, -8)` に一致し影が変化し animation 停止、reduced-motion で両方なし。浮遊中は locator が "stable" にならないため DOM API でスクロールし `mouse.move` で hover）／`tableRich`（画像 3 が alt 付きで読み込み済み（lazy のため viewport に入れて待つ）、アイコン 6 以上、ボタン 3 が 44px 以上 + sponsored nofollow、SP は overflow-x auto で枠内）／`footerCredit`（既定 none で非表示、text で法的表記の下に表示、nofollow）。`relatedQuality` は 7 型 × 2 = 14 件に拡張（slider も同一行高・2 行 clamp・16:9 を満たす）。
8. **撮影 `scripts/shots-reaction8.mjs`**（reaction7 方式）: 同名置換 4（`pr-notice-one-line-*`、`tail-share-icons-row-*`）+ 新規 8（`related-slider-*`、`axis-depth-float-*`（hover 中）、`table-rich-*`、`footer-credit-text-*`）。撮影前に PR 文言・rich テーブルの要素数・body class・ドット数・SNS 3 件・クレジット可視を検査。`CATALOG-INDEX.json` 455 → **463**。`style.css` 0.3.9 → 0.3.10。
9. 記録のみ（本 PR で実装しない）: WT-EVT-0268（LP へ インタビュー記事カード（サマリー表示）・口コミ評価・外部評価・資料ダウンロード・埋め込みフォーム・LINE 導線・フロートボタンを入れられるか／LP が弱いのでリスティング広告のリンク先からデザインを再収集せよ）→ 次段: 台帳 `2026-09-05-parts-pattern-taxonomy` に LP 面の再収集を追加してから LP パーツ試作。広告のクリックは広告主に課金が発生するため、広告のリンク先 URL を控えて直接（または検索結果経由で）read-only 取得する方針を PO へ提示。WT-EVT-0269（HP が全く定義されていない／LP 以外にイベントページ／要求漏れが多い）→ 現状: L1 `screen.md` WT-SCR-06 に「公開面: ホーム / LP / 一覧」の名はあるがホームの構成要求は無く、イベント（セミナー・キャンペーン）ページは L1 に無い。WT-FR-PAGE-01 は会社概要・問い合わせ等の固定ページのみ。PO 反応 9〜16 から要求漏れ候補を洗い出して L2 の問いとして一覧化する作業を提案（Claude 案。要求の追加は PO 判断）。

## 2.23 段5 — LP パーツ 7 種（WT-EVT-0268、台帳 lp-recapture を根拠にした Claude 案）

PO 発言（原文、WT-EVT-0268）: 「LPにはインタビュー記事カード（遷移リンクではなくサマリー表示）、口コミ評価、外部評価、資料ダウンロード系、埋め込みフォーム、LINE導線、フロートボタンまでいけるか？まだLPは弱すぎるからリスティング広告とかクリックしてデザインを再収集してきてくれるか？」

再収集は台帳 `docs/research/2026-09-05-parts-pattern-taxonomy/lp-recapture/`（39 件・5 区分、PR #158）に置いた。本節はその「観察された多数派の型」を各パーツの既定にして試作 03 の LP（`page-lp`、`patterns/lp.php`）へ 7 種を足したもの。要求・設計の決定ではない。

1. **区間の追加**: `lp_sections` に `extended`（段4 の 12 区間 + 新 6 区間 = 18 区間）を追加。新区間と order: interview 45（事例）・review 55（口コミ）・download 85（資料 DL）・rating 95（第三者確認）・form 115（申込フォーム）・line 118（LINE）。full / short / trust の表示は変更なし（`lpSections` 検査は 4 型）。
2. **型の軸（既定 = 台帳の多数派）**: `lp_interview` summary-card（顔写真・肩書・見出し・効果数値・要約、遷移なし。台帳 4/7）/ link-card（記事へ遷移）/ logo-only。`lp_review` quote-photo（引用 + 顔写真。6/9）/ stars-count（★平均 + 件数 + 分布バー）/ satisfaction-number（満足度の大数字 + 調査条件）。`lp_rating` certification（認証・許認可枠。5/14）/ client-logos / award-badge（受賞・ランキング・掲載枠。固有名なし）。`lp_download` button-to-form（8/13）/ form-inline（氏名・メール同居）。`lp_form` external（別ページへ遷移。7/14）/ inline（5 項目 + 同意）。`lp_line` button（2/3）/ qr（PC は QR 枠、SP はボタンに切替）。`lp_fixed` に `line-sticky`（左下追尾 LINE ボタン。float-cta と同位置で同時に 1 つ。PoC のリンク先は全構成に存在する `#contact`）を追加。実装時、`.wt-lp-fixed{display:none}` が同じ特異性の `.wt-lp-line__btn{display:inline-flex}` に後勝ちで負けて追尾ボタンが常時表示になる不具合を `lpVisibleAnchors` の失敗（可視な `#line` リンク）で検出し、2 クラス指定で是正。verify `lpParts` に「軸なしでは非表示」を追加。各区間には全型を置き、body の軸 class で 1 型だけ表示（hero と同じ方式）。
3. **公開・法令上の配慮**: 事例・口コミ・数値は PoC 用の架空文と明記。受賞名・認証名・媒体名・LINE の実 URL は使わない（リンクは `#line` 等のダミー）。QR は実コードではなくダミー模様の SVG（aria-label に明記）。フォームは送信ボタンを `type="button"` にし（submit ボタンなし・入力 2 つ以上で暗黙送信も起きない）、`action="#…" method="get"` として実際にどこにも送らない。注記もそのとおりに記載（Astra 是正: 当初は `method=post action=/lp/` のまま「送信されません」と書いていた）。星の色 `#d97706`、LINE ボタンの緑は白文字とのコントラスト 4.5:1 以上になる `#0a7d3a` を採用（LINE 社のブランド緑 `#06c755` は白文字で 2.26:1 と不足するため使わない。ロゴも使用しない）。
4. **verify** `lpParts`: 15 型 × SP/PC で「body に軸 class・当該型だけ表示・テキストあり・タップ要素 44px 以上・全入力に label・画像に alt と読み込み」、form は external なら form 要素なし + リンクあり／inline なら form あり、download は form-inline だけ form あり、line qr は PC で QR 可視・SP ボタン非表示、SP で逆。追尾 LINE は fixed・44px 以上・viewport 内・float-cta 非表示。JS 無効でも表示型・テキストが同じで、JS 無効の結果にも同じ判定（要素の下限・44px・label・form の送信不可）と追尾ボタン判定をかける（Astra 是正）。必要要素の下限（画像枚数・入力数・タップ要素数）を型ごとに定め、空配列の every() で合格しない。`lpSections` に extended の順序を追加。
5. **撮影 `scripts/shots-reaction9.mjs`**（reaction7/8 方式）: 新規 41 枚 = extended の分割（SP 3 + PC 6）+ 15 型 × 2 + line-sticky × 2。`CATALOG-INDEX.json` 463 → **504**。撮影環境の実測: extended（SP 11,576 css px / PC 8,556 css px）は要素撮影・fullPage とも DPR1 でも "Unable to capture screenshot"、PC は 1440×4000 / 2200 の viewport でも "Page crashed" となったため、SP 4000px・PC 1600px の viewport（DPR1）で上から順に分割撮影し、dev ごとにブラウザを起動し直す方式にした（1 インスタンスで SP→PC を続けると PC の goto で落ちる）。分割数は撮影前に区間群の高さから決めて予定集合の照合に使う。分割数が減った再撮影では、予定集合に無い旧チャンクを INDEX から外し画像も退避（成功時削除・失敗時復元）する。
...
本節は上記のうち試作 03 の記事面・LP・footer に閉じる 6 件（0270〜0273、0275、0276）と、0262（icons-row）の解釈反映（SNS グリフ）。カテゴリ面の強化リサーチ（0274）、HP 面・イベントページ（0277）は別 PR（リサーチ台帳 → Astra レビュー（0278）→ 試作）。要求・設計の決定ではない。

1. **ヘッダー +8 型（0270、Claude 案）**: PC は台帳 §1 の header レイアウト観察型から center（logo-center-nav-below）/ two-rows / tel（with-tel）/ band（テーマ A/B の帯色型）/ overlay（transparent-over-hero、`eyecatch:hero` と併用）。SP は `sp` 軸に cta（≡ + CTA、台帳 13%）/ text-nav（≡ なしテキストナビ、10%）/ center-logo（検索左・ロゴ中央・≡ 右）。overlay は sticky を外して絶対配置にし、hero 画像を画面幅いっぱいに広げ（本文幅の画像だと PC の左右でロゴ・ナビが白地に白文字で消えることを撮影で確認して是正）、上端 72px を α .82 の一定スクリム（白背景の画像でも白文字 4.5:1）にして白文字を読ませる（JS の部分固定は作用しない）。verify は文字の矩形が画像の矩形内にあることも判定する。SP の cta / text-nav 用要素は pattern `header-sp-extras` として全ヘッダー部品に含め、`body.wt-sp-*` の SP 幅でだけ表示する。
2. **announce の全幅化（0271）**: `.wt-announce__in` の `max-width:1120px` を外し、左右 gutter だけ残す。
3. **numbox × num の連動（0272、Claude 解釈）**: h2 が `is-style-wt-numbox` の本文で、h3 の `is-style-wt-num` を「親番号-連番」（01-1, 01-2 / 02-1）にし h2 ごとに振り直す。適用範囲（Astra 是正 2〜3 巡目で確定）: 「numbox の h2 と同じ親コンテナ（post-content 直下でも Group 内でも）にある num の h3」だけを親番号付きにし（`:is(.wp-block-post-content,.wp-block-post-content *):has(> .is-style-wt-numbox) > …`）、numbox を（深さを問わず）含むコンテナの直後の兄弟から wt-h3 を 0 に戻す（入れ子 Group の外へ出る境界も対象）。したがって numbox を使わない記事・別コンテナの単独 num は従来の単独連番（01 から）、h2 番号（wt-h2num）は本文全体で続く。同じ親コンテナ内に通常 h2 が混在する場合は h2 ごとに連番を振り直すが親番号は進めない（想定外の混在）。カタログ `#cat-h2-numbox-plain` 以降に「専用 class なしの Group → post-content 直下の単独 num → 後続の別 Group → 単独 num → 入れ子 Group（外側 → 内側に numbox）→ 単独 num → 入れ子 Group → 段落 → 単独 num ×2」を置き、verify が実描画文字列で N / N-1 / N-2 / 01 / N+1 / (N+1)-1 / 01 / N+2 / (N+2)-1 / 01 / N+3 / (N+3)-1 / 01 / 02（最後の 2 つは直後が段落のときの 0 リセットと後続への継承）（N = それより前の numbox h2 の数 + 1。カタログでは h2 型の見本があるので 02）を照合する。直後の兄弟が num の h3 自身の場合は counter-set が counter-increment の後に適用されるため 1 に set する。
...
7. **SNS アイコン（0262 の解釈を反映）**: footer の SNS slot の数字プレースホルダ「1 2 3」を 4 導線（X / 写真共有 / 動画 / チャット。aria-label は #157 導入済みの X / LINE 以外は汎用名（写真共有・動画）で、本 PR で第三者サービス名を増やさない、href は `#` のダミー）の自作グリフに、記事末 icons-row の X / LINE も同じグリフに（はてなは文字 B!）。グリフは交差線・カメラ形・再生ボタン付き枠・吹き出しで、各社のロゴ画像・ロゴの形を模した図形は使わない（Astra 是正: 当初の写真共有グリフは角丸四角 + 円でロゴに近かったためカメラ形に変更）。サービス名と共有 URL 自体は #157 の icons-row で導入済みで本 PR で増やしていない。
8. **verify +7**: `headerVariants`（PC 5 型 + SP 5 型 × JS 有効 / 無効 = 20 判定: 軸 class・部品描画・ナビ可視・SP は全操作要素 44×44、PC はナビ文字リンク以外の操作要素 44×44・型固有部品（center のナビ下段 / two-rows のナビ帯 6 リンク + CTA + 検索 / tel の tel: リンク + CTA）・band は背景 = accent かつ文字 4.5:1・overlay は absolute / 透過 / hero 上端 < 60px / 白文字 + 影 / 上端スクリム α ≥ .82（白背景の画像でも 4.5:1）・JS 無効で同じ要素数）、`announceFullWidth`（可視・左端 ≥ 0・右端 ≤ viewport・幅 ≥ viewport − gutter×2 かつ ≤ viewport。PC は 1120 の旧上限超え）、`numboxNum`（期待番号 01 / 01-1 / 01-2 / 02 / 02-1 を DOM から組み立て、CDP DOMSnapshot で ::before の実描画文字列（counter の解決値）を読んで見出しごとに完全一致。h3 の counter-increment を 2 にする変異で h3 のラベルが全部ずれることも合格条件。counter-set / counter-increment の値も完全一致。CSS の適用範囲は「numbox の h2 と同じ親コンテナ（post-content 直下でも Group 内でも）にある num の h3」で、同じページの別コンテナの単独 num（`#cat-h3-num`）が 01 のままであることも判定）、`graphsMore`（5 型 × PC / SP / SP JS 無効: 視覚要素が実際に可視（矩形・display・visibility・opacity）・表の値と視覚値の照合（grouped は --v / data-v / 棒幅の比、column は --v と棒高と値の順、score は data-v と塗り数、gauge は --v と表示値、radar は 5 点 × 2 系列 + 凡例 2 + 軸ラベル 5）・role=img の数を型ごとに固定・figcaption・SP の横はみ出しなし・JS 無効で同じ）、`relatedNoFixture`（6 + 1 件、既定カテゴリ名は wp-cli の `default_category` から取得（取れないときは fail、source に理由）、自記事なし、画像は読込済み naturalWidth > 0）、`lineIcon`（qr 型と button 型 × PC / SP: 可視 mark（qr 型 PC は追尾 1、他は 2）がすべて描かれたグリフ（mask-image あり・16px 以上・色が透明でない）・文字 LINE なし、qr の PC / SP 文言切替は要素数 2 ずつ、ボタン文言 / aria-label に LINE）、`snsIcons`（footer 4 導線 + 記事末 3 導線（x / line / hatena の順）、描かれたグリフ（はてなは文字 B!）・aria-label・44×44・footer は rel トークンに nofollow、JS 無効でも同じ）。
9. **撮影 `scripts/shots-reaction10.mjs`**（reaction7〜9 方式 + Astra 是正: 画像の読込失敗 / 3 秒超過は撮影失敗として例外、viewport 撮影でも画像読込を待つ、results/ に O_EXCL のロックファイルで同時実行を防ぐ。強制終了時はロックと退避 dir が残るので手で確認して消す）: 新規 22 枚（ヘッダー PC 5 + SP 5、numbox×num 2、グラフ 10）+ 同名置換 98 枚（announce 2、LINE 6、footer 56、関連グリッド 32、icons-row 2）。`CATALOG-INDEX.json` 504 → **526**。予定集合は「置換予定は INDEX 登録済み・新規予定は未登録」も照合する。
10. `style.css` 0.3.11 → 0.3.12。用語集に header 8 型・sp 3 型・h2「numbox + h3 num」・graph 5 型・lp-line / footer-extra sns / icons-row の説明を追加。
11. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 77 / fail 0**（70 + 7、skipped なし）、総合 `pass: true`。初回実行で 4 件が落ちた是正: 既存 `graphs` は figure 全件を数えて 9 ≠ 4 になったため当初 4 型に限定（追加 5 型は `graphsMore`）、`headerVariants` は PC のナビ文字リンクを既存 tap 監査と同じくインライン扱いにし（CTA・電話・検索は 44px を要求）、overlay の hero 上端は「ヘッダー高 60px 未満」に、`numboxNum` は幅比から「期待文字列を同フォントで描いた幅との一致」に置き換え、`lineIcon` は追尾ボタンの文言が aria-label 側に LINE を持つため aria-label を含めて判定。 Astra 是正後の再実行で落ちた 2 件の是正: `headerVariants` は two-rows / band の PC 検索欄（入力・ボタン 40px）と text-nav の短いリンク（幅 < 44px）が 44px 未満、overlay は main の block-gap 由来の上余白 16px で文字矩形が画像外に出ること、JS の明度判定（`data-wt-lum` の `!important` 規則）が上端スクリムを上書きすることの 4 点を CSS で直した（検索欄 44px、text-nav リンク min-width 44px、overlay+hero で main の margin-top 0、lum 3 値それぞれに上端 72px α .82 を持つスクリムを再定義）。`lineIcon` は qr 型の PC では QR 枠内の SP ボタンが非表示で可視 mark が追尾 1 つになるため、期待数を PC 1 / SP 2 に分けた。

## 2.34 段12 — ヘッダー・フッター・固定 CTA の所属を面ごとに（PO 反応 25 回目 WT-EVT-0302「増やす方向で」）

1. 入力: PO 反応「増やす方向で」（段 11 の報告への 3 語の反応。対象は明示されていない）。Claude の解釈（events.jsonl WT-EVT-0302 に記録、PO の確認待ち）: WT-EVT-0300 の前提（選択肢と対応可能範囲を最大化）に沿い、(a) 未決「lp_form / event_apply を段 11 のブロックへ置き換えるか」は置き換えず選択肢として足す方向（本段では未着手）、(b) 次段 = 共通設定の残り（ヘッダー・フッター・固定 CTA）は段 10c のサイドバーと同じく 継承（共通）/ 独自設定（面別 own）/ 非表示 を全面に用意する。根拠は WT-EVT-0296 ①「共通のヘッダー、フッターで全体制御」と WT-EVT-0299「継承・独自設定・非表示を分離」。
2. 設計: 束の台帳 `wt_chrome_bundles()` = `site`（共通。既存 `header` / `sp`、`footer_layout / above / legal / extra / totop / credit`、新設 `fixed`）と `own`（面専用。`own_<face>_header` / `own_<face>_sp` / `own_<face>_footer_*` / `own_<face>_fixed`。HOME・イベント・LP の固定 CTA は既存 `home_fixed` / `event_fixed` / `lp_fixed` をそのまま own の束にする）。面（HOME / 記事 / カテゴリ / イベント / 固定ページ / LP）ごとに所属を選ぶ軸 `<face>_head` / `<face>_foot` / `<face>_fix`（カテゴリは `cat_`）: `site` / `own` / `off`（非表示 = template part を描画しない、固定 CTA は出さない）。LP のヘッダーは既定 `lp`（テンプレート内の LP ヘッダー 3 種 = `lp_header`）を残し、`site` / `own` を選ぶと共通ヘッダーの template part を LP にも描く（`page-lp.html` に template part を追加、LP ヘッダーは class で隠す）。`width` は共通のまま（束にしない）。404 等の他面は共通。
3. 既定: すべて現状維持（ヘッダー・フッター = 共通 site。ただし LP のヘッダーは `lp` = テンプレート内の LP ヘッダー。固定 CTA = HOME・イベント・LP は own、記事・カテゴリ・固定ページは共通の `fixed:none`）。共通の `fixed` の 4 型（none / float-cta / sp-bottom-bar / float-tel）は HP の `home_fixed` と同じ（Claude 案）。
4. 実装（`functions.php` 0.3.20）: 軸は台帳から導出（own の束は共通軸の定義をコピー、所属軸の許可値は `array_keys( wt_chrome_bundles() )` + off。軸は 1 + 6 × (2 + 6) + 3 + 18 = 70 本増）。`wt_chrome_bundle($part)` / `wt_chrome_eff($key)` / `wt_chrome_fixed_type()`。body class は実効値（`wt-header-* / wt-sp-* / wt-footer-*-* / wt-fixed-*` は束の実効値、生の値は `wt-raw-*`）+ `wt-head-bundle-* / wt-foot-bundle-* / wt-fix-bundle-*` + `wt-fixed-eff-*`。ヘッダー part の差し替えは実効値で、`render_block` で off（と LP の lp）は header part を、off は footer part を空にする。共通の固定 CTA と own_<face>_fixed（記事 / カテゴリ / 固定ページ）は `wp_footer` で generic 描画（`.wt-fixed--*`、導線は実在ページ /contact/ と PoC のダミー番号）。HOME・イベント・LP のパターンの固定 CTA は所属が own のときだけ body class が付く。`<meta name="wt-chrome-bundles">` / `wt-chrome-faces` を verify が読む。
5. CSS: `.wt-fixed--*` 3 型（HP の固定 CTA と同じ見た目）、sp-bottom-bar のときフッター下端の余白と to-top の位置、LP で所属が lp 以外のとき LP ヘッダー 3 種を隠す。
6. verify `chromeOwner` 新設: 台帳（束 2・面 6）を meta から読み、面 6 × (ヘッダー site / own / off + own の面別分離 1 + LP の lp 1) × PC / SP / SP JS 無効 = 25 × 3 行（own は `own_<face>_header:tel` と `own_<face>_sp:cta` = SP で CTA が出る、共通は header:cta / sp:search のまま）、フッター site / own / off × 6 面 × PC / SP = 36 行（own は `own_<face>_footer_layout:single-row` + `own_<face>_footer_above:cta-band`、共通は columns-3 / none のまま）、固定 CTA site / own / off × 6 面 × PC / SP / SP JS 無効 = 54 行（site = 共通の型（SP は float-tel、PC は float-cta）だけが出る、own = 面の own の型（float-cta / float-apply）だけが出て generic 描画は `data-wt-from=own`、off = 1 つも見えない）、off + サイドナビ 6 行（PC。`side_nav:fixed-right-icons` はサイドバーの束に所属し固定 CTA の off で消えない。LP は束なしで 0）、既存の固定ボタンとの重なり 4 行（記事、`share:float` + `footer_totop:button`、SP 3 型 / PC float-cta: 矩形交差 0・共有ボタンは viewport 内・totop は非表示）、他面（404）は共通 1 行 = **176 行**（固定値）。site は共通軸の値（header:cta / footer_layout:columns-3 / fixed:float-cta）、own は own_<face>_*（tel / single-row / float-cta。HOME・イベント・LP の固定 CTA は home_fixed / event_fixed / lp_fixed）だけを見て他面の own を読まない、off はヘッダー / フッターの要素が無い、LP の lp は共通ヘッダー無し + LP ヘッダー 1 つ。h1 は 1 つ。
7. 撮影 `scripts/shots-reaction18.mjs`（reaction7〜17 方式）: 面 6 × (ヘッダー own PC / SP、ヘッダー off PC、フッター own PC、フッター off PC（ページ末尾）、固定 CTA site SP（sp-bottom-bar）/ PC（float-cta）) + LP に共通ヘッダー PC = **43 枚**（すべて新規、stale 0）。INDEX 931 → **974**。用語集 parts chrome-head / chrome-foot / chrome-fix、axes fixed。
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 89 / fail 0**（88 + `chromeOwner`、skipped なし）、総合 `pass: true`。`chromeOwner` 176 行 全 pass（Astra 1 巡目後。初版 148 行）。既存 88 検査の合否は変化なし（保存値のうち pageParts の数値と depthFloat の transform は実行ごとに変わる測定値）。
9. 制約・未決: 「増やす方向で」の対象の特定（未決 2 件か次段か全体方針か）は PO の確認待ち。ヘッダーの own の束は `header` / `sp` の 2 軸（`width` は共通）。所属境界: サイドナビ（`side_nav` の fixed-right-icons / fixed-left-nav / drawer-pc 等）とサイドバーは段 10c のサイドバーの束に所属し、固定 CTA の off では消えない（サイドバー側の所属で切る）。フッター off は法定表示（copyright・法定リンク）も出ない（非表示 = 描画しない。LP の `lp_legal` は LP 面の区間なので別）。固定 CTA の generic 描画は HP と同じ 4 型で、イベントの float-apply（申込導線）は own のときだけ。lp_form / event_apply への段 11 ブロック追加は未着手（次段候補）。既定値は現状維持で、既定の変更は PO 判断。
10. Astra 1 巡目（head e9f098e、重大 0・改善 3・軽微 2、merge 不可）の是正: (改善 1) generic の固定 CTA（float-tel）が追尾の totop と同じ右下、sp-bottom-bar / float-cta が共有の浮遊ボタンと重なった → HP / イベントと同じく generic の固定 CTA がある面では totop を出さず footer 下端に余白、共有ボタンは下部バー / 電話ボタンの上へ退避、float-cta は共有ボタンぶん幅を詰める。verify に重なり 4 行。(改善 2) 記事・カテゴリ・固定ページで共通と own が同じ float-cta で own が共通を読んでも通った → 共通は SP float-tel / PC float-cta と own（float-cta）で型を分け、generic 描画に `data-wt-from`（site / own）を付けて照合。(改善 3) off でサイドナビの導線が残る組合せ → 所属境界を README 項目 9 に明記し、verify に off + `side_nav:fixed-right-icons` 6 行と固定 CTA の SP JS 無効 18 行。(軽微) 項目 3 に LP のヘッダー既定 lp を併記、「既存 verify は変化なし」→「既存 88 検査の合否は変化なし」。148 → 176 行。

## 2.35 段13 — LP / イベントに段 11 のフォームブロックを選択肢として追加（PO 反応 26 回目 WT-EVT-0303「OK」）

1. 入力: 段 12 の報告で確認した「増やす方向で」（WT-EVT-0302）の解釈 = 置き換えず選択肢を足す、に PO「OK」。LP の `lp_form` とイベントの `event_apply` に段 11 のフォームブロック（`helix-wt/form`）を新しい型として足し、既存の型（external / inline、inline-form ほか 6 型）は残す。
2. 実装（`functions.php` 0.3.21、`inc/form.php`、`patterns/lp.php` / `event.php`、CSS）: `lp_form` に `block`、`event_apply` に `block-form`。パターンの区間内に動的ブロック `<!-- wp:helix-wt/form /-->`（wp:html を分割して置く。段 8 の教訓: `<?php echo ?>` は静的化するので動的ブロック）。ブロックは軸で選んだときだけ描く（既定では DOM に隠しフォームを残さない）。区間には見出しがあるのでブロック属性 `hideTitle` でブロック側の見出し・リードを省く（h2 の重複を避ける）。種別は面の文脈で決める `wt_form_kind()`: イベントの block-form は `apply`（氏名 / メール / 電話 / 参加形式 / 人数 / 本文 / 同意）、LP と固定ページは `form_kind`（既定 contact）。完了（separate）の redirect は `?wt=` を引き継ぎ、文脈の種別が `form_kind` と違うときだけ `form_kind:<種別>` を足す（/thanks/ が同じ種別で出る）。
3. 実機で見つけた実バグ: フォームの POST 処理は `template_redirect`（テンプレート解決前）で走るため、イベント面の判定（`$_wp_current_template_id` / `is_page_template()`）が false になり検証が contact の項目で走った（項目が無いのでエラーも出ず入力に戻るだけ）→ `wt_page_template_by_slug()` で block 階層の規則（template meta が空なら page-{slug}.html。静的フロントページは front-page.html が優先するので除く）を自前で当て、`wt_is_event_page()` / `wt_is_lp_page()` の両方で使う。LP のフォーム区間は段 5 の `lp_sections:extended` のときだけ表示される（block も同じ）。
4. verify: `eventFace` の event_apply に block-form（43 型 × 3。フォームは POST・送信ボタンあり・同一 URL として検査）。`formSlots` 新設 **23 行**（固定値）: LP / イベント × PC / SP / SP JS 無効（ブロックが見え、種別が文脈どおり、他の型は見えない、区間の見出し 1 つでブロックの見出しは無い、POST・同一ホスト・nonce、項目の並び、イベントは hero の申込導線が残る、h1 1 つ）+ 既定では DOM にフォームブロックが無い 2 行 + JS 無効の不正値（イベント = 参加形式なし・人数 0、LP = メール形式）がサーバ検証で当該項目のエラーになる 2 行 + 入力 → 確認（種別・項目集合の完全一致・入力値と select の選択名を確認画面で照合）→ 完了で /thanks/ へ redirect し種別を引き継ぐ 2 行 + 境界（slug が event / lp でない固定ページは面 page のまま）1 行 + wp-cli で状態を変えて必ず戻す 7 行（LP の template meta を外して slug 解決 = 既定でブロック無し / block で出る 2、event / lp に別テンプレート page-canvas を明示割当すると面 page になりブロックも出ない 2、静的フロントページにすると front-page.html が優先し面は home 3 = イベント slug 解決 / イベント明示割当 / LP 明示割当）+ 変更前の状態が正常（退避に成功し、event / lp がフロントページになっていない = 前回の復元漏れの検出）1 行 + 明示割当からの復元の実証 1 行（イベントに page-event を割り当てた状態を変更前に見立て、page-canvas へ変えて戻す） + 状態の最終一致 1 行（LP / イベントの template meta と show_on_front / page_on_front が変更前と同じ、復元コマンドの失敗 0、実描画で /event/ がイベント面）。
5. 撮影 `scripts/shots-reaction19.mjs`: lp-form-block / event-apply-block-form × PC / SP = **4 枚**（新規、stale 0）。INDEX 974 → **978**。用語集 lp-form / event-apply に型を追記。
6. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 90 / fail 0**（89 + `formSlots`、skipped なし）、総合 `pass: true`。`formSlots` **23 行 全 pass**（状態の最終一致・明示割当からの復元の実証を含む）、`eventFace` 43 型 × 3 全 pass。既存 89 検査の合否は変化なし。
8. Astra 1 巡目（head 5cb3b52、重大 0・改善 3・軽微 0、merge 不可）の是正: (改善 1) slug 判定が静的フロントページ（front-page.html 優先）を無視していた → `is_front_page()` を除外し、verify で実際にイベントページを静的フロントページにして面が home になることを検査（必ず戻す）。(改善 2) LP を slug 解決（template meta 空）で開くと `wt_is_lp_page()` が false になり既定でも隠しフォームが DOM に残った → LP も同じ規則で判定し、verify で meta を外した状態を検査。(改善 3) 遷移検証が確認画面の種別・項目・値と JS 無効のサーバ検証を見ていなかった → 確認画面の照合と JS 無効の不正値 2 行、別 slug の境界 1 行を追加。10 → 16 行。
9. Astra 2 巡目（head bc912c6、重大 0・改善 2・軽微 1、merge 不可）の是正: (改善 1) `is_front_page()` の除外が slug 分岐だけで、明示割当したページをフロントページにすると `is_page_template()` で true になった → `wt_is_event_page()` / `wt_is_lp_page()` は静的フロントページで常に false（front-page.html 優先）、verify に明示割当のフロントページ 2 行。(改善 2) 確認画面の照合が select の選択名・company・項目集合を見ていなかった → 完全一致に、別テンプレート明示割当の境界 2 行。(軽微) 是正時の置換で §2.9 の実機結果行を書き換えていた → 復元し §2.35 項目 6 を修正。16 → 20 行。
10. Astra 3 巡目（head 0a402c1、重大 0・改善 1・軽微 0、merge 不可）の是正: イベントの template meta の変更前値を退避せず `""` 固定で戻していた（開始時に明示割当があると削除される）→ LP と同じく変更前値を取得して戻し、最後に LP / イベントの meta と show_on_front / page_on_front が変更前と一致する行を追加。あわせて実機で復元漏れを 1 回起こしていた（検証台の show_on_front が page / page_on_front がイベントページのまま残り、次の実行でイベント面が HOME になった。原因は未特定 = docker 経由の wp-cli の失敗が finally の先頭で残りを飛ばした可能性）ため、復元は各コマンドを個別に try/catch し失敗を行に残す、変更前の状態が疑わしければ行で fail、最終一致は実描画（/event/ がイベント面）まで見る。20 → 22 行。検証台は show_on_front=posts / page_on_front=0 へ手で戻した（試作 01 README の記載どおり）。
11. Astra 4 巡目（head 932ec45、重大 0・改善 1・軽微 0、merge 不可）の是正: 取得失敗を未設定（`""`）と同一視していたため、退避時に一度失敗すると明示割当を削除しながら before = after で通った → `meta list` / `option list`（未設定は exit 0 の空配列）で未設定と失敗（null）を区別、退避に失敗したら変更を始めない、復元のたびに再取得して変更前と照合し不一致は failures へ、最終一致は null を不一致とみなす。明示割当からの復元の実証行を追加。22 → 23 行。
7. 制約・未決: ブロックの 11 軸（form_layout 等）は `?wt=` で LP / イベントでも切り替わる（面別の独自設定は持たない = 共通）。イベントの block-form の種別 apply は固定（`form_kind` で上書きしない）。既定は据え置き（external / inline-form）。
