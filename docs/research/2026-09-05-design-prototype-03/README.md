...
8. 実機結果（`results/verify.json`、WP 7.1 ローカル、`--wpclidir` 指定）: `summary` **pass 88 / fail 0**（87 + `formFace`、skipped なし）、総合 `pass: true`。`formFace` 139 行（Astra 1 巡目後。初版 122 行 = 軸 33 × 3 + 形式検査 3 + 遷移 18 + steps 2）全 pass。INDEX 879 → **931**（reaction17: 52 枚すべて新規、stale 0）。
