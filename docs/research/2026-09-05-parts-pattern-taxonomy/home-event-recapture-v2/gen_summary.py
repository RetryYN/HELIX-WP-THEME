...
w(f"- イベント: hero は date-place-block が主集計 {pct(sum(1 for e in EM if e['hero']=='date-place-block'), len(EM))} で、前回台帳（前回台帳 home-event-recapture, 主集計 n=" + str(len(PEM)) + "）の " + " / ".join(f"{k} {pct(v, len(PEM))}" for k, v in Counter(e['hero'] for e in PEM).most_common(2)) + " と食い違う。両台帳の合算で判断せず、§5 の規則（両台帳で最多の型のみ暫定既定値候補）では候補にならない。date-place-block を暫定既定値へ上げるなら**規則の例外**（前回 n=8 の小標本より本台帳 n を優先する）になるため、例外提案として PO への問いに留める。info_block は icon-list が " + pct(sum(1 for e in EM if e['info_block']=='icon-list'), len(EM)) + "。apply は external-form / closed-notice が多く、inline-form は観察 0 件（" + str(sum(1 for e in EM if 'inline-form' in split(e['apply']))) + "/" + str(len(EM)) + "）。target-audience（対象者）・tickets/price・organizer が高頻度で、試作 03 の 11 区間に無い「対象者」「主催」「参加費」を区間として加える。")
w("- キャンペーン / コンテスト（D）: prizes / entry-steps / target-products / judges の区間と、応募方法（`other:receipt-upload` / `other:postcard` / `other:messaging-app`、主集計での出現 " + ' / '.join(f"{k} {pct(v, len(EM))}" for k, v in Counter(x for e in EM for x in split(e['apply']) if x.startswith('other:')).most_common()) + "）を **語彙 receipt-upload / postcard / messaging-app として正式採用し** event_apply の追加型として持つことを提案する（Claude 案。§0 の other:* 原則の明示的な例外）。")
print("\n".join(L))
