...
- 数値の読み方: 出現率の分母は「na（切り出し範囲に無い / 未描画）を除いたタグ出現数」。1 shot で複数の型が出た場合は各々を 1 と数えるため、分母は shot 数より大きい（例: header.layout top/PC は 547 タグ / 244 shot）。`aggregate.md` の n は na を含む総数。
