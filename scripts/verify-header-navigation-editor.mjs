...
const login=async(p,user,password)=>{await p.goto(`${contentLab.baseUrl}/wp-login.php`);await p.locator('#user_login').fill(user);await p.locator('#user_pass').fill(password);await p.locator('#wp-submit').click();await p.waitForURL('**/wp-admin/**');};
