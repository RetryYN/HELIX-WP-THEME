...
const blog = execFileSync('docker', ['exec', contentLab.wpContainer, 'php', '-r', 'require "/var/www/html/wp-load.php"; echo get_option("blogname");'], { encoding: 'utf8' });
...
try {
  for (const width of [1440, 375]) for (const js of [true, false]) for (const extra of ['none', 'sites']) {
    ...
    await page.goto(`${contentLab.baseUrl}/?wt=footer_extra:` + extra + ',footer_layout:sitemap');
