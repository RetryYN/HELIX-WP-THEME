...
const families = {
  ...
  LP: ['lp-'], RECO: ['related', 'category-ranking'], META: ['eyecatch', 'toc', 'share', 'side-'],
...
const requirements = ir.requirements.map(r => {
  ...
  return { id: r.id, family, statement: r.statement, priority: r.priority, revision: r.revision,
    ...
    status: 'not_verified', relatedEntryIds: related.map(e => e.id),
...
const result = { schema: 'wt-selection-catalog.v1', source: prototype, requirementCount: requirements.length,
  screenshotCount: index.length, faces: glossary.faces, entries: [...entries.values()], requirements,
...
console.log(`catalog: ${entries.size} candidates / ${index.length} screenshots / ${requirements.length} requirements`);
