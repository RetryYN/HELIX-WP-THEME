...
run_and_log "テーマB 登録 REST routes (API namespace)" curl -s http://localhost:8086/wp-json/swell/v1
run_and_log "テーマB ブロックエディタブロック一覧" wp eval 'foreach (\WP_Block_Type_Registry::get_instance()->get_all_registered() as $name=>$b) { if (strpos($name, "loos/")===0) echo $name . "\n"; }'
...
run_and_log "テーマB 主要オプション値 (テーマB option key)" wp option get SWELL_SETTINGS --format=json | head -50 || echo "no テーマB option"
run_and_log "テーマB 全オプション (テーマB option pattern)" wp option list --search='swell_*' --format=table
...
run_and_log "テーマA post_meta keys (テーマA meta-key pattern)" wp eval '$keys = $GLOBALS["wpdb"]->get_col("SELECT DISTINCT meta_key FROM {$GLOBALS[\"wpdb\"]->postmeta} WHERE meta_key LIKE \"_jinr_%\" LIMIT 50"); print_r($keys);'
run_and_log "テーマA オプション (テーマA option pattern)" wp option list --search='jinr*' --format=table
