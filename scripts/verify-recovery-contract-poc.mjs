...
try {
  ...
  const rows = flatten(report.suites).map(spec => ({ name: spec.title, pass: spec.ok && spec.tests.every(t => t.results.length === 1 && t.results[0].status === 'passed') })); assert(rows.length > 0 && rows.every(row => row.pass));
