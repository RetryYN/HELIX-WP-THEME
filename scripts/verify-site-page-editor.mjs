...
try{
 ...
 await page.goto(base+'/wp-login.php');await page.locator('#user_login').fill('lab_admin');await page.locator('#user_pass').fill(credentials.admin);await page.locator('#wp-submit').click();
 ...
 await page.getByRole('button',{name:'常設案内・規約',exact:true}).click();
 await page.getByRole('button',{name:'会社案内',exact:true}).click();
 ...
 fs.writeFileSync(path.join(root,'docs/research/2026-09-08-content-faces/results/site-pages/editor.json'),JSON.stringify({schema:'wt-site-page-editor.v1',completed,rows},null,2)+'\n');
}
