...
const sources=['scripts/verify-site-search.mjs','docs/research/2026-09-08-content-faces/plugin/search.php','docs/research/2026-09-05-design-prototype-03/theme/helix-wt/assets/css/theme.css','docs/research/2026-09-05-design-prototype-03/theme/helix-wt/templates/search.html'];
...
try {
 for(const [device,width] of [['pc',1440],['sp',375]])for(const js of [true,false]){
  ...
  for(const [state,q] of [['results','判断'],['empty','no-match-search-fixture-20260908'],['blank','']]){
   ...
   if(!js){const file=`after-${state}-${device}.jpg`;await page.screenshot({path:path.join(out,file),type:'jpeg',quality:80,fullPage:true});shots.push({file,device,state,query:q});}
  }
