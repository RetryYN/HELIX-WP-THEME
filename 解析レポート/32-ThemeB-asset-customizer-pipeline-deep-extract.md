# ThemeB アセット読み込み + カスタマイザーパイプライン + ad_tag CPT 深堀り

> 解析日: 2026-04-30 / 対象: ThemeB Theme v2.16.0 親テーマ
...
| `lib/load/front.php` L18-20 | `wp_enqueue_scripts` Pri:8 | `load_style_async` | `ThemeB::get_option()` で確認 | `style_loader_tag` フィルターで対象 handle を `media="print"` + `onload` に置換 |
...
| `lib/load/admin.php` L39-46 | `admin_enqueue_scripts` | `is_customize_preview()` | - | カスタマイザー画面で `themeB_admin/customizer` + `themeB_admin/widgets` を読み込み |
...
|---------|----------------|--------|----------|
| `init()` L16-51 | `wp_head` Pri:0 | 1. メインコンテンツ parse + render_block フック監視 / 2. ウィジェット出力内容チェック / 3. ページタイプ別強制セット | `ThemeB::$used_blocks` 静的配列 |
...
| `check_dynamic_sidebar()` L343-379 | `dynamic_sidebar` フック | ウィジェットクラスから `core/calendar`, `widget/rss` 等を自動検知 | `ThemeB::$used_blocks` に直接追加 |

### キャッシュ機構
- サイドバー用 separate list → コメント行で草案あり（L193-198）
- 本体は transient キャッシュ未使用（毎回 parse）
- ただし Style.php 内で `themeB_parts_style_common` キャッシュ（30 日）
...
| `body_font_family` | select | `yugo` | `--themeB-font_family`, `--themeB-font_weight` | Style/Body::font() L15-53 | `:root{}` |
| `post_font_size_pc` `post_font_size_sp` | text | `16px` / `4vw` | `--themeB-fz--content` | Style/Body::font() L18-19 | `:root{}` (with @media) |
...
| `mv_slide_effect` 等 | select | varies | - | global_vars_on_front() L305-310 | JS 変数 `themeBVars.mvSlideEffect` |

### CSS 生成キャッシュ戦略

| キャッシュ名 | 対象範囲 | Transient 名 | 有効期限 | Reset Trigger |
|------------|---------|------------|--------|----------------|
| 共通スタイル | 全ページ（フロント共通） | `themeB_parts_style_common` | 30 日 | キャッシュクリア / カスタマイザー変更時に admin_toolbar で DELETE |
| ページタイプ別 | トップ・投稿・固定ページ等 | `themeB_parts_style_{page_type}` | 30 日 | 同上 |
...
- 投稿メタ `themeB_meta_no_mb` で個別 CSS override 可能（Style.php L538）
...
| `ad_star` | str | `0.5` ~ `5` (0.5 刻み) | select dropdown | `ThemeB_PARTS::review_stars()` で★表示 |
...
|--------|---------------|--------|------|
| 広告ブロック検知 | Pre_Parse_Blocks::check_content_str() L291-295 | `themeB/ad-tag` フラグ | CSS separate 判定用 |
...
**含意**: AGENT NEO が類似テーマを構築する際、この 2 層フィルターパターンを採用すれば、カスタム CPT・カスタムブロック毎の条件付きロード機構が実現でき、ThemeB と同等のパフォーマンス最適化が可能。`asset-policy.schema.json` でこの 2 層を明示し、AGENT NEO ブロックは block.json の `assets` 宣言で第 2 層自動連携を実現できる。
...
**含意**: AGENT NEO 個人版の広告タグ CPT は、ThemeB ad_tag をベースにしつつ以下を追加すべき:
...
### 4. 補足: ThemeB の弱点として AGENT NEO が改善すべき点
...
**分析対象**: ThemeB Theme v2.16.0 親テーマ
